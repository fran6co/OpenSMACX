...
def promote_outputs(verify_dir):
    ...
    Promotion removes the round trip. It does NOT preserve the evidence, and
    this docstring used to claim otherwise: it said the comparison "still fails
    on a nondeterministic exporter, because a second regeneration into the
    verify directory would differ from the promoted bytes". There is no second
    regeneration. `main()` regenerates once, promotes those bytes over the
    committed ones, and would then compare the two - which are equal by
    construction. The comparison could not fail, and a test asserted that it
    passed.

    So a promoting run now returns without comparing and without stamping, and
    says so. The real comparison is the next UNPROMOTED run, whose own
    regeneration is independent of this one.
    """
    promoted = []
    for name in COMPARED_OUTPUTS:
        generated = verify_dir / name
...
def main():
    ...
    if args.promote and promote_outputs(verify_dir):
        # A PROMOTING RUN CANNOT CERTIFY DETERMINISM, and must not pretend to.
        # compare_outputs() would compare verify_dir against docs/recovery/ -
        # bytes promote_outputs() just copied OUT of verify_dir. They are equal
        # by construction and the comparison cannot fail. This docstring used to
        # claim "a second regeneration into the verify directory would differ
        # from the promoted bytes"; there is no second regeneration in main().
        #
        # So do not stamp. The stamp is keyed on the promoted tree, and the next
        # run - the gate, in its own per-preset verify directory - would find it
        # and return `cached` without regenerating. That is how the check came to
        # never execute anywhere: promotion made it vacuous, then the stamp made
        # the gate skip it. Leaving this run unstamped costs the next one a warm
        # regeneration (~3 s a lane; the cold IDB reopen is ~312 s and is not on
        # this path) and buys back a comparison between two INDEPENDENT
        # regenerations, which is the only form of it that means anything.
        print("verify-recovery-metadata: NOT stamped, and this run verified "
              "NOTHING about determinism - a promoting run compares the "
              "regenerated bytes against copies of themselves. The next "
              "unpromoted run performs the real comparison.")
        return 0
