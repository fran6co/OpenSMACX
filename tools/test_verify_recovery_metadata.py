...
class VerifyRecoveryMetadataTests(unittest.TestCase):
    ...
    def test_promoting_run_neither_compares_nor_stamps(self):
        # A promoting run copies the regenerated bytes over the committed ones,
        # so comparing the two afterwards cannot fail - and stamping would let
        # the NEXT run (the gate) skip the only comparison that is real. This
        # test used to assert the opposite: that promotion re-keyed and wrote a
        # stamp. That is how the determinism check came to never run anywhere.
        idb = self.root / "test.idb"
        idb.write_bytes(b"idb-bytes")
        verify_dir = self.root / "verify"
        cache_dir = self.root / "cache"
        argv = ["verify_recovery_metadata.py",
                ...
                "--promote"]
        with mock.patch.object(verify_recovery_metadata, "run_regeneration"), \
                mock.patch.object(verify_recovery_metadata,
                                  "compare_outputs") as compare, \
        ...
        self.assertEqual(list(cache_dir.glob("*.stamp")), [],
                         "a promoting run must leave no stamp, or the gate "
                         "returns `cached` and never regenerates")

    def test_promote_that_copies_nothing_still_compares_and_stamps(self):
        # If the committed metadata was already current, nothing was copied, so
        # the comparison is between an independent regeneration and bytes this
        # run did not write - it is real, and worth stamping.
        idb = self.root / "test.idb"
        idb.write_bytes(b"idb-bytes")
        verify_dir = self.root / "verify"
        cache_dir = self.root / "cache"
        ...
        self.assertEqual(len(list(cache_dir.glob("*.stamp"))), 1)
