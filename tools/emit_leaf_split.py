...
def emit(lines, items, its, tests, slices, sid, tail_si, promoted, ppstack):
    ...
    # ---- manifest -------------------------------------------------------
    rows = '\n'.join('LEAF_FAMILY_COUNT(%s, %d)' % (n, c) for n, c in counts)
    open(os.path.join(OUT, 'leaf_case_manifest.h'), 'w').write(MANIFEST_H % rows)
...
MANIFEST_H = '''// How many cases each family file is expected to register.
//
