...
// What `Win::window_proc` routes to. It is promoted into src/win.cpp now,
// so these five are what its recovery still stands on.
//
...
int Win::get_key_window() {
    typedef int(__stdcall *pending)();
    return PENDING_BODY(0x005F6A50, pending)();
}

// 0x005F6F10  ?get_mouse_window@Win@@QAAHPAH0@Z
int Win::get_mouse_window(int *x, int *y) {
...
// set, so the forwarder has to hand the receiver over explicitly.
