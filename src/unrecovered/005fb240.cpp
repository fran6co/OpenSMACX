// ORIGINAL: 0x005FB240 FILE
// RULED-OUT: for-loop over entries_[] matching id, then a dead index==-1 guard
//            mirrored from the disassembly before the pull_down->UNK3 call
// working copy - scaffold materialised by --work
// name      ?UNK5@Menu@@QAEHHHHH@Z
// size      94 bytes
// spans     0x005FB240-0x005FB29E
...
// subject: ?UNK5@Menu@@QAEHHHHH@Z  at 0x005FB240  (94 bytes)
//
...
class Menu : public GraphicWin { public:
    ...
    int UNK5(int, int, int, int);
};
int Menu::UNK5(int a1, int a2, int a3, int a4) {
    int index = 0;
    for (;;) {
        if (entries_[index].id == -1) {
            return 0xb;
        }
        if (entries_[index].id == a1) {
            break;
        }
        index++;
        if (index > 0xe) {
    ...
    return entries_[index].pull_down->UNK3(a2, a3, a4);
}
