...
// subject: ?draw_tile_fixup@@YAXHHHH@Z  at 0x0046AF90  (161 bytes)
//
...
extern MapWin *g_tileTable[];

void __cdecl draw_tile_fixup(int a1, int a2, int a3, int a4) {
    if (*reinterpret_cast<unsigned char *>(g_009a6494) & 1) {
        int savedA4 = *g_00939284;
        MapWin **entry = g_tileTable;
        do {
            MapWin *w = *entry;
            if (w != 0 && (entry == g_tileTable || *reinterpret_cast<int *>(reinterpret_cast<char *>(w) + 0x1dd74) != 0)) {
                w->draw_radius(a1, a2, (a3 != 0) + 1, savedA4);
            }
            entry++;
        } while (reinterpret_cast<int>(entry) < 0x7d3c5c);
    } else {
        MapWin **entry = g_tileTable;
        do {
            MapWin *w = *entry;
            if (w != 0 && (entry == g_tileTable || *reinterpret_cast<int *>(reinterpret_cast<char *>(w) + 0x1dd74) != 0)) {
                w->draw_radius(a1, a2, 0, a4);
            }
            entry++;
        } while (reinterpret_cast<int>(entry) < 0x7d3c5c);
    }
