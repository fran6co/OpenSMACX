...
// subject: ??1RadioButton@@QAE@H@Z  at 0x00406F60  (130 bytes)
//
...
RadioButton::~RadioButton() {
    // The catalogue's H parameter is the compiler-synthesised "destroy
    // virtual bases" flag that a class with virtual bases picks up
    // automatically - C++ forbids writing it explicitly on a destructor,
    // and this scaffold's RadioButton has no declared base, so that flag
    // (and the final conditional `operator delete`) is not reachable from
    // here. Everything else below follows the disassembly directly.
    char *const edi = reinterpret_cast<char *>(this) - 0x18;
    char *const esi = edi + 0x18;

    int *vbtable = *reinterpret_cast<int **>(edi);
    int disp = vbtable[1];
    *reinterpret_cast<int *>(disp + esi - 0x18) = 0x669a6c;

    vbtable = *reinterpret_cast<int **>(edi);
    disp = vbtable[1];
    *reinterpret_cast<int *>(disp + esi + 0x42c) = 0x669a64;

    vbtable = *reinterpret_cast<int **>(edi);
    disp = vbtable[2];
    *reinterpret_cast<int *>(disp + esi - 0x18) = 0x669a58;

    vbtable = *reinterpret_cast<int **>(edi);
    disp = vbtable[1];
    *reinterpret_cast<int *>(disp + esi - 0x1c) = disp - 0x18;

    vbtable = *reinterpret_cast<int **>(edi);
    disp = vbtable[2];
    *reinterpret_cast<int *>(disp + esi - 0x1c) = disp - 0xa30;

    reinterpret_cast<RadioButton *>(edi)->close();
    reinterpret_cast<Dialog *>(edi + 0xa30)->~Dialog();
