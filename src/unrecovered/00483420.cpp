...
// TRIED: `a2 * 0x19c` as a raw byte offset and as `NetPlayerRec[a2].id` struct-array indexing both compiled the multiply as a single `imul`, not the original's shl/add/lea strength reduction (0.74 mnemonic sim, first divergence at #3) - not chased further.
...
// calls     0x00629710 0x00631A20 0x00645470
// indirect  0x0048346D

// GENERATED SKELETON - tools/emit_translation_unit.py
// subject: ?new_chat@NetWin@@QAEXPADH@Z  at 0x00483420  (165 bytes)
//
...
// g_0093d51c indexes a 0x19c-byte record by a2 (`[ecx*4+ADDR]` after a
// strength-reduced `a2*103`) - a plain load/store pointer folds the address
// away for that indexing, so it is declared as a struct array instead.
struct NetPlayerRec00483420 {
    unsigned long id;
    unsigned char pad[0x19c - 4];
};
extern NetPlayerRec00483420 g_0093d51c[];

class NetWin { public:
    void new_chat(char *, int);
};
void NetWin::new_chat(char * a1, int a2) {
    char *self = reinterpret_cast<char *>(this);
    char buf[0x100];
    buf[0] = 0;
    unsigned long playerId = g_0093d51c[a2].id;
    Net *net = reinterpret_cast<Net *>(g_0093cd90);
    char *name = net->get_player_name(playerId);
    strcat(buf, name);
    typedef char *(__stdcall *CharUpperFn)(char *);
    (*reinterpret_cast<CharUpperFn *>(g_0066931c))(buf);
    strcat(buf, reinterpret_cast<char *>(g_00682e94));
    strcat(buf, reinterpret_cast<char *>(g_00682820));
    strcat(buf, a1);
    StringBox *box = reinterpret_cast<StringBox *>(self + 0xd40);
    box->add(buf, a2, 1);
}
