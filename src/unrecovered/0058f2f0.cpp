// ORIGINAL: 0x0058F2F0
// RULED-OUT: original establishes a real EBP frame (flags: frame); this
//            body compiles FPO (esp-relative) instead - same mnemonics
//            through the first branch, diverging only in WHEN the `add
//            esp,4` stack cleanup after `log_set_state` happens (original
//            defers it to the shared join point at 0x58f3d9, this body
//            does it immediately before the `jmp`). Landed at MISMATCH #18.
// name      ?game_init@@YAHHH@Z
// size      305 bytes
// spans     0x0058F2F0-0x0058F421
