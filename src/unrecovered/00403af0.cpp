...
// calls     0x005D8000 0x005E2DD3

// GENERATED SKELETON - tools/emit_translation_unit.py
// subject: ?fill_func@AlphaMenu@@QAAHPAUGraphicWin@@HPAURECT@@@Z  at 0x00403AF0  (128 bytes)
//
...
class AlphaMenu { public:
    int __cdecl fill_func(GraphicWin *, int, RECT *);
};
struct BoxTableEntry { unsigned char data[0x28]; };
extern BoxTableEntry g_box_table[];

int __cdecl AlphaMenu::fill_func(GraphicWin * a1, int a2, RECT * a3) {
    char *base = reinterpret_cast<char *>(a1);
    Buffer *buf = reinterpret_cast<Buffer *>(base + 0x444);

    RECT localRect = *a3;
    localRect.left = 1;
    localRect.right = *reinterpret_cast<int *>(base + 0x47C) - 1;
    buf->set_clip(&localRect);

    RECT *rect2 = reinterpret_cast<RECT *>(base + 0x474);
    BoxSpriteParams *params = reinterpret_cast<BoxSpriteParams *>(
        &g_box_table[(a2 != 0) ? 2 : 0]);
    buf->box_sprite(rect2, params);
