...
// subject: ?bring_parent_to_top@Win@@QAAXPAVWin@@@Z  at 0x005F5140  (183 bytes)
//
...
void __cdecl Win::bring_parent_to_top(Win * a1) {
    if (a1 != 0 && (reinterpret_cast<unsigned int *>(a1)[0x26] & 0x2000000) == 0) {
        ...
        if (count > 0) {
            while (true) {
                if (g_009b6e48_arr[i] == a1) {
                    if (i < count) {
                        int j = i;
                        while (j > 0) {
                            g_009b6e48_arr[j] = g_009b6e48_arr[j - 1];
                            --j;
                        }
                        g_009b6e48_arr[0] = a1;
                    }
