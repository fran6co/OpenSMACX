...
// subject: ?init_dummy@MapWin@@QAEXXZ  at 0x00470ED0  (152 bytes)
//
...
void MapWin::init_dummy() {
    ...
    owned_ = mem_get((*g_00949884 + *g_00949884 * 2) << 2);
    count = *g_00949884;
    if (count > 0) {
        offset = 0;
        i = 0;
        do {
            for (j = 1; j <= 4; j++) {
                base = (unsigned char *)owned_;
                base[offset + j - 1] = 0xFF;
            }
            for (j = 1; j <= 5; j++) {
                base = (unsigned char *)owned_;
                base[offset + j + 3] = 0xFF;
            }
            base = (unsigned char *)owned_;
            base[offset + 9] = 0;
            base = (unsigned char *)owned_;
            base[offset + 9] |= (*(unsigned char *)*g_0094a30c) & 0xD0;
            offset += 0xC;
            i++;
        } while (i < *g_00949884);
    }
