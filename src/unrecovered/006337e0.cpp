// ORIGINAL: 0x006337E0 FILE
// RULED-OUT: field mapping confirmed against Ghidra (param_1[0x28a]/
//            [0x289] deref'd at +0x18/+0x1c, isToggled_ at 0xA18); the
//            9-arg GraphicWin::init call and the a3==0 guard match.
//            Divergence is push/mov ordering in the argument
//            marshalling (mnemonic_similarity 0.97, one insn swapped
//            at #19) - a scheduling artifact, not a semantic gap.
// working copy - scaffold materialised by --work
// name      ?init@CheckButton@@QAEHHHPAUGraphicWin@@H@Z
// size      97 bytes
// spans     0x006337E0-0x00633841
...
// subject: ?init@CheckButton@@QAEHHHPAUGraphicWin@@H@Z  at 0x006337E0  (97 bytes)
//
...
// GraphicWin::init and VCall's slot001 both feed/take values the
// catalogue's void/nullary attribution doesn't give them (a checked
// return, and an int argument), so they are shadowed here under their
// own names rather than edited in place.
class GraphicWinInitShadow { public:
    int init(int, int, int, int, char *, int, Win *, Menu *, BorderSizing *);
};

class VCallShadow { public:
    virtual void slot000();
    virtual void slot001(int);
};

int CheckButton::init(int a1, int a2, GraphicWin * a3, int a4) {
    if (a3 == 0) {
        return 3;
    }
    int deref28 = *reinterpret_cast<int *>(field_A28_ + 0x18);
    int deref24 = *reinterpret_cast<int *>(field_A24_ + 0x1c);
    int result = reinterpret_cast<GraphicWinInitShadow *>(this)->init(
        a1, a2, deref28, deref24,
        0, 0x1020,
        reinterpret_cast<Win *>(a3),
        0, 0);
    if (result == 0) {
        isToggled_ = a4;
        reinterpret_cast<VCallShadow *>(this)->slot001(0);
        result = 0;
    }
    return result;
}
