// ORIGINAL: 0x00631A60 FILE
// RULED-OUT: single-level reinterpret_cast<VCall*>(self+0x58)->slotN() for the
//        embedded subobject at +0x58, recomputed at each call site instead
//        of cached in a local (0.73 -> 0.96 mnemonic similarity); slots
//        34/35/37 needed non-void signatures (int(), void(int),
//        unsigned int()) since the shared VCall shim declares every slot
//        void(); the outer `if (flags & mask)` condition polarity is
//        INVERTED versus the ghidra hypothesis (`!= 0` first, `== 0` in the
//        else) to match which branch the original falls into vs jumps to;
//        the two call sites (sub_62f390/send_packet_type) each re-read
//        `*(int*)(packet+0x20) - offset` inline rather than hoisting a
//        shared `length` local, since the original recomputes `sub edx,eax`
//        once per branch.
// RULED-OUT: hoisting length/chan into locals shared by both branches
//            (dropped the ebx push/pop and one sub entirely).
// working copy - scaffold materialised by --work
// name      ?process_voice@Net@@QAEXXZ
// size      206 bytes
// spans     0x00631A60-0x00631B2E
