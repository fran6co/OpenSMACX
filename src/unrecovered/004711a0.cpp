...
// TRIED: hoisting `base = owned_+offset` into its own statement before the `j = j+1` increment, to force add-before-inc ordering - same MISMATCH #30 'add' vs 'inc' either way (/O2 schedules inc first regardless of source order). similarity 0.9667, rebuilt_bytes == original_bytes (155).
...
// subject: ?terrain_fixup@@YAXPAUMapWin@@@Z  at 0x004711A0  (155 bytes)
//
...
void __cdecl terrain_fixup(MapWin * a1) {
    int count2 = 0;
    if (a1 != 0) {
        if (a1->owned_ != 0) {
            free(a1->owned_);
        ...
        a1->owned_ = mem_get(*g_00949884 * 0xc);
        if (*g_00949884 > 0) {
            int offset = 0;
            do {
                int j = 0;
                do {
                    j = j + 1;
                    *(reinterpret_cast<unsigned char *>(a1->owned_) + offset - 1 + j) = 0xff;
                } while (j < 4);
                j = 0;
                do {
                    j = j + 1;
                    *(reinterpret_cast<unsigned char *>(a1->owned_) + offset + 3 + j) = 0xff;
                } while (j < 5);
                *(reinterpret_cast<unsigned char *>(a1->owned_) + 9 + offset) = 0;
                unsigned char *record = reinterpret_cast<unsigned char *>(a1->owned_) + 9 + offset;
                offset = offset + 0xc;
                *record = *record | (*reinterpret_cast<unsigned char *>(*g_0094a30c) & 0xd0);
                count2 = count2 + 1;
            } while (count2 < *g_00949884);
        }
    }
}
