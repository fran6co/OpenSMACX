...
// calls     0x004EC3B0 0x005D5A70 0x005F7E90 0x00644F3A
// indirect  0x0041A892

// GENERATED SKELETON - tools/emit_translation_unit.py
// subject: ?check_loc@BaseWin@@QAEXHHH@Z  at 0x0041A7E0  (199 bytes)
//
...
extern LocEntry g_097d040[];

void BaseWin::check_loc(int a1, int a2, int a3) {
    if (reinterpret_cast<Win *>(this)->is_visible()) {
        char *self = reinterpret_cast<char *>(this);
        int idx = *reinterpret_cast<int *>(self + 0x40b0c);
        LocEntry *entry = &g_097d040[idx];
        int y = entry->y;
        int x = entry->x;
        int dx = abs(a1 - x);
        if (!(*g_0094988c & 1) && *g_0068faf0 < dx) {
            dx = *g_00949870 - dx;
        }
        int dy = abs(a2 - y);
        int adx = abs(dx);
        ...
        int result = mx - (((((adx + ady) >> 1) - mn) + 1) >> 1);
        if (result <= a3) {
            base_compute(1);
