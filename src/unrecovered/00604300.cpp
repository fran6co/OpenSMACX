// ORIGINAL: 0x00604300 FILE
// RULED-OUT: goto-label mirror of the Ghidra shape; collapsed to an
//            if/else-if/else that sets field_3100_ in both terminal arms
//            instead. Dynamic vtable-relative `this` for get_selected_id
//            (dialogs->vbtable_pointer_ / field_58_ + 8) reaches #18
//            lea-vs-add, deep past the branch structure
// working copy - scaffold materialised by --work
// name      ?on_button_clicked@BasePop@@QAEXH@Z
// size      202 bytes
// spans     0x00604300-0x006043CA
// prototype void (__thiscall ?on_button_clicked@BasePop@@QAEXH@Z)(BasePop* this, int)
// callers   0   call targets   2
// kind      game
// flags     hidden;sp_ready;purged_ok
// calls     0x00609A50 0x00613740
// indirect  0x006043A1 0x006043AD 0x006043C2

// GENERATED SKELETON - tools/emit_translation_unit.py
// subject: ?on_button_clicked@BasePop@@QAEXH@Z  at 0x00604300  (202 bytes)
//
...
void BasePop::on_button_clicked(int a1) {
    char *self = reinterpret_cast<char *>(this);
    field_A40_ = 0;
    Dialogs *dialogs = reinterpret_cast<Dialogs *>(self + 0x21d0);
    if (dialogs->get_num_items() != 0) {
        if (dialogs->kind_ == 1) {
            int off = *reinterpret_cast<int *>(dialogs->field_58_ + 8);
            field_A40_ = *reinterpret_cast<int *>(self + 0x2314 + off);
        } else {
            int adj = *reinterpret_cast<int *>(dialogs->vbtable_pointer_ + 8);
            Dialog *dlg = reinterpret_cast<Dialog *>(reinterpret_cast<char *>(dialogs) + adj);
            field_A40_ = dlg->get_selected_id();
        }
    }
    if (a1 == -2) {
        if ((field_30A8_ & 0x40) != 0) {
            field_A40_ = -1;
        }
        field_3100_ = 0;
    } else if (a1 != -1) {
    ...
    if ((field_30A8_ & 0x100) == 0) {
        if (field_A14_ == 0) {
            reinterpret_cast<VCall *>(this)->slot058();
        } else {
            reinterpret_cast<VCall *>(field_A14_)->slot058();
        }
    }
    if ((field_30A8_ & 0x4000) != 0) {
