// ORIGINAL: 0x005ED880 FILE
// RULED-OUT: three raw vtable calls (offsets 0xc/0x30/0x108, i.e. slots
//            3/12/66) - `__thiscall` function-pointer typedefs are rejected
//            by VC6 (C4234), so each slot needs its own dummy-padded
//            virtual class reinterpret_cast, one class per distinct slot
//            index and signature (0xc takes 3 ints; 0x30 and 0x108 are
//            nullary). That forces FILE mode with its own minimal Win: the
//            scaffold's Win/VCall are already fully defined, and these
//            extra classes' own braces would end body-mode's brace-counted
//            extraction before reaching the function. Landed at MISMATCH
//            #0 (best-scoring flag set is `/Oy-`, framed; a frameless
//            manual build tracks the first ~15 instructions exactly).
// name      ?resize@Win@@QAEHHHH@Z
// size      309 bytes
// spans     0x005ED880-0x005ED9B5
