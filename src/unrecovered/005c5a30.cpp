...
// calls     0x0046FD90 0x004C45E0 0x0051E760 0x00523DD0 0x0059C790 0x005C38B0 0x005C3F70 0x005C4170 0x005C4470 0x005C55C0 0x006257E0 0x006262F0
// indirect  0x005C5A7B

// GENERATED SKELETON - tools/emit_translation_unit.py
// subject: ?world_climate@@YAXXZ  at 0x005C5A30  (171 bytes)
//
...
static int *const g_00691e2c = (int *)0x00691E2C;
static int *const g_00691e4c = (int *)0x00691E4C;
...
void __cdecl world_climate() {
    do_checksums(2);
    log_say(reinterpret_cast<char *>(g_00691e2c), 0, 0, 0);
    world_shorelines();
    world_temperature();
    world_rivers();
    world_rainfall();
    ...
    random_reseed(get_time());

    if (*g_0068f21c == 0) {
        int *p = g_007d3c3c;
        do {
            MapWin *win = reinterpret_cast<MapWin *>(*p);
            if (*reinterpret_cast<int *>(reinterpret_cast<char *>(win) + 0x1dd74) != 0) {
                win->clear_terrain();
            }
            ++p;
        } while (reinterpret_cast<int>(p) < reinterpret_cast<int>(g_007d3c5c));
        reinterpret_cast<WorldWin *>(g_008e9f60)->clear_terrain();
    }

    log_say(reinterpret_cast<char *>(g_00691e4c), 0, 0, 0);
    do_checksums(3);
}
