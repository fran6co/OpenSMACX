// ORIGINAL: 0x00638DE0 sub_638de0 0x00638DE0-0x00638F2A
// RULED-OUT: calling sub_638620/sub_638650/sub_639390 by name - all three are catalogued unmangled with no CRT_SIGNATURES entry, so declfix leaves them nullary (C2660 direct, C2733 if redeclared); routed through function-pointer casts. The fallback branch's indirect call goes through `*(int*)0x9c0b90`, a stored function pointer value (not a vtable), called the same way. Sharing ONE `sub_638650` call site between the 0x4040302/0x4040303 cases via goto (matching the disassembly's single shared call at 0x638e99) was not attempted - the two branches' locals would cross the jump into each other's scope, which C++ refuses; each case is expanded with its own call instead. Best reached: MISMATCH, edit_count 17, 315 bytes vs 330, mnemonic similarity ~0.67.
// size      330 bytes
// prototype
