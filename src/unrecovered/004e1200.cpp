...
// subject: ?editor_clear_random@Console@@QAEXXZ  at 0x004E1200  (139 bytes)
//
...
void Console::editor_clear_random() {
    auto_undo();
    if (*g_00949878 == 0) {
        *g_00949878 = random(0, 0x7fff) + 1;
        draw_map(1);
        return;
    }
    *g_00949878 = 0;
    if (*g_00949884 > 0) {
        int base = *g_0094a30c;
        char *ptr = (char *)base + 8;
        int i = 0;
        do {
            uint32_t v = *(uint32_t *)ptr;
            if ((v & 0x400) != 0 && (v & 0x20000000) == 0) {
                v &= 0xdffffbff;
                *(uint32_t *)ptr = v;
                uint32_t *dst = (uint32_t *)(ptr + 8);
                int count = 7;
                do {
                    *dst = v;
                    dst++;
