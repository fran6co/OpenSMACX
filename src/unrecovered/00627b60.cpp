...
// subject: ?pop_ask_number@@YAHPADP6AHXZ@Z  at 0x00627B60  (200 bytes)
//
...
int __cdecl pop_ask_number(char * a1, int (__cdecl *a2)()) {
    char buf[80];
    ...
    *reinterpret_cast<char *>(g_009b86a0) = 0;
    reinterpret_cast<ItoaFn>(_itoa)(0, buf, 10);
    reinterpret_cast<StrcatFn>(_strcat)(reinterpret_cast<char *>(g_009b86a0), buf);
    int started = win->start(reinterpret_cast<char *>(g_009b8aa8),
                              reinterpret_cast<const char *>(a2), 9,
                              reinterpret_cast<char *>(g_009b86a0), 0x44, 0);
    if (started != 0) {
        return -1;
    }
    win->write_check(*g_009bc06c);
    int ret = win->exec(0, a2);
    if (win->field_A14_ == 0) {
        do_all_draws();
    }
