...
// calls     0x00401100 0x0060A020 0x0060C920 0x0060C9B0 0x0062B870

// GENERATED SKELETON - tools/emit_translation_unit.py
// subject: ?add_base@MessageWin@@QAEXHPADPAUSprite@@@Z  at 0x004713B0  (177 bytes)
//
...
// indexed table bases - address does work, not plain loads, so these are
// declared as arrays (own names, distinct from the plain g_0097d044/g_0093a7b0
// pointers above) rather than reused as fixed pointers.
extern unsigned char g_0097d044_tab[];
extern char g_0093a7b0_recs[];

void MessageWin::add_base(int a1, char * a2, Sprite * a3) {
    if (*g_0093a95c != 0) {
        int idx = g_0097d044_tab[a1 * 0x134];
        if (idx != *g_00939284) {
            char *rec = g_0093a7b0_recs + idx * 0x30;
            *reinterpret_cast<int *>(rec + 0x1C) = reinterpret_cast<int>(a2);
            *reinterpret_cast<int *>(rec + 0x20) = 0;
            *reinterpret_cast<int *>(rec + 0x24) = 0;
            reinterpret_cast<StringStruct *>(rec)->add(a1 | 0x40000000);
            return;
        }
    }
    if (*g_0093f660 != 0) {
    ...
    ++*g_009393d0;
    listBox_.item(a2, a1 | 0x40000000);
    listBox_.set_selected_pos(-1, 1);
    listBox_.update_changes();
    reinterpret_cast<ButtonGroup *>(g_007cd098)->set(1000, 1);
}
