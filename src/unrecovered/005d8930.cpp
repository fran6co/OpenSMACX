...
// RULED-OUT: same shape as sub_5d8740 (0x005D8740) - opaque Buffer* self,
//   field_58_/ppv_bits_/field_50_/field_6C_/field_4A8_/width_/height_ read
//   through the named Buffer members instead of raw offsets, and the
//   stdcall surface-Unlock reached via a `void(__stdcall*)(void*,void*)`
//   function pointer (a C++ virtual call would put `this` in ecx, but the
//   original pushes it on the stack like a COM method). `a1` (declared
//   Buffer* by the mangled name) is compared to -1 and stored as a byte, so
//   it's cast through `(int)`, not dereferenced.
// RULED-OUT: nothing past that - 94.9% mnemonic similarity, first gap at
//   #41 (mov vs cmp) in the NULL-surface lock-count arm, same spot as
//   0x005D8740's closest reach (see that file's note for what was tried).
...
// subject: ?draw_mono@Buffer@@QAEHPAVBuffer@@HHHHHH@Z  at 0x005D8930  (248 bytes)
//
...
int Buffer::draw_mono(Buffer * a1, int a2, int a3, int a4, int a5, int a6, int a7) {
    int transparentIndex = (int)a1;
    if (field_58_ == 0) {
        if (transparentIndex == -1) {
            transparentIndex = *reinterpret_cast<uint8_t *>(g_00696d14);
        }

        int bits = (int)ppv_bits_;
        field_50_ = bits;
        if (bits != 0) {
            ++field_6C_;
        }
        ...
        *g_009b3a60 = width_;
        *g_009b3a64 = -static_cast<int>(height_);
        *g_009b3a68 = width_;
        *g_009b3a6c = -static_cast<int>(height_);
        *g_009b3a70 = 0;
        *g_009b3a74 = 0;
        *reinterpret_cast<char *>(g_009b3a58) = static_cast<char>(transparentIndex);
        *g_009b3a78 = 0;

        void *surface = reinterpret_cast<void *>(field_58_);
        if (surface == 0) {
            int count = static_cast<int>(field_6C_) - 1;
            field_6C_ = count;
            if (count > 0) {
                goto forward;
            }
            ...
            int count = static_cast<int>(field_6C_) - 1;
            field_6C_ = count;
            int cachedBits = field_50_;
            if (cachedBits == 0 || count > 0) {
                goto forward;
            }
            typedef void(__stdcall * UnlockFn)(void *, void *);
            UnlockFn unlock = *reinterpret_cast<UnlockFn *>(
                *reinterpret_cast<int *>(surface) + 0x80);
            unlock(surface, reinterpret_cast<void *>(cachedBits));
        }
        field_50_ = 0;
        field_6C_ = 0;
    }
...
forward:
    {
        int result = reinterpret_cast<Sprite *>(g_009b3a50)
                         ->draw_mono(a1, a2, a3, a4, a5, a6, a7);
        *g_009b3a54 = 0;
        return result;
    }
}
