...
// TRIED: reproducing MSVC's own vtordisp adjustor thunk mechanism; wrote `raw - *(int*)(raw-4)` as explicit pointer arithmetic in a shim member instead - first instruction diverges (sub vs push)
...
// subject: sub_6117d0  at 0x006117D0  (70 bytes)
//
...
class ScalarDeletingDtorTarget { public:
    void *scalarDtor(unsigned int flags);
};

void *ScalarDeletingDtorTarget::scalarDtor(unsigned int flags) {
    char *raw = reinterpret_cast<char *>(this);
    char *adjusted = raw - *reinterpret_cast<int *>(raw - 4);
    char *mostDerived = adjusted - 0x8C;

    reinterpret_cast<SpriteBox *>(adjusted)->~SpriteBox();
    reinterpret_cast<Dialog *>(mostDerived + 0xAA4)->~Dialog();
    reinterpret_cast<GraphicWin *>(adjusted)->~GraphicWin();

    if (flags & 1) {
        operator delete(mostDerived);
    }
    return mostDerived;
}
