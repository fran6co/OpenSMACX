...
// subject: ?nonclient_to_client@Win@@QAEXPAH0@Z  at 0x005EEF60  (240 bytes)
//
...
// This body dispatches through slot(s): 91
class VCall { public:
...
//            compiler - each carries its own full copy of the
//            border-adjustment code, reloading `border_thickness_`
//            separately for `*a1` and `*a2` rather than sharing one
//            negated/doubled value (sharing one local collapsed the two
//            branches' object code, costing 4 instructions and moving the
//            divergence much earlier - reloading per-target fixed that).
//            What remains: after storing the *a2 border adjustment,
//            original reuses `edx` directly for the "+ *a2" term in the
//            bottom_border_thickness_ fixup and loads bottom_border_thickness_
//            into a fresh `eax`; every source spelling tried here
//            (`(x) + *a2`, `*a2 + (x)`, `*a2 += x`) instead has the
//            compiler copy the live *a2 value out of edx into eax first
//            and load bottom_border_thickness_ into edx - same values,
//            swapped registers, 8 bytes off from there on.
// The `NCCall` shim below is declared HERE as well as in
// src/unrecovered/005ef110.cpp: each file is its own translation unit,
// so a helper introduced while scoring does not travel with the body
// unless it is landed with it. Only declaration ORDER pins the vtable
// offset, so the 92-slot shim reproduces slot091 with an int return
// where the scaffolding's own VCall types every slot void/nullary.
class NCCall { public:
    virtual void slot000();
    virtual void slot001();
    virtual void slot002();
