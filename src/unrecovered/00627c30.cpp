// ORIGINAL: 0x00627C30 BYTE_EXACT FILE
// working copy - scaffold materialised by --work
// name      ?pop_ask_number@@YAHPADPADHP6AHXZ@Z
// size      204 bytes
// spans     0x00627C30-0x00627CFC
// prototype int (__cdecl ?pop_ask_number@@YAHPADPADHP6AHXZ@Z)(int8* srcFileID, int8* sectionID, int, int (__cdecl *)())
...
// subject: ?pop_ask_number@@YAHPADPADHP6AHXZ@Z  at 0x00627C30  (204 bytes)
//
...
int __cdecl pop_ask_number(char * a1, char * a2, int a3, int (__cdecl *a4)()) {
    Win *w = reinterpret_cast<Win *>(*g_009bc074);
    int visible = w->is_visible();
    BasePop *pop = reinterpret_cast<BasePop *>(*g_009bc078);
    if (!visible) {
        pop = reinterpret_cast<BasePop *>(*g_009bc074);
    }

    char buffer[80];
    *reinterpret_cast<char *>(g_009b86a0) = 0;
    (reinterpret_cast<ItoaFn>(_itoa))(a3, buffer, 10);
    strcat(reinterpret_cast<char *>(g_009b86a0), buffer);

    if (pop->start(a1, a2, 9, reinterpret_cast<char *>(g_009b86a0), 0x44, 0) != 0) {
        return -1;
    }

    pop->write_check(*g_009bc06c);
    int result = pop->exec(0, a4);

    if (pop->field_A14_ == 0) {
        do_all_draws();
    }
    *g_009bc070 = pop->field_3100_;
