...
// subject: ?cursor_on@Console@@QAEXHH@Z  at 0x00510490  (141 bytes)
//
...
void Console::cursor_on(int a1, int a2) {
    field_23D14_ = static_cast<uint32_t>(a2);
    field_23D10_ = static_cast<uint32_t>(a1);
    for (int *p = g_007d3c3c; p != g_007d3c5c; ++p) {
        int esiVal = *p;
        if (esiVal != 0) {
            char *obj = reinterpret_cast<char *>(esiVal);
            if (p == g_007d3c3c || *reinterpret_cast<int *>(obj + 0x1dd74) != 0) {
                char *target = reinterpret_cast<char *>(g_0093aa70) + a1 * 0x2c;
                if (*reinterpret_cast<char **>(obj + 0x1ef4c) != target) {
                    int *vt = *reinterpret_cast<int **>(obj);
                    int off = *reinterpret_cast<int *>(reinterpret_cast<char *>(vt) + 4);
                    Win *winObj = reinterpret_cast<Win *>(obj + off);
                    winObj->set_cursor(reinterpret_cast<Sprite *>(target),
                                        g_0093aa10[a1], g_0093aa40[a1]);
                    *reinterpret_cast<char **>(obj + 0x1ef4c) = target;
                }
            }
        }
    }
