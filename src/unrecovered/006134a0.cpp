// ORIGINAL: 0x006134A0 FILE
// working copy - scaffold materialised by --work
// name      ?init@Dialogs@@QAEHH@Z
// size      132 bytes
// spans     0x006134A0-0x00613524
// prototype int (__thiscall ?init@Dialogs@@QAEHH@Z)(Dialogs* this, int)
// callers   1   call targets   6
// kind      game
// flags     hidden;sp_ready;purged_ok
// calls     0x006095F0 0x00609F20 0x0060D1B0 0x0060E7C0 0x00610420 0x00611A90

// GENERATED SKELETON - tools/emit_translation_unit.py
// subject: ?init@Dialogs@@QAEHH@Z  at 0x006134A0  (132 bytes)
//
...
int Dialogs::init(int a1) {
    char *self = reinterpret_cast<char *>(this);

    RadioButton *radio = reinterpret_cast<RadioButton *>(self + 0x44);
    radio->close();
    int *rvtbl = *reinterpret_cast<int **>(radio);
    Dialog *rdlg = reinterpret_cast<Dialog *>(reinterpret_cast<char *>(radio) + rvtbl[2]);
    int result = rdlg->init(a1);
    if (result == 0) {
        ListBox *listbox = reinterpret_cast<ListBox *>(self);
        listbox->close();
        int *lvtbl = *reinterpret_cast<int **>(listbox);
        Dialog *ldlg = reinterpret_cast<Dialog *>(reinterpret_cast<char *>(listbox) + lvtbl[2]);
        result = ldlg->init(a1);
        if (result == 0) {
            CheckBox *check = reinterpret_cast<CheckBox *>(self + 0x58);
            check->close();
            int *cvtbl = *reinterpret_cast<int **>(check);
            Dialog *cdlg = reinterpret_cast<Dialog *>(reinterpret_cast<char *>(check) + cvtbl[2]);
            result = cdlg->init(a1);
            if (result == 0) {
                SpriteBox *sbox = reinterpret_cast<SpriteBox *>(self + 0x70);
                result = sbox->init(a1);
                if (result == 0) {
                    EditGroup *eg = reinterpret_cast<EditGroup *>(self + 0xf8);
                    eg->close();
                    int *evtbl = *reinterpret_cast<int **>(eg);
                    Dialog *edlg = reinterpret_cast<Dialog *>(reinterpret_cast<char *>(eg) + evtbl[2]);
                    result = edlg->init(a1);
                }
            }
        }
