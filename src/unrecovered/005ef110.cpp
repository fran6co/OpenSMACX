...
// subject: ?nonclient_to_client@Win@@QAEXPAURECT@@@Z  at 0x005EF110  (193 bytes)
//
...
// LEVER: `class VCall` slot091 supplied by scaffolding is void/nullary; the
//        call site needs its int return added into a1->top, so this body
//        declares its OWN 92-slot shim (`NCCall`, matching the landed
//        `BeginVCall` pattern in src/recovered/00447920.cpp) with only
//        slot091 retyped `int slot091();`. Only declaration ORDER pins the
//        vtable offset, so a same-name-avoiding second shim class is safe
//        to add alongside the one scaffolding already declared.
//        Second lever: original computes the border adjustment by negating
//        border_thickness_ ONCE (`neg eax`) and reusing it with `sub` for
//        the near edges and `add` for the far edges (`left -= -border;
//        right += -border;` and the same pair for top/bottom) rather than
//        adding/subtracting the positive value directly at each site -
//        writing it as four independent `+border`/`-border` expressions
//        compiles to four ADD/SUB with no shared negation and mismatches.
class NCCall { public:
    virtual void slot000();
    virtual void slot001();
    virtual void slot002();
