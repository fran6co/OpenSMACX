...
// TRIED: `flags & 0x400` compiles to a single `test [mem], imm`; original keeps the mask 0x400 in a register and does `test eax,edx`. Named local for the flags read didn't change it.
...
// subject: ?set_external_window@BasePop@@QAEXPAUGraphicWin@@HHHH@Z  at 0x00605010  (215 bytes)
//
...
class BasePop : public GraphicWin { public:
    ...
    void set_external_window(GraphicWin *, int, int, int, int);
};
void BasePop::set_external_window(GraphicWin * a1, int a2, int a3, int a4, int a5) {
    char *self = reinterpret_cast<char *>(this);
    *reinterpret_cast<GraphicWin **>(self + 0xA14) = a1;
    *reinterpret_cast<int *>(self + 0xA18) = a2;
    *reinterpret_cast<int *>(self + 0xA1C) = a3;
    *reinterpret_cast<int *>(self + 0xA20) = a4;
    *reinterpret_cast<int *>(self + 0xA24) = a5;
    if (a1 != 0) {
        int base = *reinterpret_cast<int *>(*reinterpret_cast<int *>(self + 0x21D0) + 8);
        *reinterpret_cast<int *>(self + base + 0x21FC) = a4;
        unsigned int flags = *reinterpret_cast<unsigned int *>(self + 0x30A8);
        if (*reinterpret_cast<int *>(self + 0xA14) == 0
            && (flags & 0x400) == 0
            && *g_009b7b1c > 0x3ff) {
            int val = (a5 * 3) / 2;
            int off = *reinterpret_cast<int *>(*reinterpret_cast<int *>(self + 0x21D0) + 8);
            *reinterpret_cast<int *>(self + off + 0x2200) = val;
        } else {
            int off = *reinterpret_cast<int *>(*reinterpret_cast<int *>(self + 0x21D0) + 8);
            *reinterpret_cast<int *>(self + off + 0x2200) = a5;
        }
        if (a2 != 0x2000) {
            *reinterpret_cast<int *>(self + 0x30F8) = a2;
        }
        if (a3 != 0x2000) {
            *reinterpret_cast<int *>(self + 0x30FC) = a3;
        }
        int off2 = *reinterpret_cast<int *>(*reinterpret_cast<int *>(self + 0x21D0) + 8);
        *reinterpret_cast<unsigned int *>(self + off2 + 0x21F0) =
            *reinterpret_cast<unsigned int *>(self + off2 + 0x21F0) | 3;
    }
}
