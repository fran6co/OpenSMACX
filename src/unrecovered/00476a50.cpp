...
// TRIED: literal 0 store to g_007fd634 before the loop (forces a 10-byte mov-immediate instead of a 6-byte mov-register); computing the vtable-slot cast after the *g_007fa9a4 store instead of before (reorders `mov edx,[0x7f9f58]` late). Fixed both -> MNEMONIC_ONLY. Remaining: compiler puts the vtable pointer in eax not edx, and folds the (0,0,0) call args to immediate pushes instead of reusing the dead return-value register; not reproduced.
...
// calls     0x00472230 0x005D4E40 0x005FCBB0
// indirect  0x00476B0A

// GENERATED SKELETON - tools/emit_translation_unit.py
// subject: ?monument@@YAXH@Z  at 0x00476A50  (210 bytes)
//
...
static int *const g_007f9f58 = (int *)0x007F9F58;
static int *const g_007fa9a4 = (int *)0x007FA9A4;
...
union VCallShim3 {
    void (VCallObj::*memberFn)(int, int, int);
    void *raw;
};

extern unsigned char g_946f58_arr[];
extern int g_94ca18_arr[];

void __cdecl monument(int a1) {
    int idx = *g_00939284;
    if (((g_946f58_arr[idx * 0x59c] & 0x80) == 0) && (*g_0093f660 == 0) &&
        (a1 == 0 ||
         (((*g_009a6494 & 0x1000000) == 0) &&
          ((*(unsigned char *)g_009a64c0 & 0x10) == 0) &&
          (((*(unsigned char *)g_009a64c0 & 1) == 0) || ((*(unsigned char *)g_009a64c0 & 8) == 0))))) {
        int count = 0;
        *g_007fd634 = count;
        int *rec = g_94ca18_arr + idx * 0x13d;
        int i = 15;
        do {
            if (*rec != 0) {
                count = count + 1;
        ...
        *g_007fd634 = count;
        if (count != 0) {
            int rc = reinterpret_cast<MonuWin *>(g_007f9f58)->init();
            if (rc == 0) {
                int vtbl = *g_007f9f58;
                *g_007fa9a4 = (a1 != 0) ? 1 : 0;
                VCallShim3 shim;
                shim.raw = *reinterpret_cast<void **>(vtbl + 0xe4);
                (reinterpret_cast<VCallObj *>(g_007f9f58)->*shim.memberFn)(rc, rc, rc);
                reinterpret_cast<GraphicWin *>(g_007f9f58)->close();
                do_all_draws();
            }
        }
    }
