// ORIGINAL: 0x00613610 FILE
// working copy - scaffold materialised by --work
// name      ?init@Dialogs@@QAEHPAURECT@@PAUHeap@@@Z
// size      145 bytes
// spans     0x00613610-0x006136A1
// prototype int (__thiscall ?init@Dialogs@@QAEHPAURECT@@PAUHeap@@@Z)(Dialogs* this, RECT*, Heap*)
// callers   0   call targets   6
// kind      game
// flags     hidden;sp_ready;purged_ok
// calls     0x006097F0 0x00609F20 0x0060D1B0 0x0060E7C0 0x00610510 0x00611A90

// GENERATED SKELETON - tools/emit_translation_unit.py
// subject: ?init@Dialogs@@QAEHPAURECT@@PAUHeap@@@Z  at 0x00613610  (145 bytes)
//
...
int Dialogs::init(RECT * a1, Heap * a2) {
    char *self = reinterpret_cast<char *>(this);

    char *radio = self + 0x44;
    reinterpret_cast<RadioButton *>(radio)->close();
    int *vbt1 = *reinterpret_cast<int **>(radio);
    Dialog *dlg1 = reinterpret_cast<Dialog *>(radio + vbt1[2]);
    int result = dlg1->init(reinterpret_cast<Heap *>(a1), a2);
    if (result != 0) {
        return result;
    }

    reinterpret_cast<ListBox *>(self)->close();
    int *vbt2 = *reinterpret_cast<int **>(self);
    Dialog *dlg2 = reinterpret_cast<Dialog *>(self + vbt2[2]);
    result = dlg2->init(reinterpret_cast<Heap *>(a1), a2);
    if (result != 0) {
        return result;
    }

    char *check = self + 0x58;
    reinterpret_cast<CheckBox *>(check)->close();
    int *vbt3 = *reinterpret_cast<int **>(check);
    Dialog *dlg3 = reinterpret_cast<Dialog *>(check + vbt3[2]);
    result = dlg3->init(reinterpret_cast<Heap *>(a1), a2);
    if (result != 0) {
        return result;
    }

    result = reinterpret_cast<SpriteBox *>(self + 0x70)->init(reinterpret_cast<Heap *>(a1), a2);
    if (result != 0) {
        return result;
    }

    char *editgroup = self + 0xf8;
    reinterpret_cast<EditGroup *>(editgroup)->close();
    int *vbt4 = *reinterpret_cast<int **>(editgroup);
    Dialog *dlg4 = reinterpret_cast<Dialog *>(editgroup + vbt4[2]);
    return dlg4->init(reinterpret_cast<Heap *>(a1), a2);
}
