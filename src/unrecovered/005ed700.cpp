// ORIGINAL: 0x005ED700 BYTE_EXACT FILE
// working copy - scaffold materialised by --work
// name      sub_5ed700
// size      180 bytes
// spans     0x005ED700-0x005ED7B4
// prototype 
...
// subject: sub_5ed700  at 0x005ED700  (180 bytes)
//
...
typedef int (__stdcall *IsRectEmptyFn)(const RECT *);

class WinCentering { public:
    int center_on_parent();
};

int WinCentering::center_on_parent() {
    Win *self = reinterpret_cast<Win *>(this);
    Win *parent = self->win_parent_;
    if (parent == 0) {
        return 1;
    }
    RECT local = parent->client_rect_;
    if ((*(IsRectEmptyFn)*g_0066933c)(&local)) {
        return 7;
    }
    return self->move(
        local.left + (((parent->client_rect_.right - self->client_rect_.right) +
                        self->client_rect_.left - parent->client_rect_.left) >> 1),
        local.top + (((parent->client_rect_.bottom - parent->client_rect_.top) -
                       self->client_rect_.bottom + self->client_rect_.top) >> 1));
}
