...
// subject: ?line_timer@@YAXH@Z  at 0x0050EE80  (129 bytes)
//
...
void __cdecl line_timer(int a1) {
    ...
    Win *w1 = reinterpret_cast<Win *>(g_006a7628);
    if (w1->is_visible()) return;

    Win *w2 = reinterpret_cast<Win *>(g_008a6270);
    if (w2->is_visible()) return;

    int count = *g_00939fd4;
    count--;
    *g_00939fd4 = count;

    PlanWin *pw = reinterpret_cast<PlanWin *>(g_00856dc0);
    pw->on_redraw();

    for (int *p = g_007d3c3c; reinterpret_cast<int>(p) < reinterpret_cast<int>(g_007d3c5c); p++) {
        int obj = *p;
        if (obj != 0) {
            if (p == g_007d3c3c || *reinterpret_cast<int *>(obj + 0x1dd74) != 0) {
                if (*reinterpret_cast<int *>(obj + 0x1dd70) & 0xc40000) {
