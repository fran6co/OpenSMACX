...
// calls     0x0045B550 0x004C3950 0x004C3990 0x004E0A50
// indirect  0x0045CCEA

// GENERATED SKELETON - tools/emit_translation_unit.py
// subject: ?on_iface_group_clicked@MainInterface@@QAEXHHH@Z  at 0x0045CCB0  (197 bytes)
//
...
extern int g_7ae778_arr[];
extern int g_691888_table[];

void MainInterface::on_iface_group_clicked(int a1, int a2, int a3) {
    if (g_7ae778_arr[*g_007d392c] != 0) {
        return;
    }
    if (a3 < 0) {
        return;
    ...
    if (a1 == 0) {
        reinterpret_cast<VCall *>(reinterpret_cast<char *>(this) - 0xa14)->slot012();
        return;
    }
    int val = a3 - 1000;
    if (a1 == 2) {
        if (val >= 0x11) {
            int idx = val - 0x11;
            int i = 0;
            char *entry = reinterpret_cast<char *>(g_691888_table);
            while (reinterpret_cast<int>(entry) < 0x691b08) {
                if (*reinterpret_cast<int *>(entry) == 0 && i <= idx) {
                    idx++;
                }
                entry += 0x20;
                i++;
            ...
            idx += 6;
            reinterpret_cast<Console *>(0x9156b0)->editor_paints(idx);
        } else {
            reinterpret_cast<Console *>(0x9156b0)->editor_paints(val - 0xb);
        }
        return;
    }
    if (a1 == 3) {
        *g_00945ae4 = val - 0x24;
        reinterpret_cast<MainInterface *>(0x7ae820)->draw_edit_pane();
        return;
    }
    if (val == 6) {
        reinterpret_cast<WorldWin *>(0x8e9f60)->set_world_map();
    } else {
        reinterpret_cast<WorldWin *>(0x8e9f60)->set_detail_map();
    }
}
