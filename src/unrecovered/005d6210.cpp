...
// indirect  0x005D62B4 0x005D62C2 0x005D62FE

// GENERATED SKELETON - tools/emit_translation_unit.py
// subject: ?on_size@GraphicWin@@QAEXIHH@Z  at 0x005D6210  (248 bytes)
//
...
// This body dispatches through slot(s): 62, 66, 74
class VCall { public:
...
class GraphicWin : public Win { public:
    ...
    void redraw();
    void soft_update();
    void on_size(unsigned int, int, int);
};
...
void GraphicWin::on_size(unsigned int a1, int a2, int a3) {
    char *self = reinterpret_cast<char *>(this);
    if (a1 != 1) {
        int x = a2;
        int y = a3;
        if (x == 0 || y == 0) {
            x = *reinterpret_cast<int *>(self + 0x4C4);
            y = -*reinterpret_cast<int *>(self + 0x4C8);
        }
        unsigned int flags = *reinterpret_cast<unsigned int *>(self + 0x98);
        if ((flags & 8) != 0) {
            x -= *g_009b8dd4;
        }
        if ((flags & 4) != 0) {
        ...
        buffer_.init(x, y, *reinterpret_cast<int *>(self + 0x460), 0);
        buffer_.sync_to_palette(reinterpret_cast<Palette *>(g_009b8180));

        flags = *reinterpret_cast<unsigned int *>(self + 0x98);
        if ((flags & 0x40000000) != 0 &&
            *reinterpret_cast<int *>(self + 0xA08) != 0) {
            buffer_.resize(x, y);
            reinterpret_cast<Buffer *>(*reinterpret_cast<int *>(self + 0xA08))
                ->sync_to_palette(reinterpret_cast<Palette *>(g_009b8180));
        }

        reinterpret_cast<WinVCall *>(self)->slot066();
        reinterpret_cast<WinVCall *>(self)->slot074(0, -1);

        resize_event(*reinterpret_cast<int *>(self + 0x47C) -
                         *reinterpret_cast<int *>(self + 0x474),
                     *reinterpret_cast<int *>(self + 0x480) -
                         *reinterpret_cast<int *>(self + 0x478));
        if (is_visible()) {
            reinterpret_cast<WinVCall *>(self)->slot062();
        }
    }
}
