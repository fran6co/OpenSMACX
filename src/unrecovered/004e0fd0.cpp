...
// subject: ?editor_generate@Console@@QAEXXZ  at 0x004E0FD0  (236 bytes)
//
...
// an indirect virtual call on the fixed-address singleton at 0x007B0CB8 is
// spelled by calling the Nth virtual of a class that is never defined and
// never instantiated. `vtable_slots` in emit_translation_unit.py only
// recognises `call [reg+N]` where `reg` was loaded from ANOTHER register's
// deref; this body loads the vtable pointer from a fixed absolute address
// (`mov edx, [0x7B0CB8]`), which the heuristic does not classify as an
// object load, so no shim was auto-generated.
class VCall { public:
    virtual void slot000();
    virtual void slot001(int);  // <-- used, called with arg 0
    virtual void slot002();     // <-- used, called with no args
};

void Console::editor_generate() {
    auto_undo();

    int count = *g_00949884;
    unsigned int *p = reinterpret_cast<unsigned int *>(*g_0094a30c + 8);
    ...
    *reinterpret_cast<int *>(*g_007d3c3c + 0x1dd98) = 0xfffffff2;
    reinterpret_cast<MapWin *>(*g_007d3c3c)->draw_map(1);

    int flags = *g_009a64c0;
    int wasSet = flags & 0x80;
    flags |= 0x80;
    *g_009a64c0 = flags;
    world_build();
    draw_map(1);
    ...
    if ((flagByte & 0x80) != 0 && (flagByte & 0x40) != 0) {
        reinterpret_cast<VCall *>(0x007B0CB8)->slot001(0);
        return;
    }
    reinterpret_cast<VCall *>(0x007B0CB8)->slot002();
}
