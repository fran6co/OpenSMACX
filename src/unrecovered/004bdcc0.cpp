...
// TRIED: `extern int g_009a64bc_rmw;` for the trailing `or [addr],esi` (RMW on a fixed global) turned the const-pointer's load/or/store into a single `or`, reaching MNEMONIC_ONLY (61/61 mnemonics match); the one remaining divergence is register choice in the tail, where the original reuses esi for child2 and ours picks another.
...
// calls     0x005FCBB0
// indirect  0x004BDCDA 0x004BDCF2 0x004BDCFD

// GENERATED SKELETON - tools/emit_translation_unit.py
// subject: ?on_button_clicked@TutWin@@QAEXH@Z  at 0x004BDCC0  (226 bytes)
//
...
// This body dispatches through slot(s): 2, 58, 63
class VCall { public:
...
extern int g_009a64bc_rmw;

void TutWin::on_button_clicked(int a1) {
    char *self = reinterpret_cast<char *>(this);
    int prevFlag = *g_008cc244;
    if (*reinterpret_cast<int *>(self + 0x53b8) != 0) {
    ...
    if (*reinterpret_cast<int *>(self + 0x53d4) != 0) {
        *reinterpret_cast<int *>(self + 0x53a4) = 0;
        ...
        int *child = *reinterpret_cast<int **>(self + 0x53d4);
        reinterpret_cast<VCall *>(child)->slot063();
        do_all_draws();
    }
    ...
    *g_008cc244 = 0;
    ...
    int bcc = (a1 < 0) ? (a1 + 1) : a1;
    *reinterpret_cast<int *>(self + 0x53bc) = bcc;
    if (a1 < -1) {
    ...
    if (*reinterpret_cast<int *>(self + 0x53b4) == 0) {
        if (prevFlag != 0) {
            *g_008cc248 = 1;
        }
        if (*reinterpret_cast<int *>(self + 0x53b4) == 0) {
    ...
    *reinterpret_cast<int *>(self + 0x53b4) = 0;
skip_clear:
    int child2 = *reinterpret_cast<int *>(self + 0x53d0);
    if (child2 != 0) {
        g_009a64bc_rmw |= child2;
    }
}
