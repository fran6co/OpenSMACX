...
// subject: ?on_key_down@BasePop@@QAEHH@Z  at 0x00604410  (126 bytes)
//
...
int BasePop::on_key_down(int a1) {
    char *self = reinterpret_cast<char *>(this);
    if ((*reinterpret_cast<unsigned int *>(self + 0x30a8) & 0x4000) != 0) {
        return 0;
    }
    *reinterpret_cast<int *>(self + 0xa40) = 0;
    int kind = *reinterpret_cast<int *>(self + 0x2350);
    if (kind != -1) {
        int base1 = *reinterpret_cast<int *>(self + 0x21d0);
        int off1 = *reinterpret_cast<int *>(base1 + 8);
        int adj = off1 + reinterpret_cast<int>(self);
        if (*reinterpret_cast<int *>(adj + 0x229c) != 0) {
            if (kind == 1) {
                int base2 = *reinterpret_cast<int *>(self + 0x2228);
                int off2 = *reinterpret_cast<int *>(base2 + 8);
                *reinterpret_cast<int *>(self + 0xa40) =
                    *reinterpret_cast<int *>(off2 + reinterpret_cast<int>(self) + 0x2314);
                return 1;
            }
            Dialog *dlg = reinterpret_cast<Dialog *>(adj + 0x21d0);
            *reinterpret_cast<int *>(self + 0xa40) = dlg->get_selected_id();
        }
    }
    return 1;
}
