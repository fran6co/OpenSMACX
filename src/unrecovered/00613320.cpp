// ORIGINAL: 0x00613320 FILE
// RULED-OUT: 208 vs 210 bytes; the RadioButton::close() receiver compiles
//            to `mov ecx,eax; call` instead of a direct `lea ecx,[..]; call`.
//            Tried a shared base-address local, no local at all, and a
//            named typed-pointer local for the receiver; all three landed
//            on the same divergence, so it looks like inherent codegen for
//            a computed `this` into a non-virtual thiscall call, not a
//            source-form choice.
// working copy - scaffold materialised by --work
// name      ?init@Dialogs@@QAEHHHHHH@Z
// size      210 bytes
// spans     0x00613320-0x006133F2
// prototype int (__thiscall ?init@Dialogs@@QAEHHHHHH@Z)(Dialogs* this, int, int, int, int, int)
// callers   0   call targets   6
// kind      game
// flags     hidden;sp_ready;purged_ok
// calls     0x00609300 0x00609F20 0x0060D1B0 0x0060E7C0 0x00610440 0x00611A90

// GENERATED SKELETON - tools/emit_translation_unit.py
// subject: ?init@Dialogs@@QAEHHHHHH@Z  at 0x00613320  (210 bytes)
//
...
int Dialogs::init(int a1, int a2, int a3, int a4, int a5) {
    char *base1 = reinterpret_cast<char *>(this) + 0x44;
    reinterpret_cast<RadioButton *>(base1)->close();
    int *vtbl1 = *reinterpret_cast<int **>(base1);
    Dialog *dlg1 = reinterpret_cast<Dialog *>(base1 + *reinterpret_cast<int *>(reinterpret_cast<char *>(vtbl1) + 8));
    int result = dlg1->init(a1, a2, a3, a4, a5);
    if (result != 0) {
        return result;
    }

    reinterpret_cast<ListBox *>(this)->close();
    int *vtbl2 = *reinterpret_cast<int **>(this);
    Dialog *dlg2 = reinterpret_cast<Dialog *>(reinterpret_cast<char *>(this) + *reinterpret_cast<int *>(reinterpret_cast<char *>(vtbl2) + 8));
    result = dlg2->init(a1, a2, a3, a4, a5);
    if (result != 0) {
        return result;
    }

    char *base3 = reinterpret_cast<char *>(this) + 0x58;
    reinterpret_cast<CheckBox *>(base3)->close();
    int *vtbl3 = *reinterpret_cast<int **>(base3);
    Dialog *dlg3 = reinterpret_cast<Dialog *>(base3 + *reinterpret_cast<int *>(reinterpret_cast<char *>(vtbl3) + 8));
    result = dlg3->init(a1, a2, a3, a4, a5);
    if (result != 0) {
        return result;
    }

    result = reinterpret_cast<SpriteBox *>(reinterpret_cast<char *>(this) + 0x70)->init(a1, a2, a3, a4, a5);
    if (result != 0) {
        return result;
    }

    char *base5 = reinterpret_cast<char *>(this) + 0xf8;
    reinterpret_cast<EditGroup *>(base5)->close();
    int *vtbl5 = *reinterpret_cast<int **>(base5);
    Dialog *dlg5 = reinterpret_cast<Dialog *>(base5 + *reinterpret_cast<int *>(reinterpret_cast<char *>(vtbl5) + 8));
    return dlg5->init(a1, a2, a3, a4, a5);
}
