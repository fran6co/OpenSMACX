...
// RULED-OUT: `cnt <= a1` remove-vs-decrement polarity matched (was `a1<cnt`,
//   flipped to match `jbe`); loading head into a local before the `if` still
//   compiles the null check as `test` where the original uses a pre-zeroed
//   register and `cmp` (it reuses that zero for two later stack inits) -
//   not reproduced, that reuse is a scheduling artifact not a source shape.
//   Adjustor-thunk release pattern (vtbl[1]-as-delta, then vtbl2[0]) and the
//   list-container slot1 dispatch both copied verbatim from 0x0045F640's
//   proven VCallObjI/VCallObjP shim and matched structurally.
...
// subject: ?clip_ids@StringBox@@QAEXK@Z  at 0x00629A70  (249 bytes)
//
...
void StringBox::clip_ids(unsigned long a1) {
    char *self = reinterpret_cast<char *>(this);
    void *head = *reinterpret_cast<void **>(self + 0x2b78);
    char *lst = self + 0x2b70;

    if (head != 0) {
        bool removed = false;
        int i = 0;
        if (*reinterpret_cast<int *>(lst + 0x10) > 0) {
            do {
                char *cur = *reinterpret_cast<char **>(lst + 0xc);
                unsigned int cnt = *reinterpret_cast<unsigned int *>(cur + 4);
                if (cnt <= a1) {
                    removed = true;
                    char *next = *reinterpret_cast<char **>(cur + 0xc);
                    char *prev = *reinterpret_cast<char **>(cur + 0x10);
                    *reinterpret_cast<char **>(next + 0x10) = prev;
                    *reinterpret_cast<char **>(prev + 0xc) = next;

                    if (cur == *reinterpret_cast<char **>(lst + 8)) {
                        *reinterpret_cast<char **>(lst + 8) = next;
                    }
                    *reinterpret_cast<char **>(lst + 0xc) = next;

                    void *payload = *reinterpret_cast<void **>(cur + 8);
                    {
                        VCallShimP shimP;
                        shimP.raw = *reinterpret_cast<void **>(*reinterpret_cast<int *>(lst) + 4);
                        (reinterpret_cast<VCallObjP *>(lst)->*shimP.memberFn)(payload);
                    }
                    if (payload != 0) {
                    ...
                    *reinterpret_cast<int *>(cur + 8) = 0;
                    if (cur != 0) {
                        int vtbl2 = *reinterpret_cast<int *>(cur);
                        char *adj2 = cur + *reinterpret_cast<int *>(vtbl2 + 4);
                        VCallShimI shimI2;
                        shimI2.raw = *reinterpret_cast<void **>(adj2);
                        (reinterpret_cast<VCallObjI *>(adj2)->*shimI2.memberFn)(1);
                    }
                    *reinterpret_cast<int *>(lst + 0x10) = *reinterpret_cast<int *>(lst + 0x10) - 1;
                } else {
                    *reinterpret_cast<unsigned int *>(cur + 4) = cnt - a1;
                    *reinterpret_cast<char **>(lst + 0xc) = *reinterpret_cast<char **>(cur + 0xc);
                }
                ++i;
            } while (i < *reinterpret_cast<int *>(lst + 0x10));
        }
        if (*reinterpret_cast<int *>(lst + 0x10) == 0) {
            *reinterpret_cast<void **>(lst + 8) = 0;
        }
        *reinterpret_cast<int *>(lst + 0x14) = *reinterpret_cast<int *>(lst + 0x10) - 1;
        if (removed) {
            add_fixup();
        }
    }
