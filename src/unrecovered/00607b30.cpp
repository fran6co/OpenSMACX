...
// RULED-OUT: reading the 4 rect fields through a `RECT*` base pointer, and
//   through 4 plain named int locals (Ghidra's own local_10/local_c/local_8/
//   local_4 shape) - neither recovers the original's single `lea`-then-4-reads
//   codegen; /O2 keeps re-deriving `[esi+0x13c+k]` per read either way. Not
//   reproduced; everything else (branch shape, vtable shim at +0x50, Time
//   start/stop calls) matches.
...
// subject: ?timer_callback@BaseButton@@QAEXH@Z  at 0x00607B30  (249 bytes)
//
...
class VCallObjII { public: void slot(int, int); };
union VCallShimII {
    void (VCallObjII::*memberFn)(int, int);
    void *raw;
};

void BaseButton::timer_callback(int a1) {
    if (a1 == 0) {
        ((Time *)((char *)this + 0xa1c))->stop();
        if (*(int *)((char *)this + 0xa80) != 0) {
            RECT *src = (RECT *)((char *)this + 0x13c);
            RECT r;
            int x = 0;
            int y = 0;
            r.left = src->left;
            ...
            r.right += dx;
            r.bottom += dy;
            set_bubble_text(*(char **)((char *)this + 0xa80), &r);
        }
        return;
    } else if (a1 == 1) {
        int t = *(int *)((char *)this + 0xa48);
        if (t <= 0) {
            t = *(int *)((char *)this + 0xa44);
        }
        ((Time *)((char *)this + 0xa4c))->start((void (__cdecl *)(int, int))0x607b10, 2, (int)this, t, 5);
        return;
    } else {
        VCallShimII shim;
        shim.raw = *(void **)((char *)*(void **)this + 0x50);
        (reinterpret_cast<VCallObjII *>(this)->*shim.memberFn)(0, 0);
    }
}
