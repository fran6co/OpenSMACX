...
// TRIED: computing the a1*0xd flag-slot index once vs re-indexing read+write - both compile; landed at MISMATCH (edit_count 3, 97% mnemonic similarity) with the array-indexed form.
// size      238 bytes
...
// calls     0x0047A890 0x004E3A50 0x00531020 0x005310F0 0x00531480 0x00593220

// GENERATED SKELETON - tools/emit_translation_unit.py
// subject: ?set_bombing_run@Console@@QAEXHHH@Z  at 0x004D8CD0  (238 bytes)
//
...
static int *const g_00688ec0 = (int *)0x00688EC0;
...
void Console::set_bombing_run(int a1, int a2, int a3) {
    extern unsigned char g_byte_952836[];
    extern unsigned char g_byte_97d044[];
    extern unsigned char g_byte_96c9f8[];
    extern int g_dword_95282c[];
    extern unsigned char g_byte_95284e[];
    extern short g_word_95283e[];
    extern short g_word_952846[];

    int baseIdx = base_at(a2, a3);
    if (baseIdx < 0 ||
        (g_byte_96c9f8[(g_byte_952836[a1 * 0x34] * 0x833 +
                         g_byte_97d044[baseIdx * 0x134]) * 4] & 0x10) == 0) {
        NetMsg *msg = reinterpret_cast<NetMsg *>(0x00805338);
        msg->pop("NOBOMB", 5000, 0, 0);
        return;
    }
    NetDaemon *nd = reinterpret_cast<NetDaemon *>(0x0093CD90);
    if (nd->lock_veh(&a1, 0, -1, -1, 0) != 0) {
        return;
    }
    g_dword_95282c[a1 * 0xd] = (g_dword_95282c[a1 * 0xd] & 0xfcffffff) | 0x200;
    g_byte_95284e[a1 * 0x34] = 10;
    g_word_95283e[a1 * 0x1a] = static_cast<short>(a2);
    g_word_952846[a1 * 0x1a] = static_cast<short>(a3);
    synch_veh(a1);
    nd->await_synch();
    nd->unlock_veh();
}
