...
//        as nullary returning int; the disassembly reads a real stack
//        argument at [esp+4] and never sets eax on purpose, so it is a
//        one-`int`-parameter `void` function instead. Min-of-two-sizes has
//        to be spelled `if (count > mySize) count = mySize;` (condition
//        polarity `>`, not `<` on the flipped operands) to get the
//        original's `jbe`; the algebraically-equivalent `if (mySize <
