// ORIGINAL: 0x005C0CB0 FILE
// RULED-OUT: plain `abs()` gets intrinsic-inlined by /O2 (cdq/xor/sub) where
//   the original has a real `call _abs`; `extern "C" int abs(int);` plus
//   `#pragma function(abs)` forces the real call and fixed nearly all of it.
//   Remaining MISMATCH is `or eax,0FFFFFFFFh` vs `mov eax,-1` for best_idx's
//   initial -1 - an instruction-selection/scheduling choice tied to the
//   adjacent `mov ecx,[DAT_9a64c8]` load, not source form; not reproduced.
// name      ?veh_find@@YAHHHHH@Z
// size      249 bytes
// spans     0x005C0CB0-0x005C0DA9
// prototype
...
// subject: ?veh_find@@YAHHHHH@Z  at 0x005C0CB0  (249 bytes)
//
...
static int *const g_009b227c = (int *)0x009B227C;
extern "C" int abs(int);
#pragma function(abs)

void veh_find(int a1, int a2, int a3, int a4) {
    int best_dist = 9999;
    *(int *)0x9b227c = 9999;
    int best_idx = -1;
    int i = 0;
    if (*(int *)0x9a64c8 > 0) {
        short *rec = (short *)0x95282a;
        do {
            if (((a3 < 0) || (*((unsigned char *)rec + 0xc) == (unsigned)a3)) && (i != a4)) {
                int y = rec[0];
                int x = rec[-1];
                int d1 = abs(a1 - x);
                if (((*(unsigned char *)0x94988c & 1) == 0) && (d1 > *(int *)0x68faf0)) {
                    d1 = *(int *)0x949870 - d1;
                }
                int d2 = abs(a2 - y);
                int ad1 = abs(d1);
            ...
            ++i;
            rec += 0x1a;
        } while (i < *(int *)0x9a64c8);
        if (best_idx >= 0) {
            *(int *)0x9b227c = best_dist;
        }
    }
}
