// ORIGINAL: 0x00604EA0
// name      ?UNK2@BasePop@@QAEHXZ
// size      362 bytes
// spans     0x00604EA0-0x0060500A
...
//            and the flattened two-`if` spelling both lower to inline
//            `jne`/`mov` sequences here, where the original keeps a
//            `je`/`push`/`jmp`-to-common-call-site shape for the same
//            three-way string fallback (repeated twice, for the two
//            caption strings). 0.76 mnemonic similarity, first divergence
//            at #16 (that shape difference); not chased further. The
//            `*(int*)4` read at self+0x215c's null-fallback branch
//            (`xor eax,eax; mov eax,[eax+4]`) is reproduced literally via
//            a fixed global at address 4, matching what Ghidra's
//            `uRam00000004` names.
