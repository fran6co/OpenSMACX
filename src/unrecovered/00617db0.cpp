// ORIGINAL: 0x00617DB0
// name      ?add_object@Caviar@@QAEHPAUCaviarData@@PAEPAUCaviarData@@H@Z
// size      367 bytes
// spans     0x00617DB0-0x00617F1F
...
// calls     (none)
// RULED-OUT: field offsets (0x74C/0x754/0x10AC/0x10C/0x110/0x13CC) match
//            src/caviar.h's carved-out `Caviar` layout exactly, so the
//            scaffold's real class (from the catalogued mangled name) is
//            used as-is; only `CaviarData` is redefined here (opaque,
//            three raw fields matching src/caviar.h's private layout) to
//            avoid its `close()` method and static_assert. Three parallel
//            per-entry arrays get written each loop iteration (a
//            single-byte table at +0x74C stride 0xC, a 3-dword record
//            table at +0x9C0 stride 0xC via a struct copied out of `a3`,
//            and a 9-dword table at +0x754 stride 0xC via the `rep movsd`
//            in the original, written here as a plain element loop), plus
//            a pointer table at +0x10AC and an 8-byte-record table at
//            +0x10C/+0x110 sourced from `a1`. 0.56 mnemonic similarity,
//            first divergence at #0 - not chased further.
