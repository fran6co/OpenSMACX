...
// TRIED: extra VC24/VC43 shims for the two non-void vtable slots (23 returns int, 42 takes int); IAT-style function pointers for timeGetTime/Sleep instead of named imports. First divergence #43 test/push.
...
// calls     0x00616730
// indirect  0x00607739 0x00607767 0x00607782 0x006077AC 0x006077B7 0x006077D1 0x006077DB

// GENERATED SKELETON - tools/emit_translation_unit.py
// subject: ?on_left_up@BaseButton@@QAEXHH@Z  at 0x00607720  (198 bytes)
//
...
// This body dispatches through slot(s): 4, 7, 23, 42, 62
class VCall { public:
...
// Slot 23 returns int (used as a boolean); slot 42 takes one int. The
// scaffold's VCall types every slot void(), so a second/third shim
// with the real trailing signature is needed for these two calls.
class VC24 { public:
    ...
    virtual int  s23();  // <-- used, slot23, returns int
};
...
class VC43 { public:
    ...
    virtual void s42(int);  // <-- used, slot42, takes one int
};

typedef unsigned int (__stdcall *FnTimeGetTime)();
typedef void (__stdcall *FnSleep)(unsigned int);

void BaseButton::on_left_up(int a1, int a2) {
    ...
    char *owner = *reinterpret_cast<char **>(self + 0xc4);
    if (owner != 0) {
        time2_.stop();
        FnTimeGetTime timeGetTimeFn = *reinterpret_cast<FnTimeGetTime *>(g_00669368);
        unsigned int now = timeGetTimeFn();
        unsigned int elapsed = now - field_AA4_;
        if (elapsed < field_AA0_) {
            unsigned int remaining = field_AA0_ - elapsed;
            FnSleep sleepFn = *reinterpret_cast<FnSleep *>(g_0066912c);
            sleepFn(remaining);
        }
        if (field_A9C_ == 1) {
            field_A9C_ = 0;
            owner = *reinterpret_cast<char **>(self + 0xc4);
            char *inner = *reinterpret_cast<char **>(owner + 0x74);
            if (inner != 0) {
                int result = reinterpret_cast<VC24 *>(inner)->s23();
                if (result == 0) {
                    reinterpret_cast<VCall *>(inner)->slot007();
                }
            }
            owner = *reinterpret_cast<char **>(self + 0xc4);
            reinterpret_cast<VC43 *>(owner)->s42(field_A78_);
