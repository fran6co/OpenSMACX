...
// calls     0x0043C1A0 0x004C5D90 0x0051D7D0 0x00590E90 0x0059A2D0 0x005C98E0
// indirect  0x0058F27F 0x0058F2D1

// GENERATED SKELETON - tools/emit_translation_unit.py
// subject: ?system_close@@YAXXZ  at 0x0058F250  (150 bytes)
//
...
void __cdecl system_close() {
    ...
    int result;
    if (obj != 0) {
        int off = *reinterpret_cast<int *>(reinterpret_cast<char *>(obj) + 4);
        result = reinterpret_cast<int>(obj) + off;
    } else {
        result = 0;
    }
    *g_009b8dd0 = result;

    VCall1 *snd = *reinterpret_cast<VCall1 **>(g_00945824);
    if (snd != 0) {
        snd->slot000(1);
        *g_00945824 = 0;
    }
    ...
    reinterpret_cast<MultiDebug *>(g_009b22f0)->close();
    reinterpret_cast<Console *>(g_009156b0)->close();
    reinterpret_cast<Path *>(g_00945b00)->shutdown();
    map_shutdown();
    ...
    for (int *p = g_007d3c40; p < g_007d3c5c; p++) {
        ...
        if (base != 0) {
            ...
            int off2 = vtbl[1];
            VCall1 *o = reinterpret_cast<VCall1 *>(base + off2);
            o->slot000(1);
            *p = 0;
        }
    }
