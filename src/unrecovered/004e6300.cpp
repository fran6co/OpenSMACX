...
void __cdecl bases_reset(int a1, int a2, int a3) {
    int mask = 1 << a2;
    if (((unsigned char)mask & *(unsigned char *)g_009a64e8) == 0) {
        int count = *(int *)g_009a64cc;
        int i = 0;
        if (count > 0) {
            unsigned char *table = (unsigned char *)*(int *)g_0094a30c;
            int mult = *(int *)g_0068faf0;
            short *rec = (short *)g_0097d042;
            do {
                if ((*(unsigned char *)((char *)rec + 2) == a2) &&
                    (a1 < 0 ||
                     *(unsigned char *)(table + 3 + (rec[0] * mult + (rec[-1] >> 1)) * 0x2c) == a1) &&
                    (a3 == 0 ||
                     (*(unsigned char *)(table + 8 + (rec[0] * mult + (rec[-1] >> 1)) * 0x2c) & 2) == 0 ||
                     7 < (*(unsigned char *)(table + (rec[0] * mult + (rec[-1] >> 1)) * 0x2c + 2) & 0xf)) &&
                    (((unsigned char)mask & *(unsigned char *)g_009a64e8) == 0 ||
                     (*(unsigned int *)(*(int *)g_0090ea30 + 0x30) & 0x80000000) == 0)) {
                    base_reset(i, 0);
                    table = (unsigned char *)*(int *)g_0094a30c;
                    mult = *(int *)g_0068faf0;
                }
                i++;
                rec += 0x9a;
