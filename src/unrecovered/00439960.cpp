...
// TRIED: dropping the trailing `return;` on the last case (falling out of the switch) - identical trailing divergence either way. All 61 original mnemonics match 1:1 (similarity 0.90); the remaining MISMATCH #61 'end' vs 'mov' is 26 bytes of dead code VC6 appends after the last epilogue, not yet explained.
...
// calls     0x00439F20 0x0043A300 0x0043A760 0x0043AA00 0x0043AC70 0x004BDDE0 0x005FAB00

// GENERATED SKELETON - tools/emit_translation_unit.py
// subject: ?on_left_click@DesignWin@@QAEXHH@Z  at 0x00439960  (154 bytes)
//
...
static int *const g_004399fc = (int *)0x004399FC;
...
void DesignWin::on_left_click(int a1, int a2) {
    reinterpret_cast<TutWin *>(g_008c6e68)->reset();

    int position;
    int type;
    if (spot_.check(a1, a2, &position, &type) != -1) {
        switch (type) {
        case 0:
            select_chassis();
            return;
