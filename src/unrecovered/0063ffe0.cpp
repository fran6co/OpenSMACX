...
// RULED-OUT: the three callees (sub_627f50/sub_628220/sub_6281b0) are
//            catalogued nullary, so they are reached through explicit
//            cdecl function-pointer casts rather than by name. The first
//            round-to-nearest computation tests the sign of localC[0] (the
//            sub_628220 output) but rounds local18 (the sub_6281b0 output)
//            - an asymmetry the disassembly shows plainly and this keeps,
//            even though axes 1 and 2 are self-referential. A shared
//            `round_nearest` free-function helper ahead of the definition
//            compiled to zero external .text symbols (body-mode extraction
//            only special-cases a helper CLASS, not a helper free
//            function) - inlined the three round computations instead.
