// ORIGINAL: 0x0046F660 FILE
// RULED-OUT: `int` return (mov eax,1 before ret) - the scaffold's
//            declared prototype is void, so C2556/C2371 on redefinition;
//            keeping void drops the eax=1 the early-return path sets.
//            MISMATCH #6 'mov' vs 'push' remains open (instruction
//            scheduling around the in_box/rect stores).
// working copy - scaffold materialised by --work
// name      ?on_nc_hittest@MapWin@@QAEXHH@Z
// size      153 bytes
// spans     0x0046F660-0x0046F6F9
// prototype void (__thiscall ?on_nc_hittest@MapWin@@QAEXHH@Z)(MapWin* this, int, int)
// callers   0   call targets   2
// kind      game
// flags     frame;hidden;sp_ready;purged_ok
// calls     0x005F5AD0 0x005FA7E0

// GENERATED SKELETON - tools/emit_translation_unit.py
// subject: ?on_nc_hittest@MapWin@@QAEXHH@Z  at 0x0046F660  (153 bytes)
//
...
void MapWin::on_nc_hittest(int a1, int a2) {
    // [esi-0x21a6c] is the vbptr for the virtual Win base; vbptr[1] is the
    // displacement to that base, matching the vbtable-slot-2 lever.
    char *self = reinterpret_cast<char *>(this);
    RECT rect;
    rect.top = 0xe;
    rect.bottom = 0x1f;

    int *vbptr = *reinterpret_cast<int **>(self - 0x21a6c);
    int disp = vbptr[1];
    int v1 = *reinterpret_cast<int *>(self + disp - 0x21918);
    int v2 = *reinterpret_cast<int *>(self + disp - 0x21920);
    int diff = v1 - v2 - 0x5b;
    rect.left = diff;
    rect.right = diff + 0x50;

    if (in_box(a1, a2, &rect)) {
        return;
    }

    Win *win_this = reinterpret_cast<Win *>(self + disp - 0x21a6c);
    win_this->on_nc_hittest(a1, a2);
}
