...
// calls     0x00609A50
// indirect  0x0060C73B 0x0060C761 0x0060C789 0x0060C7A8 0x0060C7B3

// GENERATED SKELETON - tools/emit_translation_unit.py
// subject: ?on_left_double_click@ListBox@@QAEXHH@Z  at 0x0060C710  (171 bytes)
//
...
// This body dispatches through slot(s): 7, 23, 55, 56
class VCall { public:
    ...
    virtual void slot055();  // <-- used
    virtual void slot056();  // <-- used
};
...
static int *const g_009b8eec = (int *)0x009B8EEC;

// A second vtable shim: the generated VCall above types every slot
// void(), but slot055/slot056 here take arguments.
class VCall2 { public:
    virtual void slot000();
    virtual void slot001();
    virtual void slot002();
    ...
    virtual void slot053();
    virtual void slot054();
    virtual void slot055(int, int);  // <-- used
...
void ListBox::on_left_double_click(int a1, int a2) {
    char *self = (char *)this;
    int gw_disp = (*(int **)(self - 0x48))[1];
    char *gw_base = self + gw_disp;
    void *target = *(void **)(gw_base + 0x7c);
    *(void **)g_009b7ab8 = target;

    void (__cdecl *cb)(int) = *(void (__cdecl **)(int))(self - 8);
    if (cb != 0) {
        int dlg_disp = (*(int **)(self - 0x48))[2];
        Dialog *dlg = (Dialog *)(self - 0x48 + dlg_disp);
        int id = dlg->get_selected_id();
        cb(id);
    }

    gw_disp = (*(int **)(self - 0x48))[1];
    gw_base = self + gw_disp;
    VCall2 *obj1 = *(VCall2 **)(gw_base + 0x7c);
    if (obj1 != 0) {
        int dlg_disp = (*(int **)(self - 0x48))[2];
        Dialog *dlg = (Dialog *)(self - 0x48 + dlg_disp);
        int id = dlg->get_selected_id();
    ...
    int gvar = *g_009b8eec;
    gw_disp = (*(int **)(self - 0x48))[1];
    int dlg_disp2 = (*(int **)(self - 0x48))[2];
    gw_base = self + gw_disp;
    VCall2 *obj2 = *(VCall2 **)(gw_base + 0x7c);
    Dialog *dlg2 = (Dialog *)(self - 0x48 + dlg_disp2);
    int id2 = dlg2->get_selected_id();
    obj2->slot055(id2, gvar);

    gw_disp = (*(int **)(self - 0x48))[1];
    gw_base = self + gw_disp;
    char *finalobj = *(char **)(gw_base + 0x7c);
    VCall2 *child = *(VCall2 **)(finalobj + 0x94);
    if (child != 0) {
        if (child->slot023() == 0) {
            child->slot007();
        }
