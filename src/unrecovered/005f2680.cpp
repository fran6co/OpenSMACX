...
// TRIED: the auto-scaffold's `VCall` shim fixes every slot to `void()`, so slot023's return value (tested with `test eax,eax` after the call) can't be read through it (C2120, void compared to 0). Declared a second, unused-by-the-scaffold `MyVCall` shim with the same slot layout but `int slot023()`. `return 0` after the SC_CLOSE branch adds an extra "xor eax,eax" the original's bare tail doesn't have (#46, both early-return and nested-if forms land the same divergence).
...
// calls     (none)
// indirect  0x005F2689 0x005F26C1 0x005F26DA 0x005F26E0 0x005F26EE 0x005F26F9

// GENERATED SKELETON - tools/emit_translation_unit.py
// subject: ?OnSysCommand@Win@@QAAHPAXIHH@Z  at 0x005F2680  (127 bytes)
//
...
// This body dispatches through slot(s): 7, 14, 23
class VCall { public:
...
class Win { public:
    ...
    int __cdecl OnSysCommand(void *, unsigned int, int, int);
};
...
// A second shim alongside the auto-generated `VCall` above: that one fixes
// every slot to `void()`, but slot023's return value is tested here
// (`test eax,eax` after the call), which `void` cannot express.
...
int __cdecl Win::OnSysCommand(void * a1, unsigned int a2, int a3, int a4) {
    Win *self = reinterpret_cast<Win *>(
        (*reinterpret_cast<GetWindowLongAFn *>(g_0066934c))(a1, -0x15));
    if (self == 0) {
        return 0;
    }
    if (a2 != 0xf060) {
    ...
    *reinterpret_cast<Win **>(g_009b7ab8) = self;
    ...
    reinterpret_cast<MyVCall *>(self)->slot014();
