...
// subject: ?redraw@StatusWin@@QAEXXZ  at 0x004B9EA0  (219 bytes)
//
...
typedef void (__stdcall *RectFn)(int, int, int);

void StatusWin::redraw() {
    char *self = reinterpret_cast<char *>(this);
    int sizeVal = (*g_009b7b1c != 0x320) ? 0xC : 0xA;
    int *sizeSlot = reinterpret_cast<int *>(self + 0x15ac);
    if (sizeVal != *sizeSlot) {
        *sizeSlot = sizeVal;
        char *fontFile = reinterpret_cast<char *>(*g_00691b2c);
        reinterpret_cast<Font *>(self + 0x1530)->init(fontFile, sizeVal, 0);
        fontFile = reinterpret_cast<char *>(*g_00691b2c);
        reinterpret_cast<Font *>(self + 0x1558)->init(fontFile, sizeVal, 2);
        fontFile = reinterpret_cast<char *>(*g_00691b2c);
        reinterpret_cast<Font *>(self + 0x1580)->init(fontFile, sizeVal, 1);
    }
    if (*g_009b7b1c != 0x320) {
    ...
    RECT localRect;
    RectFn fn = reinterpret_cast<RectFn>(*g_00669328);
    fn(reinterpret_cast<int>(&localRect), 0x7af50c, 0x7af4ec);
    fn(reinterpret_cast<int>(&localRect), reinterpret_cast<int>(&localRect), 0x7af4fc);
    reinterpret_cast<GraphicWin *>(g_007ae820)->soft_update(&localRect);
}
