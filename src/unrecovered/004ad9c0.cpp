// ORIGINAL: 0x004AD9C0 ?close@SetupWin@@QAEXXZ 0x004AD9C0-0x004ADA7B FILE
// RULED-OUT: MISMATCH, closest at 188/187 bytes (mnemonic_similarity 0.92). SetupWin has no scaffold fields, so everything is raw offset arithmetic on `this`. The two "deleting-dtor-style" indirect calls (vtbl[1] read as a byte displacement, added to the object, then that adjusted pointer's OWN vtbl[0] called with arg 1 - a vbtable/virtual-base adjustment thunk, not a plain vcall) are expressed as a second `AdjVCall` shim distinct from the embedded manager's own `MgrVCall` shim. First call site (via an explicit `adjusted` local) reproduces byte-for-byte; the second (`adjusted2`, structurally identical source) still compiles to add+mov[ecx] where the original used lea+mov via a differently-addressed load - register-allocator nuance at the tail of the if-scope, not chased further. No flag set reproduces the original's exact push-ebp framed prologue AND register choice (esi=this, ebx=0) simultaneously; the frameless /O2 build won on mnemonic_similarity despite not matching the prologue shape.
