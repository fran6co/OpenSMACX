...
// subject: ??0PlanWin@@QAE@H@Z  at 0x0048BCD0  (227 bytes)
//
...
static int *const g_006581fb = (int *)0x006581FB;
static int *const g_0066d2a4 = (int *)0x0066D2A4;
static int *const g_0066d2ac = (int *)0x0066D2AC;
static int *const g_0066d414 = (int *)0x0066D414;
static int *const g_00677b30 = (int *)0x00677B30;

class PlanWin { public:
    PlanWin(int);
};
// Real ctor: an SEH frame (fs:[0]) protects a conditionally-constructed
// embedded GraphicWin at +0x22050 so the 0x6581d0/0x6581ed handlers can
// call GraphicWin's/MapWin's destructors on unwind. Neither destructor is
// declared on the scaffold's GraphicWin/MapWin/Buffer (ctor-only), so the
// frame is not reachable here - see RULED-OUT. Placement `new` reproduces
// the plain constructor calls; MapWin's `this` is `esi` directly (offset
// 0), so PlanWin is entered at MapWin's own base address, not through a
// declared C++ base (the scaffold's PlanWin carries no fields/bases).
inline void *operator new(unsigned int, void *p) { return p; }

PlanWin::PlanWin(int a1) {
    char *esi = reinterpret_cast<char *>(this);

    if (a1 != 0) {
        *reinterpret_cast<void **>(esi) = g_0066d414;
        new (esi + 0x22050) GraphicWin();
    }

    new (esi) MapWin(0);
    new (esi + 0x21a70) Buffer();

    int *vtbl = *reinterpret_cast<int **>(esi);
    int offset1 = *reinterpret_cast<int *>(reinterpret_cast<char *>(vtbl) + 4);
    *reinterpret_cast<void **>(esi + offset1) = g_0066d2ac;

    int *vtbl2 = *reinterpret_cast<int **>(esi);
    int offset1b = *reinterpret_cast<int *>(reinterpret_cast<char *>(vtbl2) + 4);
    *reinterpret_cast<void **>(esi + offset1b + 0x444) = g_0066d2a4;

    int *vtbl3 = *reinterpret_cast<int **>(esi);
    int offset1c = *reinterpret_cast<int *>(reinterpret_cast<char *>(vtbl3) + 4);
    *reinterpret_cast<int *>(esi + offset1c - 4) = offset1c - 0x22050;

    *reinterpret_cast<int *>(esi + 0x21a6c) = 0;
    *reinterpret_cast<int *>(esi + 0x21a68) = 0;
}
