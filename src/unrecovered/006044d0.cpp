...
// calls     0x00609A50
// indirect  0x0060455D 0x00604569 0x0060457B

// GENERATED SKELETON - tools/emit_translation_unit.py
// subject: ?double_click@BasePop@@QAEXH@Z  at 0x006044D0  (177 bytes)
//
...
class BasePop : public GraphicWin { public:
    ...
    void double_click(int);
};
void BasePop::double_click(int a1) {
    BasePop *pop = *reinterpret_cast<BasePop **>(g_009b8d7c);
    if (pop != 0) {
        char *base = reinterpret_cast<char *>(pop);
        int tab = *reinterpret_cast<int *>(base + 0x2350);
        unsigned int flags = pop->field_30A8_;
        if (tab != 1) {
            if (tab != -1) {
                int rel = *reinterpret_cast<int *>(base + 0x21d0);
                int off = *reinterpret_cast<int *>(rel + 8);
                char *addr = base + off;
                int flag2 = *reinterpret_cast<int *>(addr + 0x229c);
                if (flag2 != 0) {
                    if (tab == 1) {
                        int rel2 = *reinterpret_cast<int *>(base + 0x2228);
                        int off2 = *reinterpret_cast<int *>(rel2 + 8);
                        int val = *reinterpret_cast<int *>(base + off2 + 0x2314);
                        pop->field_A40_ = val;
                    } else {
                        Dialog *dlg = reinterpret_cast<Dialog *>(addr + 0x21d0);
                        pop->field_A40_ = dlg->get_selected_id();
                    }
                }
            }
            pop->field_3100_ = 0;
            if ((flags & 0x100) == 0) {
                if (pop->field_A14_ == 0) {
                    reinterpret_cast<VCall *>(pop)->slot058();
                } else {
                    reinterpret_cast<VCall *>(pop->field_A14_)->slot058();
                }
            }
            if ((flags & 0x4000) != 0) {
                reinterpret_cast<VCall *>(pop)->slot002();
            }
        }
    }
}
