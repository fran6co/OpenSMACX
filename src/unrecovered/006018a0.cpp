...
// subject: ?set_def_ok_text@BasePop@@QAAHPAD@Z  at 0x006018A0  (98 bytes)
//
...
class BasePop : public GraphicWin { public:
    ...
    int __cdecl set_def_ok_text(char *);
};
int __cdecl BasePop::set_def_ok_text(char * a1) {
    if (a1 == 0) {
        return 3;
    }
    char **buffer = reinterpret_cast<char **>(g_009b8d80);
    if (*buffer != 0) {
        free(*buffer);
        *buffer = 0;
    }
    unsigned int length = strlen(a1);
    *buffer = reinterpret_cast<char *>(mem_get(length + 1));
    if (*buffer == 0) {
        return 5;
    }
    **buffer = 0;
