...
// TRIED: if/else polarity flipped to match layout (jne first, matches); remaining MISMATCH is the compiler omitting the EBP frame that the original allocates (esp-relative &a2 suffices here), not reproduced by reordering statements
...
// calls     0x005FAB00 0x005FC680
// indirect  0x004035AA 0x004035B4 0x004035D0 0x004035F8 0x00403602

// GENERATED SKELETON - tools/emit_translation_unit.py
// subject: ?on_mouse_move@AlphaMenu@@QAEXHH@Z  at 0x00403550  (191 bytes)
//
...
// This body dispatches through slot(s): 4, 62, 63
class VCall { public:
...
void AlphaMenu::on_mouse_move(int a1, int a2) {
    char *self = reinterpret_cast<char *>(this);
    if (*reinterpret_cast<int *>(self + 0xa24) != -1) {
        reinterpret_cast<Menu *>(this)->on_mouse_move(a1, a2);
    } else {
        if (*g_009b7abc == 0) {
    ...
    int result = reinterpret_cast<Spot *>(self + 0xa2c)->check(a1, a2, &a2, 0);
    if (result == -1) {
        *g_00696e3c = -1;
