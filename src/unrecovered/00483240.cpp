...
// TRIED: do/while (empty body) instead of while - the inner `if` already proved the first iteration, so the loop itself is a do/while; `while` produced an extra redundant pre-test call (18B over)
// TRIED: duplicating `*g_00811d10=1;` into both sides of the count<1 check to match the store landing before `jl` in the original - regressed to an earlier divergence; left as single store (4B over, one instruction reordered around the branch)
