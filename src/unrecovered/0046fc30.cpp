// ORIGINAL: 0x0046FC30 FILE
// RULED-OUT: short-circuit && vs nested if vs hoisting the global count into
//            a local all reproduce the same divergence (#7 mov vs cmp) -
//            VC6 folds the `> 0` compare straight against the global memory
//            operand instead of loading it into a register first
// working copy - scaffold materialised by --work
// name      ?clear_alt@MapWin@@QAEXXZ
// size      75 bytes
// spans     0x0046FC30-0x0046FC7B
...
// subject: ?clear_alt@MapWin@@QAEXXZ  at 0x0046FC30  (75 bytes)
//
...
void MapWin::clear_alt() {
    if (owned_ != 0 && *g_00949884 > 0) {
        int row_offset = 0;
        int j = 0;
        do {
            int i = 0;
            do {
                ((uint8_t *)owned_)[row_offset + i + 4] = 0xFF;
                i++;
            } while (i < 5);
            j++;
            row_offset += 0xC;
