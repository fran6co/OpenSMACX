// ORIGINAL: 0x0052BE60 ?council_get_vote@@YAHHHH@Z 0x0052BE60-0x0052BF58 FILE
// RULED-OUT: `result < votes` for the loop-max compare (jle/jge polarity flip at #18) - flipped to `votes > result` to match, fixed. The surviving gap is a shared-epilogue layout choice, not semantics: the original falls through to `return a3` and jumps away to the shared tail for `return TableC[idx]`; every source order tried (plain if, inverted if, goto to a shared label) makes VC6 pick the opposite layout. Best reached is 96.3% mnemonic similarity (#32 je vs jne). 0x0096C9F8/0x0096CDE0/0x0096CE0C are indexed by `faction*2099+idx` (0x833 = 2099, matching spying_recovery.h's SpyingStatusTable stride at the same 0x0096C9F8 base) so they are `extern T name[]`, not the scaffold's folded `static T *const`, per the indexed-table-base lever.
