// ORIGINAL: 0x0052BE60 FILE
// RULED-OUT: `result < votes` for the loop-max compare (jle/jge polarity
//   flip at #18) - flipped to `votes > result` to match, fixed. The
//   surviving gap is a shared-epilogue layout choice, not semantics: the
//   original falls through to `return a3` and jumps away to the shared
//   tail for `return TableC[idx]`; every source order tried (plain if,
//   inverted if, goto to a shared label) makes VC6 pick the opposite
//   layout. Best reached is 96.3% mnemonic similarity (#32 je vs jne).
//   0x0096C9F8/0x0096CDE0/0x0096CE0C are indexed by `faction*2099+idx`
//   (0x833 = 2099, matching spying_recovery.h's SpyingStatusTable stride
//   at the same 0x0096C9F8 base) so they are `extern T name[]`, not the
//   scaffold's folded `static T *const`, per the indexed-table-base lever.
// working copy - scaffold materialised by --work
// name      ?council_get_vote@@YAHHHH@Z
// size      248 bytes
// spans     0x0052BE60-0x0052BF58
// prototype 
...
// subject: ?council_get_vote@@YAHHHH@Z  at 0x0052BE60  (248 bytes)
//
...
int __cdecl wants_prop(int, int, int);
...
extern int g_councilTableB[];        // 0x0096CDE0, [faction*2099 + proposal]
extern int g_councilTableC[];        // 0x0096CE0C, [faction*2099 + proposal]
extern unsigned int g_councilStatus[]; // 0x0096C9F8, [faction*2099 + proposal]

int __cdecl council_get_vote(int faction, int proposal, int a3) {
    unsigned int status;
    int result;
    ...
    if (proposal == 0) {
        ...
        int i = 1;
        do {
            int votes = wants_prop(faction, 0, i);
            if (votes > result) {
                result = votes;
                a3 = i;
            }
        ...
        int idx = faction * 2099;
        if (g_councilTableB[idx] == 0) {
            return a3;
        }
        result = g_councilTableC[idx];
        if ((g_councilStatus[idx + result] & 0x40c30) != 0) {
            return a3;
        }
    } else {
        result = wants_prop(faction, proposal, a3);
        int idx = faction * 2099;
        status = g_councilStatus[idx + a3];
        if ((status & 0x10) != 0) {
            --result;
        }
        if ((status & 0x20) != 0) {
        ...
        int direct = g_councilTableB[idx + proposal];
        if (direct != 0) {
            return direct;
        }
        if (result > 0) {
