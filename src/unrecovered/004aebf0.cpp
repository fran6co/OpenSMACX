...
// calls     0x00445130 0x004AF310 0x005B4210

// GENERATED SKELETON - tools/emit_translation_unit.py
// subject: ?UNK1@SocialWin@@QAEXHH@Z  at 0x004AEBF0  (226 bytes)
//
...
void social_calc(int *, int *, int, int, int);
...
void SocialWin::UNK1(int a1, int a2) {
    char *self = reinterpret_cast<char *>(this);
    field_A1C_ = 0xffffffff;
    *reinterpret_cast<int *>(self + 0xcf8 + a1 * 4) = a2;
    draw_social(a1);

    int *dst = reinterpret_cast<int *>(self + 0xd34);
    volatile int count = 11;
    do {
        *dst = dst[-11];
        dst = dst + 1;
        count = count - 1;
    ...
    social_calc(reinterpret_cast<int *>(self + 0xcf8),
                reinterpret_cast<int *>(self + 0xd08),
                factionID_, 0, 0);

    social_calc(reinterpret_cast<int *>(self + 0xcf8),
                reinterpret_cast<int *>(reinterpret_cast<char *>(g_0096cc2c) + factionID_ * 0x20cc),
                factionID_, 0, 0);

    energy_compute(factionID_, 1);

    *reinterpret_cast<int *>(self + 0xd64) = *g_00749ae4;
    *reinterpret_cast<int *>(self + 0xd60) = *g_00749c0c;

    int faction2 = factionID_;
    extern int g_96cc08[];
    social_calc(reinterpret_cast<int *>(reinterpret_cast<char *>(g_96cc08) + faction2 * 0x20cc),
                reinterpret_cast<int *>(reinterpret_cast<char *>(g_0096cc2c) + faction2 * 0x20cc),
                faction2, 0, 0);
}
