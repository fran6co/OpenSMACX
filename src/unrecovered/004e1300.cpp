...
// calls     0x0046B190 0x00523DD0 0x005ABF20

// GENERATED SKELETON - tools/emit_translation_unit.py
// subject: ?editor_clear_vehicles@Console@@QAEXXZ  at 0x004E1300  (127 bytes)
//
...
void Console::editor_clear_vehicles() {
    auto_undo();
    *g_009a64c0 |= 4;
    int count = 0;
    *g_009a64c8 = 0;
    ...
    *g_0093928c = -1;
    if (*g_00949884 > 0) {
        unsigned int *rec = reinterpret_cast<unsigned int *>(*g_0094a30c + 8);
        do {
            rec[0] &= 0xfffffffc;
            unsigned int val = rec[0];
            unsigned int *dst = rec + 2;
            for (int i = 0; i < 7; i++) {
                dst[i] = val;
            }
            count++;
