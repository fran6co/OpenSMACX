// ORIGINAL: 0x00493AF0 FILE
// RULED-OUT: `a1 > 0x3f` (jl/jle polarity mismatch) - fixed with `a1 >= 0x40`
//            to match the original's literal `cmp eax, 0x40; jl`. All 29
//            mnemonics now match; the only leftover diff is a trailing
//            2-byte `mov edi,edi` alignment pad after the switch's jump
//            table that this single-function unit picks up on its own.
// working copy - scaffold materialised by --work
// name      ?edbc_unit@ProdPicker@@QAEHH@Z
// size      90 bytes
// spans     0x00493AF0-0x00493B4A
...
// subject: ?edbc_unit@ProdPicker@@QAEHH@Z  at 0x00493AF0  (90 bytes)
//
...
static int *const g_00493b4c = (int *)0x00493B4C;
...
static uint8_t *const g_flags_00493af0 = (uint8_t *)0x009AB898;
static uint8_t *const g_kind_00493af0 = (uint8_t *)0x009AB892;

int ProdPicker::edbc_unit(int a1) {
    if (a1 >= 0x40 && (g_flags_00493af0[a1 * 0x34] & 4) == 0) {
        return 1;
    }
    switch (g_kind_00493af0[a1 * 0x34]) {
        case 3:
        case 7:
        case 8:
            return 0;
