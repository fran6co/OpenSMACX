...
extern "C" int __cdecl my_isdigit(int);
extern "C" char *__cdecl my_itoa(int, char *, int);

void __cdecl yearmotize(char * a1) {
    char *p = a1;
    if (*p == 0) return;
    char c;
    do {
        p++;
        char *cur = p - 1;
        if (strlen(cur) < 4) return;
        if (my_isdigit((unsigned char)cur[0]) && my_isdigit((unsigned char)cur[1]) &&
            my_isdigit((unsigned char)cur[2]) && my_isdigit((unsigned char)cur[3]) &&
            !my_isdigit((unsigned char)cur[4]) &&
            (cur == a1 || !my_isdigit((unsigned char)cur[-1]))) {
            char buf[80];
            char year_str[80];
            buf[0] = 0;
            int year = game_year(*g_009a64d4);
            my_itoa(year, year_str, 10);
            strcat(buf, year_str);
            cur[0] = buf[0];
            cur[1] = buf[1];
            cur[2] = buf[2];
