...
// TRIED: calling sub_628150 through a cast function pointer (its scaffold prototype is nullary) turns the direct `call rel32` into a loaded/indirect call; the loop body and field offsets (record_->0x24 loop count, record_->0x20 divisor, the index*3+0x1d4 table read, the 0x10ac/0x10c/0x110 tri-tables) match the disassembly, but the prologue register allocation (push/sub order) diverges early. Landing the closest MISMATCH.
// size      286 bytes
