...
//            sets `ecx = esi` (this=a2) before every call - a free
//            __thiscall declaration is C4234, so it is reached through the
//            Dummy member-function-pointer trick. The four mmio* Windows
//            multimedia calls go through fixed-address function-pointer
//            globals rather than by name, since the catalogue does not
//            model them. Landing the closest control-flow-faithful form
//            (divergence starts at instruction #6, on the mmio-open guard).
