...
// calls     0x00609470 0x00609F20 0x0060D1B0 0x0060E7C0 0x00610480 0x00611A90

// GENERATED SKELETON - tools/emit_translation_unit.py
// subject: ?init@Dialogs@@QAEHPAURECT@@H@Z  at 0x00613400  (145 bytes)
//
...
int Dialogs::init(RECT * a1, int a2) {
    char *self = reinterpret_cast<char *>(this);

    RadioButton *rb = reinterpret_cast<RadioButton *>(self + 0x44);
    rb->close();
    int *rbBase = *reinterpret_cast<int **>(self + 0x44);
    int rbOff = *reinterpret_cast<int *>(reinterpret_cast<char *>(rbBase) + 8);
    Dialog *d1 = reinterpret_cast<Dialog *>(self + 0x44 + rbOff);
    int r1 = d1->init(a1, a2);
    if (r1 != 0) {
        return r1;
    }

    ListBox *lb = reinterpret_cast<ListBox *>(self);
    lb->close();
    int *lbBase = *reinterpret_cast<int **>(self);
    int lbOff = *reinterpret_cast<int *>(reinterpret_cast<char *>(lbBase) + 8);
    Dialog *d2 = reinterpret_cast<Dialog *>(self + lbOff);
    int r2 = d2->init(a1, a2);
    if (r2 != 0) {
        return r2;
    }

    CheckBox *cb = reinterpret_cast<CheckBox *>(self + 0x58);
    cb->close();
    int *cbBase = *reinterpret_cast<int **>(self + 0x58);
    int cbOff = *reinterpret_cast<int *>(reinterpret_cast<char *>(cbBase) + 8);
    Dialog *d3 = reinterpret_cast<Dialog *>(self + 0x58 + cbOff);
    int r3 = d3->init(a1, a2);
    if (r3 != 0) {
        return r3;
    }

    SpriteBox *sb = reinterpret_cast<SpriteBox *>(self + 0x70);
    int r4 = sb->init(a1, a2);
    if (r4 != 0) {
        return r4;
    }

    EditGroup *eg = reinterpret_cast<EditGroup *>(self + 0xf8);
    eg->close();
    int *egBase = *reinterpret_cast<int **>(self + 0xf8);
    int egOff = *reinterpret_cast<int *>(reinterpret_cast<char *>(egBase) + 8);
    Dialog *d4 = reinterpret_cast<Dialog *>(self + 0xf8 + egOff);
    return d4->init(a1, a2);
}
