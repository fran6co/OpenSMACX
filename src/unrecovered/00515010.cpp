...
// calls     0x00593340 0x005BEB70 0x005BF310 0x00625E30 0x00625E50

// GENERATED SKELETON - tools/emit_translation_unit.py
// subject: ?set_tech@Console@@QAEXH@Z  at 0x00515010  (178 bytes)
//
...
int tech_pick(int, int, int, char *);
void synch_research(int);

// ---- fixed globals this body references ----
// The const-pointer spelling reproduces the original's
// encoding including the address; `extern T *g` does not.
static int *const g_0068a8d4 = (int *)0x0068A8D4;
...
void Console::set_tech(int a1) {
    int idx = a1 * 0x20cc;
    int *cost = (int *)((char *)0x0096CDA8 + idx);
    int *techId = (int *)((char *)0x0096CDAC + idx);
    if (*cost != 0 && *techId >= 0) {
        int techVal = g_0094f35c[*techId * 11];
        parse_say(0, techVal, -1, -1);
        parse_num(0, *cost);
        if (X_pop((const char *)g_0068a8d4, 0) == 0) {
            return;
        }
    }
    int picked = tech_pick(a1, 0, -1, 0);
    if (picked >= 0) {
        if (*techId >= 0 && picked != *techId) {
            *cost = *cost / 2;
        }
        *techId = picked;
    }
