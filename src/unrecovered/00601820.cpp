...
// subject: ?set_cancel_text@BasePop@@QAEHPAD@Z  at 0x00601820  (113 bytes)
//
...
// `strcat`/`strlen` are the fast intrinsics MSVC recognises by name and
// inlines under /O2; the original calls out to both. Reach the real
...
int BasePop::set_cancel_text(char * a1) {
    if (a1 == 0) {
        return 3;
    }
    if (cancel_text_ != 0) {
    ...
    cancel_text_ = reinterpret_cast<LPSTR>(mem_get(reinterpret_cast<StrlenFn>(_strlen)(a1) + 1));
    if (cancel_text_ == 0) {
        return 5;
    }
    *cancel_text_ = 0;
    reinterpret_cast<StrcatFn>(_strcat)(cancel_text_, a1);
    return 0;
}
