...
// indirect  0x005FB852 0x005FB86D 0x005FB897 0x005FB8A9 0x005FB8C6 0x005FB8D1

// GENERATED SKELETON - tools/emit_translation_unit.py
// subject: ?menu_event@Menu@@QAEHH@Z  at 0x005FB830  (168 bytes)
//
...
// Second vtable shim: slot023 returns int (its result is tested); the
// file's own VCall types every slot void(). Declaration order pins the
// offset, so this carries 0..23 in order even though only 23 differs.
class VCall2 { public:
...
typedef void (__cdecl *MenuProcFn)(int);

int Menu::menu_event(int a1) {
    unsigned int selected = field_A24_;
    if (selected != 0xffffffff) {
        MenuEntry *entry = &entries_[selected];
        reinterpret_cast<VCall *>(entry->pull_down)->slot002();
        field_A24_ = 0xffffffff;
        *g_00696e3c = -1;
        reinterpret_cast<VCall *>(this)->slot062();
        flush_input();
        do_all_tasks(reinterpret_cast<Win *>(entry->pull_down));
    ...
    if (proc_ != 0) {
        reinterpret_cast<MenuProcFn>(proc_)(a1);

        Win *parent = win_parent_;
        if (parent != 0) {
            void *sub = *reinterpret_cast<void **>(reinterpret_cast<char *>(parent) + 0x88);
            if (sub != 0) {
                if (reinterpret_cast<VCall2 *>(sub)->slot023() == 0) {
                    reinterpret_cast<VCall *>(sub)->slot007();
                }
            }
        }
    }
    return 0;  // eax is never set on any path in the disassembly; the
               // catalogue's `int` return forces this extra `xor eax,eax`
}
