...
// TRIED: the plain `int*const` globals g_009392c0/g_00939340 for the index-scaled table reads (address does work via `[eax*4+..]`) - own `extern int[]` names keep the indexed addressing mode. MISMATCH #1 'push' vs 'mov' remains open.
...
// subject: ?draw_cursor@MapWin@@QAEXXZ  at 0x0046AD60  (145 bytes)
//
...
extern int g_table_9392c0[];
extern int g_table_939340[];

void MapWin::draw_cursor() {
    int idx = *g_009392b8;
    int x = g_table_9392c0[idx];
    int y = g_table_939340[idx];

    if (y < (int)field_1DDA8_) return;
    if (y >= (int)(field_1DDD8_ + field_1DDD0_ + field_1DDA8_)) return;

    int adjX = x;
    if (!(*g_0094988c & 1)) {
        if (x < (int)field_1DDA4_) {
            adjX = *g_00949870 + x;
        }
        if (adjX >= (int)(field_1DDD4_ + field_1DDCC_ + field_1DDA4_)) {
            adjX = adjX - *g_00949870;
        }
    }

    if (adjX < (int)field_1DDA4_) return;
    if (adjX >= (int)(field_1DDD4_ + field_1DDCC_ + field_1DDA4_)) return;
