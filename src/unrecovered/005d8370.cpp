// ORIGINAL: 0x005D8370
// name      ?draw@Buffer@@QAEHPAVBuffer@@HHH@Z
// size      327 bytes
// spans     0x005D8370-0x005D84B7
...
// RULED-OUT: routing the slot-0x74/0x14/0x80 calls through the shared
//            VCall shim (thiscall receiver) - the raw disassembly pushes
//            the object pointer as an EXPLICIT stack argument (`push eax`)
//            with no matching `mov ecx,...` setting a receiver, and ecx at
//            call time already holds the vtable pointer itself rather
//            than the object, so these are plain cdecl function-pointer
//            calls that happen to take the object pointer as their own
//            first argument, not virtual dispatch through an introduced
//            class - matches Ghidra's own rendering exactly, which shows
//            no separate `this`.
//            Best reached: MISMATCH, edit_count first at #0 (prologue
//            reserves the two locals via push instead of `sub esp,8`),
//            309 bytes vs 327, mnemonic similarity ~0.88.
