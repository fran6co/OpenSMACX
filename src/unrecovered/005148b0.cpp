// ORIGINAL: 0x005148B0 ?move_cursor@Console@@QAEXH@Z 0x005148B0-0x00514990 FILE
// RULED-OUT: `g_66ef50`/`g_66ef74`/`g_9392c0`/`g_939340` as the scaffold's `int *const` globals - the address does work (index by `a1` or by `*g_009392b8`), so switched to `extern int name[]`. Landed at 89.8% similarity: the final bounds check needs a zero register for the `newX >= 0` test, and this source's compiler hoists that zero and reuses it for the earlier wrap check too (`cmp esi,edx` where the original has a plain `test eax,eax`/`jge` sign test there) - tried reordering the wrap-adjustment operands to match Ghidra's `width + iVar1` form exactly; the CSE persisted regardless.
