// ORIGINAL: 0x005148B0 FILE
// RULED-OUT: `g_66ef50`/`g_66ef74`/`g_9392c0`/`g_939340` as the scaffold's
//   `int *const` globals - the address does work (index by `a1` or by
//   `*g_009392b8`), so switched to `extern int name[]`. Landed at 89.8%
//   similarity: the final bounds check needs a zero register for the
//   `newX >= 0` test, and this source's compiler hoists that zero and
//   reuses it for the earlier wrap check too (`cmp esi,edx` where the
//   original has a plain `test eax,eax`/`jge` sign test there) - tried
//   reordering the wrap-adjustment operands to match Ghidra's
//   `width + iVar1` form exactly; the CSE persisted regardless.
// working copy - scaffold materialised by --work
// name      ?move_cursor@Console@@QAEXH@Z
// size      224 bytes
// spans     0x005148B0-0x00514990
// prototype void (__thiscall ?move_cursor@Console@@QAEXH@Z)(Console* this, int)
// callers   0   call targets   5
// kind      game
// flags     frame;hidden;sp_ready;purged_ok
// calls     0x0046A550 0x0046AD60 0x0046B310 0x004B9EA0 0x005109B0

// GENERATED SKELETON - tools/emit_translation_unit.py
// subject: ?move_cursor@Console@@QAEXH@Z  at 0x005148B0  (224 bytes)
//
...
void Console::move_cursor(int a1) {
    extern int g_66ef50[];
    extern int g_66ef74[];
    extern int g_9392c0[];
    extern int g_939340[];

    int player = *g_009392b8;
    int newX = g_66ef50[a1] + g_9392c0[player];
    if (!(*reinterpret_cast<unsigned char *>(g_0094988c) & 1)) {
        if (newX < 0) {
            newX = *g_00949870 + newX;
        } else if (newX >= *g_00949870) {
            newX = newX - *g_00949870;
        }
    }
    int newY = g_66ef74[a1] + g_939340[player];
    if (newY >= 0 && newY < *g_00949874 && newX >= 0 && newX < *g_00949870) {
        field_23BF8_ = 0;
        if (field_23C00_ != 0) {
            field_23C00_ = 0;
            reinterpret_cast<MapWin *>(0x7d3c3c)->draw_map(1);
        }
        reinterpret_cast<MapWin *>(0x7d3c3c)->draw_cursor();
        cursor_next(newX, newY);
        reinterpret_cast<StatusWin *>(0x8c5568)->redraw();
        field_23BF8_ = 1;
        field_23BFC_ = 1;
        reinterpret_cast<MapWin *>(0x7d3c3c)->draw_cursor();
        reinterpret_cast<MapWin *>(0x7d3c3c)->focus(newX, newY);
    }
}
