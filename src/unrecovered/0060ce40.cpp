// ORIGINAL: 0x0060CE40 sub_60ce40 0x0060CE40-0x0060CE48;0x0060CE50-0x0060CECF FILE
...
// subject: sub_60ce40  at 0x0060CE40  (135 bytes)
//
...
// This row's own catalogued span is only the 8-byte thunk prologue
// (`sub ecx, [ecx-4]; jmp`); the remaining bytes are a SHARED_TAIL claimed
// by the sibling thunks at 0x60D010/0x60D030 (COMDAT-folded), so no verdict
// past SHARED_TAIL is well defined for this address regardless of body.
class NCListBoxDtorShim60ce40 { public:
    int shared_tail(unsigned char);
};

class NC60ce40 { public:
    int destroy(unsigned char a2);
};

int NC60ce40::destroy(unsigned char a2) {
    char *self = reinterpret_cast<char *>(this);
    char *adjusted = self - *reinterpret_cast<int *>(self - 4);
    return reinterpret_cast<NCListBoxDtorShim60ce40 *>(adjusted)->shared_tail(a2);
}
