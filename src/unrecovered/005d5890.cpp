...
// RULED-OUT: RECT r = buffer_.rect2_; then left/right += dx, top/bottom += dy
//   after client_to_screen - MISMATCH #33 mov/add, deep into the field-add
//   sequence, not the branch shape.
...
// subject: ?soft_update@GraphicWin@@QAEXXZ  at 0x005D5890  (157 bytes)
//
...
void GraphicWin::soft_update() {
    typedef int (__stdcall *InvalidateRectFn)(int, RECT *, int);
    if (is_visible() != 0) {
        int dx = 0, dy = 0;
        RECT r = buffer_.rect2_;
        client_to_screen(&dx, &dy);
        r.left += dx;
        r.right += dx;
        r.top += dy;
        r.bottom += dy;
        ((InvalidateRectFn)*g_00669304)(*g_009b7b28, &r, 0);
    }
