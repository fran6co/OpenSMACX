// ORIGINAL: 0x005F9480 FILE
// RULED-OUT: field_A14_/items_ as named in the scaffold do NOT sit at the
//            disasm's 0x474/0xA24 (raw self+offset used instead, per the
//            0xA14-lower-than-actual caution). PtInRect(RECT*, long, long)
//            instead of PtInRect(RECT*, POINT) diverges one instruction
//            earlier (prologue reserves stack differently either way).
// working copy - scaffold materialised by --work
// name      ?on_left_up@PullDown@@QAEXHH@Z
// size      227 bytes
// spans     0x005F9480-0x005F9563
// prototype void (__thiscall ?on_left_up@PullDown@@QAEXHH@Z)(PullDown* this, int, int)
// callers   0   call targets   0
// kind      game
// flags     sp_ready;purged_ok
// calls     (none)
// indirect  0x005F94B7 0x005F9546 0x005F9558

// GENERATED SKELETON - tools/emit_translation_unit.py
// subject: ?on_left_up@PullDown@@QAEXHH@Z  at 0x005F9480  (227 bytes)
//
...
static int *const g_006692cc = (int *)0x006692CC;
...
extern "C" int __stdcall PtInRect(const RECT *, POINT);

void PullDown::on_left_up(int a1, int a2) {
    ...
    if (PtInRect(&rect, pt) != 0) {
        int *font = reinterpret_cast<int *>(*g_009b7b68);
        int lineH;
        if (*font < 0) {
            lineH = font[3];
        } else {
        ...
        int closest = -1;
        int count = *reinterpret_cast<int *>(self + 0xF20);
        *reinterpret_cast<int *>(self + 0xF28) = (a2 - *g_00696e1c) / lineH;
        int index = 0;
        if (count > 0) {
            unsigned char *item = reinterpret_cast<unsigned char *>(self + 0xA24);
            do {
                if ((*item & 1) != 0) {
                    closest++;
                }
                if (closest == *reinterpret_cast<int *>(self + 0xF28)) {
                    break;
                }
                index++;
                item += 0x14;
        ...
        if (index == count) {
            *reinterpret_cast<int *>(self + 0xF28) = -1;
        } else {
            *reinterpret_cast<int *>(self + 0xF28) = index;
            if ((*reinterpret_cast<unsigned char *>(self + 0xA24 + index * 0x14) & 4) != 0) {
                *reinterpret_cast<int *>(self + 0xF28) = -1;
            }
        }
        if (*reinterpret_cast<int *>(self + 0xF28) == -1) {
