// ORIGINAL: 0x0060FED0 FILE
// RULED-OUT: signature stdcall(int) -> thiscall member(unsigned int flag),
//            returning void*; same body as 0x0060FD80 (this thunk only differs
//            in the leading adjustor, `sub ecx,[ecx-4]; sub ecx,0xa18` vs
//            `sub ecx,[ecx-4]`), which is why it is COMDAT-folded with it.
//            Vtordisp thunk not modeled; see 0x0060FD80 for the reasoning.
// working copy - scaffold materialised by --work
// name      sub_60fed0
// size      190 bytes
// spans     0x0060FED0-0x0060FEDE;0x0060FD90-0x0060FE40
// prototype 
...
// subject: sub_60fed0  at 0x0060FED0  (190 bytes)
//
...
class Sub60fed0Base { public:
    void *dtor(unsigned int flag);
};

void *Sub60fed0Base::dtor(unsigned int flag) {
    char *esi = (char *)this;
    char *edi = esi + 0x1c;

    int vptrA = *(int *)esi;
    int offA = *(int *)(vptrA + 4);
    *(int *)(esi + offA) = (int)g_006705b0;

    int vptrB = *(int *)esi;
    int offB = *(int *)(vptrB + 4);
    *(int *)(offB + (int)edi + 0x428) = (int)g_006705a8;

    int vptrC = *(int *)esi;
    int offC = *(int *)(vptrC + 8);
    *(int *)(esi + offC) = (int)g_0067059c;

    int vptrD = *(int *)esi;
    int offD = *(int *)(vptrD + 4);
    *(int *)(offD + (int)edi - 0x20) = offD - 0x1c;

    int vptrE = *(int *)esi;
    int offE = *(int *)(vptrE + 8);
    *(int *)(offE + (int)edi - 0x20) = offE - 0xa34;

    int vptrF = *(int *)esi;
    *(int *)(esi + 4) = 0;
    *(int *)(esi + 8) = 0;
    *(int *)(esi + 0xc) = 0;
    *(int *)(esi + 0x14) = *g_00697108;
    *(int *)(esi + 0x10) = *g_00697104;

    int closeOffDialog = *(int *)(vptrF + 8);
    ((Dialog *)(esi + closeOffDialog))->close();

    int vptrG = *(int *)esi;
    int closeOffGfx = *(int *)(vptrG + 4);
    ((GraphicWin *)(esi + closeOffGfx))->close();

    ((Dialog *)(esi + 0xa34))->~Dialog();
    ((GraphicWin *)edi)->~GraphicWin();

    if (flag & 1) {
        typedef void (__cdecl *DeleteFn)(void *);
        ((DeleteFn)0x0064557F)(esi);
    }
    return (void *)esi;
}
