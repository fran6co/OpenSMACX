...
// calls     0x004863F0 0x005D5890
// indirect  0x00486063

// GENERATED SKELETON - tools/emit_translation_unit.py
// subject: ?on_mouse_move@PickTech@@QAEXHH@Z  at 0x00485FC0  (230 bytes)
//
...
void PickTech::on_mouse_move(int a1, int a2) {
    char *self = reinterpret_cast<char *>(this);
    int y = a2 - 0x1d;
    if (a1 < 0 || y < 0 ||
        a1 >= *reinterpret_cast<int *>(self + 0xa24) * (*reinterpret_cast<int *>(self + 0xa14) + 1) ||
        y >= *reinterpret_cast<int *>(self + 0xa28) * (*reinterpret_cast<int *>(self + 0xa18) + 1)) {
        int cur = *reinterpret_cast<int *>(self + 0xa38);
        if (cur >= 0) {
            draw_item(cur, 0);
            reinterpret_cast<GraphicWin *>(this)->soft_update();
        }
        *reinterpret_cast<int *>(self + 0xa3c) = 0;
        *reinterpret_cast<int *>(self + 0xa38) = -1;
    } else {
        int index = (y / (*reinterpret_cast<int *>(self + 0xa18) + 1)) * *reinterpret_cast<int *>(self + 0xa24)
                    + a1 / (*reinterpret_cast<int *>(self + 0xa14) + 1);
        if (index != *reinterpret_cast<int *>(self + 0xa38) || *reinterpret_cast<int *>(self + 0xa3c) == 0) {
            int flag = draw_item(index, 1);
            *reinterpret_cast<int *>(self + 0xa3c) = flag;
            int prev = *reinterpret_cast<int *>(self + 0xa38);
            if (prev >= 0 && prev != index) {
                draw_item(prev, 0);
            }
            *reinterpret_cast<int *>(self + 0xa38) = index;
