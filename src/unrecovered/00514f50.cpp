...
// calls     0x00593310 0x005BF3F0

// GENERATED SKELETON - tools/emit_translation_unit.py
// subject: ?set_ai@Console@@QAEXH@Z  at 0x00514F50  (191 bytes)
//
...
extern char g_096cdc0[];

void Console::set_ai(int a1) {
    int offset = a1 * 0x20CC;
    int *entry = reinterpret_cast<int *>(g_096cdc0 + offset);
    unsigned char flags = (entry[0] != 0) ? 1 : 0;
    if (entry[1] != 0) {
        flags |= 2;
    } else {
        flags &= 0xfd;
    ...
    *reinterpret_cast<unsigned char *>(g_009bc06c) = flags;
    if (X_pop(reinterpret_cast<const char *>(g_0068a8c8), 1, 0) >= 0) {
        char c = *reinterpret_cast<char *>(g_009bc06c);
        entry[0] = c & 1;
        entry[1] = (c & 2) >> 1;
        entry[2] = (c & 4) >> 2;
        entry[3] = (c & 8) >> 3;
