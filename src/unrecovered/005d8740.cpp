...
// RULED-OUT: register-copy-before-store for the two lock-count decrements
//   (`int c=X-1; store; if(c>0)`) matches Ghidra's shape but VC6 collapses
//   the redundant `mov eax,ecx` copy the original keeps for the NULL-surface
//   arm; best reached is 97.5% mnemonic similarity, diverging only in that
//   micro reg-alloc order (#41 mov vs cmp). __fastcall(self, unused-edx,
//   7 stack ints) instead of the given __stdcall(a1..a7): `mov esi, ecx`
//   with no matching stack slot is a receiver, not a stack arg.
...
// subject: sub_5d8740  at 0x005D8740  (248 bytes)
//
...
class Sprite { public:
    ...
    int iTopOffset_;
    int fObj1Exists_;
    int UNK1(int, int, int, int, int, int, int);
    void close();
};
...
extern "C" int __fastcall sub_5d8740(void *self, void * /*edx*/,
                                     int a1, int a2, int a3, int a4,
                                     int a5, int a6, int a7) {
    char *pSelf = reinterpret_cast<char *>(self);
    void **pSurface    = reinterpret_cast<void **>(pSelf + 0x58);
    int   *pCachedBits = reinterpret_cast<int *>(pSelf + 0x54);
    int   *pField50    = reinterpret_cast<int *>(pSelf + 0x50);
    int   *pLockCount  = reinterpret_cast<int *>(pSelf + 0x6C);
    int   *pField4A8   = reinterpret_cast<int *>(pSelf + 0x4A8);
    int   *pWidth      = reinterpret_cast<int *>(pSelf + 0x80);
    int   *pHeight     = reinterpret_cast<int *>(pSelf + 0x84);

    int transparentIndex = a2;
    if (*pSurface == 0) {
        if (transparentIndex == -1) {
            transparentIndex = *reinterpret_cast<uint8_t *>(g_00696d14);
        }

        int bits = *pCachedBits;
        *pField50 = bits;
        if (bits != 0) {
            ++*pLockCount;
        }
        *g_009b3a54 = bits;
        *g_009b3a5c = *pField4A8;
        *g_009b3a60 = *pWidth;
        *g_009b3a64 = -*pHeight;
        *g_009b3a68 = *pWidth;
        *g_009b3a6c = -*pHeight;
        *g_009b3a70 = 0;
        *g_009b3a74 = 0;
        *reinterpret_cast<char *>(g_009b3a58) = (char)transparentIndex;
        *g_009b3a78 = 0;
...
forward:
    {
        int result = reinterpret_cast<Sprite *>(g_009b3a50)->UNK1(a1, a2, a3, a4, a5, a6, a7);
        *g_009b3a54 = 0;
        return result;
    }
}
