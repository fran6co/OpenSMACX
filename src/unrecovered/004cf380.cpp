// ORIGINAL: 0x004CF380 FILE
// RULED-OUT: veh_lift's catalogue signature is void __cdecl(int), but the
//   call site immediately `push eax` with no reload before veh_drop, so eax
//   must be its post-call value; reproduced via a reinterpret_cast through a
//   local `int(__cdecl*)(int)` function-pointer type (redeclaring veh_lift
//   itself conflicts with the scaffold's own decl, C2556). Best reached is
//   86.6% mnemonic similarity: flags say `frame` (push ebp; mov ebp,esp) but
//   this body compiles frameless under /O2 - a spill-count difference, not
//   a control-flow one.
// working copy - scaffold materialised by --work
// name      ?action_gate@@YAXHH@Z
// size      247 bytes
// spans     0x004CF380-0x004CF477
// prototype void (__cdecl ?action_gate@@YAXHH@Z)(int vehID, int baseID)
// callers   3   call targets   5
// kind      game
// flags     frame;sp_ready;purged_ok
// calls     0x0046AF40 0x004E3A50 0x005108A0 0x005BFFA0 0x005C0080

// GENERATED SKELETON - tools/emit_translation_unit.py
// subject: ?action_gate@@YAXHH@Z  at 0x004CF380  (247 bytes)
//
...
void __cdecl veh_drop(int, int, int);
...
extern char Vehs[][0x34];
extern char Bases[][0x134];

void __cdecl action_gate(int veh_id, int base_id) {
    typedef int(__cdecl * VehLiftIntFn)(int);
    short vehX = *reinterpret_cast<short *>(Vehs[veh_id] + 0);
    short vehY = *reinterpret_cast<short *>(Vehs[veh_id] + 2);
    int gateBase = base_at(vehX, vehY);
    if (gateBase >= 0) {
        *reinterpret_cast<int *>(Bases[gateBase] + 0x30) |= 0x20000000;
    }
    *reinterpret_cast<int *>(Bases[base_id] + 0x30) |= 0x20000000;

    int lifted = reinterpret_cast<VehLiftIntFn>(&veh_lift)(veh_id);
    veh_drop(lifted, *reinterpret_cast<short *>(Bases[base_id] + 0),
             *reinterpret_cast<short *>(Bases[base_id] + 2));

    if (gateBase >= 0) {
        draw_tile(*reinterpret_cast<short *>(Bases[gateBase] + 0),
                  *reinterpret_cast<short *>(Bases[gateBase] + 2), 2);
    }
    draw_tile(*reinterpret_cast<short *>(Bases[base_id] + 0),
              *reinterpret_cast<short *>(Bases[base_id] + 2), 2);

    if (veh_id == *g_0093928c) {
        unsigned char vehFaction = *reinterpret_cast<unsigned char *>(Vehs[veh_id] + 0xE);
        reinterpret_cast<Console *>(g_009156b0)->focus(vehX, vehY, vehFaction);
    }
}
