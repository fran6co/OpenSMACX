// ORIGINAL: 0x00529E70 FILE
// RULED-OUT: byte-exact - 94.5% mnemonic similarity; only remaining
//            divergence is instruction-scheduling order of the arg-0 push
//            for set_view() vs the two independent global stores before it
//            (tried statement order and a cached Console* local, no change).
// working copy - scaffold materialised by --work
// name      ?net_not_my_turn@@YAXXZ
// size      188 bytes
// spans     0x00529E70-0x00529F2C
// prototype 
// callers   0   call targets   4
// kind      game
// flags     hidden;sp_ready;purged_ok
// calls     0x00479330 0x00511480 0x0052DBF0 0x00530360

// GENERATED SKELETON - tools/emit_translation_unit.py
// subject: ?net_not_my_turn@@YAXXZ  at 0x00529E70  (188 bytes)
//
...
void __cdecl net_not_my_turn() {
    ...
    *g_0093e8ec = *g_0093e8ec | (1 << *g_00939284);
    while (*g_009b2068 == 0 && *g_0093a950 == 0 && *g_009a6820 != *g_00939284) {
        *g_00939294 = 1;
        reinterpret_cast<NetDaemon *>(g_0093cd90)->net_tasks();
        if (*g_0093e8c0 != 0) {
            int level = *g_009a6820;
            if (level <= 0) {
                next_player_turn();
            } else {
                unsigned char bit = static_cast<unsigned char>(1 << level);
                if ((*reinterpret_cast<unsigned char *>(g_009a64e8) & bit) == 0) {
                    next_player_turn();
                } else if ((*reinterpret_cast<unsigned char *>(g_009a64e9) & bit) == 0) {
                    next_player_turn();
                }
            }
        }
