// ORIGINAL: 0x00477640 FILE
// name      ?mon_native_life_bred@@YAXH@Z
// size      254 bytes
// spans     0x00477640-0x0047773E
// prototype 
...
// subject: ?mon_native_life_bred@@YAXH@Z  at 0x00477640  (254 bytes)
//
...
static int *const g_0094cc48 = (int *)0x0094CC48;
static int *const g_0094cc4c = (int *)0x0094CC4C;
static int *const g_0094f3e8 = (int *)0x0094F3E8;
static int *const g_0094f3ec = (int *)0x0094F3EC;
static int *const g_009a64d4 = (int *)0x009A64D4;

struct MonRecord {
    int year;
    unsigned char pad_04[0xC - 0x4];
    int owner;
    int active;
    int count;
    unsigned char pad_18[0x268 - 0x18];
    int state;
    unsigned char pad_end[0x4F4 - 0x268 - 4];
};

void __cdecl mon_native_life_bred(int a1) {
    int year = game_year(*g_009a64d4);
    unsigned char flag = *(reinterpret_cast<unsigned char *>(g_00946f58) + a1 * 0x59c);
    if ((flag & 0x80) == 0) {
        MonRecord *table = reinterpret_cast<MonRecord *>(0x0094CC38);
        if (table[a1].active == 0) {
            table[a1].count = 0;
            for (int i = 0; i < 8; i++) {
                if (i != a1 && table[i].active != 0) {
                    table[a1].count++;
                }
            }
            int builder = a1;
            if (table[a1].count == 0) {
                ...
                table[a1].count = 0;
                for (int j = 0; j < 8; j++) {
                    if (j != a1 && table[j].active != 0 && table[j].count != 0) {
                        builder = j;
                    }
                }
            }
            table[a1].active = 1;
            table[a1].owner = builder;
            table[a1].year = year;
