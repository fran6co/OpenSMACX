...
// RULED-OUT: for(;;){...if(p>=end)return;} vs do/while(p<end) vs goto-found
//            form - all three land the same `jae + jmp` where the original
//            has a single `jl` (140B vs 141B, one instruction over)
// working copy - scaffold materialised by --work
// name      sub_470fa0
// size      141 bytes
// spans     0x00470FA0-0x0047102D
// prototype 
// callers   0   call targets   2
// kind      game
// flags     frame;sp_ready;purged_ok
// calls     0x00462870 0x00470A90
// indirect  0x00471027

// GENERATED SKELETON - tools/emit_translation_unit.py
// subject: sub_470fa0  at 0x00470FA0  (141 bytes)
//
...
extern "C" void __cdecl sub_470fa0(int a1, int a2) {
    MapWin **table = reinterpret_cast<MapWin **>(g_007d3c3c);
    MapWin **p = reinterpret_cast<MapWin **>(g_007d3c40);
    MapWin **end = reinterpret_cast<MapWin **>(g_007d3c5c);
    int idx = 1;
    for (;;) {
        MapWin *mw = *p;
        if (mw != 0 && mw->field_1DD74_ == 0) {
            break;
        }
        ++p;
        ++idx;
    ...
    MapWin *mw = table[idx];
    mw->clear(mw->field_1DD78_);
    table[idx]->init(0, 1);
    table[idx]->field_1DD9C_ = a1;
    table[idx]->field_1DDA0_ = a2;
    char *bytes = reinterpret_cast<char *>(table[idx]);
    int disp = (*reinterpret_cast<int **>(bytes))[1];
    reinterpret_cast<VCall2 *>(bytes + disp)->slot001(0);
}
