// ORIGINAL: 0x005DAB90 sub_5dab90 0x005DAB90-0x005DAC65 FILE
// RULED-OUT: this is a real __thiscall Buffer member (ecx=this pervasively); slot001 (self+4) is a genuine virtual thiscall dispatch via the scaffold's VCall class, but slots at offset 0x44/0x68 on the secondary object (field_58_) push that object as an explicit stack arg while ecx holds the vtable pointer - not a real thiscall, so those two use raw `int**`/function-pointer casts instead of VCall. field_50_/0x58/0x60/0x64/0x68/0x50C offsets match Buffer's known layout from the wrap_cent brief.
// RULED-OUT: tracking a single `iVar3` local mirroring ghidra's variable (should instead re-read *piVar1 fresh at the "iVar3 != 0" gate, since the original reloads memory there); several statement-order and branch-order permutations for the piVar2==0 vs *piVar1==0 arms, none reached better than ~0.81 mnemonic similarity.
