...
// calls     0x0045D440 0x0045D970 0x005D56B0 0x005ED9D0 0x005EDCD0 0x005FCBB0
// indirect  0x004B213E 0x004B2152 0x004B216F 0x004B21D6 0x004B21F1

// GENERATED SKELETON - tools/emit_translation_unit.py
// subject: ?on_status@SocialWin@@QAEXH@Z  at 0x004B2120  (228 bytes)
//
...
// VCall's slot001 is void; this call passes an arg, so it is shadowed
// here under its own name rather than edited in place.
class VCallArg { public:
    virtual void slot000();
    virtual void slot001(int);  // <-- used, arg
};

void SocialWin::on_status(int a1) {
    char *self = reinterpret_cast<char *>(this);
    if (a1 != 0) {
        char *base = self - 0xa14;
        reinterpret_cast<VCall *>(base)->slot012();

        char *cursor = self + 0x560c;
        for (int i1 = 0x23; i1 != 0; --i1) {
            reinterpret_cast<VCallArg *>(cursor)->slot001(0);
            cursor += 0xb4c;
        }
        cursor = self + 0x358;
        ...
        do_all_draws();
        slide_show(reinterpret_cast<GraphicWin *>(base), 0);
        reinterpret_cast<Win *>(base)->show(3);
    } else {
        char *base = self - 0xa14;
        slide_hide(reinterpret_cast<GraphicWin *>(base), 0);
        reinterpret_cast<Win *>(base)->hide();
        do_all_draws();

        char *cursor = self + 0x560c;
        for (int i3 = 0x23; i3 != 0; --i3) {
            reinterpret_cast<VCall *>(cursor)->slot002();
            cursor += 0xb4c;
        }
        cursor = self + 0x358;
