...
// TRIED: MISMATCH #0 mov/push, prologue register allocation differs
...
// subject: ?mouse_move@PullDown@@QAEXHH@Z  at 0x005F9590  (161 bytes)
//
...
void PullDown::mouse_move(int a1, int a2) {
    char *self = (char *)this;
    int old_selected = *(int *)(self + 0xf28);

    int diff = a2 - *g_00696e1c;

    void *metrics = *(void **)g_009b7b68;
    int first = *(int *)metrics;
    int divisor;
    if (first < 0) {
        divisor = *(int *)((char *)metrics + 0xc);
    } else {
        divisor = *(int *)((char *)metrics + 0x10) + first;
    }

    int idx = diff / divisor;
    *(int *)(self + 0xf28) = idx;

    int count = *(int *)(self + 0xf20);
    int found;
    if (idx < 0) {
        found = count;
    } else {
        ...
        found = 0;
        if (count > 0) {
            char *item = self + 0xa24;
            do {
                if ((*(unsigned char *)item & 1) != 0) {
                    used = used + 1;
                }
                if (used == *(int *)(self + 0xf28)) {
                    break;
                }
                found = found + 1;
                item = item + 0x14;
    ...
    if (found == count) {
        ...
        *(int *)(self + 0xf28) = found;
        if ((*(unsigned char *)(self + 0xa24 + found * 0x14) & 4) != 0) {
            *(int *)(self + 0xf28) = -1;
        }
    }

    pulldown_update(old_selected, *(int *)(self + 0xf28));
}
