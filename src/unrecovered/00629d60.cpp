...
// subject: ??0Flic@@QAE@XZ  at 0x00629D60  (151 bytes)
//
...
static int *const g_0066332b = (int *)0x0066332B;
static int *const g_0067ffb0 = (int *)0x0067FFB0;
...
Flic::Flic() {
    // Sub-objects Buffer (offset 4) and Palette (offset 0x5bc) are
    // constructed through a local layout type so their ctors run with real
    // member semantics; the scattered zero stores are plain assignments in
    // the DISASSEMBLY's order, which is not declaration order.
    struct Layout {
        char flag0_;
        Buffer buffer_;
        char pad_58c_[0x10];
    ...
    Layout *self = reinterpret_cast<Layout *>(this);
    new (self) Layout();
    self->f5ac_ = 0;
    self->f5b0_ = 0;
    self->f5b4_ = 0;
