...
// subject: ?hide@Popup@@QAEXXZ  at 0x00404A80  (127 bytes)
//
...
void Popup::hide() {
    if (is_visible()) {
        int seconds = static_cast<int>(field_30AC_) / 1000;
        if (seconds != 1000) {
            if (field_A14_ == 0) {
                if (reinterpret_cast<Win *>(g_007ae820)->is_visible()) {
                    if (seconds == 0 || seconds == 5) {
                        if (scroll_.is_visible()) {
                            scroll_hide(this, &scroll_);
                        }
                        slide_hide(this, 0);
                    }
                }
