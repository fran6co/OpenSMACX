...
// subject: ?set_from_dib@Palette@@QAEHPAUDib@@@Z  at 0x005FE650  (119 bytes)
//
...
int Palette::set_from_dib(Dib * a1) {
    ...
    char *dst = self + 0x2a;
    char *src = reinterpret_cast<char *>(a1) + 0x51;
    int count = 0xec;
    do {
        dst[-2] = src[1];
        dst[-1] = src[0];
    ...
    *reinterpret_cast<unsigned int *>(self + 0x400) = 0;
    unsigned int r;
    do {
        r = random(0, 0xffff);
        *reinterpret_cast<unsigned int *>(self + 0x400) = r;
    } while (r == 0);
    return 0;
}
