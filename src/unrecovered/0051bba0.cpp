...
// TRIED: the game_reload/exit(3)/desktop_init(0) call site is a compiler shared-tail (desktop_init's arg slot is reused dead code after the never-returning exit(3)); wrote the two reachable paths as separate if/else instead. First divergence #8 test/mov.
...
// calls     0x0041A4C0 0x00460DD0 0x0046B190 0x0058EE60 0x0058F450 0x005AAAB0 0x005D5A70 0x005FCBB0 0x00644DFF

// GENERATED SKELETON - tools/emit_translation_unit.py
// subject: ?load_proc@Console@@QAEXHH@Z  at 0x0051BBA0  (194 bytes)
//
...
extern "C" void __cdecl call_exit_644dff(int);

void Console::load_proc(int a1, int a2) {
    reinterpret_cast<BaseWin *>(g_006a7628)->exit();
    if (*g_0093f660 != 0 && (*g_009a64c0 & 0x1000) == 0) {
        return;
    }
    if (*g_0093a95c != 0 && (*g_009a64c0 & 0x1000) == 0) {
        return;
    }
    if (load_game(a1, a2) != 0) {
        return;
    }
    if (*g_009b2074 == 0) {
        desktop_init(0);
    } else {
        if (game_reload(1, 0) != 0) {
            call_exit_644dff(3);
        }
    }
    if (a1 != 0) {
    ...
    reinterpret_cast<MainMenu *>(reinterpret_cast<char *>(this) + 0x22a2c)->check(field_23BDC_);
}
