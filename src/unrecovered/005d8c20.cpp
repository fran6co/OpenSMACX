...
// TRIED: 223 vs 224 bytes; `int al=0; al=*(byte*)g_00696d14;` (two statements) reproduced the original's xor-then-byte-mov where a single `unsigned char al = *...;` collapsed to one mov. The remaining divergence is register/instruction ordering inside the `piVar1==0` branch (mov/cmp order, an extra `mov eax,ecx`); tried `--*(int*)(esi+0x6c)` for the pre-decrement, close but not exact.
...
// subject: sub_5d8c20  at 0x005D8C20  (224 bytes)
//
...
// `esi = ecx` at entry with every field read off `esi` and no stack access
// for that value - `this`, not a stack argument - so the contract's plain
// `__stdcall(int,int,int,int)` is wrong; recast as `__thiscall` with the
// four ints forwarded straight through to `draw_dest_unk1`.
class Obj5d8c20 {
public:
    void method(Buffer *a1, int a2, int a3, unsigned char *a4);
};

void Obj5d8c20::method(Buffer *a1, int a2, int a3, unsigned char *a4) {
...
skip:
    reinterpret_cast<Sprite *>(g_009b3a50)->draw_dest_unk1(a1, a2, a3, a4);
    *g_009b3a54 = 0;
}
