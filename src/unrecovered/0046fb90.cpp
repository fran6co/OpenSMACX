...
// INDEXED TABLE BASE lever: `[eax*4+0x66f8c8]` etc bake the address into the
// addressing mode itself, so the folded-literal globals above would let /O2
// change the instruction shape. Own externs keep the relocation live.
extern int32_t g_table_f8c8[];
extern int32_t g_table_efbc[];
extern int32_t g_table_f440[];
...
void MapWin::clear_alt(int a1, int a2, int a3) {
    ...
    // between the int32 and byte stores, writing the byte store before the
    // int32 store - still lowers to a single fused `mov dword ptr[rec],-1`
    // (equal cost, different bytes). ~10 tries; not found.
    if (owned_ != 0) {
        int i = 0;
        int count = g_table_f8c8[a3];
        if (count > 0) {
            int width = *g_00949870;
            do {
                int dx = g_table_efbc[i] + a1;
                if ((*g_0094988c & 1) == 0) {
                    if (dx < 0) {
                        dx += width;
                    } else if (dx >= width) {
                ...
                int dy = g_table_f440[i] + a2;
                if (dy >= 0 && dy < *g_00949874 && dx >= 0 && dx < width) {
                    char *rec = reinterpret_cast<char *>(owned_) + ((width >> 1) * dy + (dx >> 1)) * 0xc + 4;
                    *reinterpret_cast<int32_t *>(rec) = -1;
                    *reinterpret_cast<uint8_t *>(rec + 4) = 0xff;
                    width = *g_00949870;
                }
                i++;
            } while (i < count);
