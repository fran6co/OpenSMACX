...
// TRIED: `a2` is read nowhere in the disasm (Ghidra recovers only 2 params: this + the RECT*); kept in the signature, unused in the body. Mismatch is #0 in the prologue (frame-size choice).
...
// subject: ?update@GraphicWin@@QAEXPAURECT@@PAUGraphicWin@@@Z  at 0x005D5630  (122 bytes)
//
...
void GraphicWin::update(RECT * a1, GraphicWin * a2) {
    RECT *rect = a1;
    if (rect == 0) {
        rect = reinterpret_cast<RECT *>(reinterpret_cast<char *>(this) + 0x474);
    }
    long top = rect->top;
    long left = rect->left;
    ...
    if (this->is_visible()) {
        ...
        local.left = left;
        local.top = top;
        this->update_window(&local);
        this->client_to_screen(&local);
        this->flip(&local);
    }
}
