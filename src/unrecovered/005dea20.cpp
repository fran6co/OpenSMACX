// ORIGINAL: 0x005DEA20 FILE
// name      sub_5dea20
// size      252 bytes
// spans     0x005DEA20-0x005DEB1C
// prototype 
// callers   1   call targets   1
// kind      game
// flags     hidden;sp_ready;purged_ok
// calls     0x00628AD0
// RULED-OUT: `int *a1, int a2` signature (IDA's int,int guess with the
//            pointer made explicit - the disassembly never touches [ecx]),
//            do/while over a hoisted `idx` local mirroring the ASM's
//            search-then-fill-record shape. Reproduces the whole body but
//            diverges at MISMATCH #2 (original never sets up an ebp frame,
//            addressing args purely off esp; every C form tried here gets
//            a `push`-heavy prologue instead of the original's early `mov
//            ebx,[esp+0xc]`).
// GENERATED SKELETON - tools/emit_translation_unit.py
// subject: sub_5dea20  at 0x005DEA20  (252 bytes)
//
...
extern "C" int __cdecl sub_5dea20(int *a1, int a2) {
    if (a2 != *g_009b39c4) {
        int idx;
        do {
            idx = a1[0] + a2;
            ...
            int diff = *(int *)(*g_009b3a0c + 4 + idx * 8) - *(int *)(*g_009b3a0c + 4 + a2 * 8);
            a1[1] = diff;
            if (diff != 0) {
                if (diff < 0) {
                    return 0;
                ...
                a1[2] = idx;
                a1[7] = *(int *)(*g_009b3a0c + a2 * 8);
                int span = *(int *)(*g_009b3a0c + idx * 8) - *(int *)(*g_009b3a0c + a2 * 8);
                int q;
                if (span < 0) {
                    span = -span;
                    a1[10] = 1 - a1[1];
                ...
                a1[0xc] = a1[1];
                a1[0xb] = span % a1[1];
                int fp = *(int *)(*g_009b3a18 + a2 * 4);
                a1[0xd] = fp * 0x10000;
                int result = fixed_div(*(int *)(*g_009b3a18 + idx * 4) * 0x10000 - fp * 0x10000,
                                        diff * 0x10000);
                a1[0xe] = result;
                return 1;
            }
            a2 = idx;
        } while (idx != *g_009b39c4);
    }
    return 0;
}
