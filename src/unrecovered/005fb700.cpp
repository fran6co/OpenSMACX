...
// TRIED: direct transcription only tried once; diverges at #0 (push vs mov) - the compiled prologue does not preserve esi/edi the same way the original does
...
// subject: ?rename_menu_item@Menu@@QAEHHHPAD@Z  at 0x005FB700  (89 bytes)
//
...
int Menu::rename_menu_item(int a1, int a2, char * a3) {
    int i = 0;
    for (;;) {
        int id = entries_[i].id;
        if (id == -1) {
            return 0xB;
        }
        if (id == a1) {
    ...
    return entries_[i].pull_down->rename_item(a2, a3);
}
