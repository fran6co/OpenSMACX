// ORIGINAL: 0x005D91D0 BYTE_EXACT FILE
// name      sub_5d91d0
// size      238 bytes
...
// 005d8650/005d8840/005d8a30/005d8df0/005d8fe0/005d93c0: a thiscall Buffer
...
// needs a helper class (BufferSelf, padded to the real offsets) ahead of
// the definition, and body-mode's census extractor stops at the first
// top-level closing brace it sees - which would be the class's, not the
...
// this body scored BYTE_EXACT with no change to a single line of it.
// tools/verify_span_termination.py now fails the gate on a truncated span,
// so the next one is caught before an agent has to out-argue a verdict.
// GENERATED SKELETON - tools/emit_translation_unit.py
// subject: sub_5d91d0  at 0x005D91D0  (238 bytes)
//
...
class BufferSelf { public:
    unsigned char pad_0_[0x50];
    uint32_t field_50_;
    uint32_t ppv_bits_;
    uint32_t field_58_;
    unsigned char pad_1_[0x6C - 0x5C];
    uint32_t field_6C_;
    unsigned char pad_2_[0x80 - 0x70];
    uint32_t width_;
    uint32_t height_;
    unsigned char pad_3_[0x4A8 - 0x88];
    uint32_t field_4A8_;

    int sub_5d91d0(int, int, int, int, int);
};

int BufferSelf::sub_5d91d0(int a1, int a2, int a3, int a4, int a5) {
    int transparent = a2;
    if (field_58_ == 0) {
        if (a2 == -1) {
...
tail:
    {
        int result = reinterpret_cast<Sprite *>(g_009b3a50)->draw_dest_unk2(
            reinterpret_cast<Buffer *>(a1), a2, a3, a4,
            reinterpret_cast<unsigned char *>(a5));
        *g_009b3a54 = 0;
        return result;
    }
}
