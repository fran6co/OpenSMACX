...
// The managed globals, DEFINED in init_thunks.cpp as real objects. The
// image places each at a fixed address in its data segment - 0x00822610 for
// the NewTechWin sprite array, 0x009B6F08 for the buffer Win draws into,
// 0x009B8EF8..0x009B96B0 for the rest - and this tree deliberately does not
// care where they land: construction is the construct_* thunks' and the
// runtime's business, and every measurement that references one of these
// reads through the relocation mask, not the literal address.
extern Sprite g_NEWTECHWIN_SPRITES[6];
