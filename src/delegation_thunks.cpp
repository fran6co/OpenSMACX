 ...
 *
 * Most rows are free redirects over opaque storage. The 32 Midi rows are the
 * exception since 2026-08-23: their delegate is modelled as `MidiDelegate`
 * in hypothesis_layouts.h with virtuals at the image's own slot order, so
 * each is a real `Midi::` method whose `delegate_->f(...)` lets VC6 emit the
 * image's `call/jmp dword ptr [eax + slot]` itself.
 */
