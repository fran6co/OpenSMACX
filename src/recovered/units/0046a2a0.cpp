...
//   build/byte-match/0046a2a0/unit.cpp
// and score it with tools/agent_brief.py.
// GENERATED SKELETON - tools/emit_translation_unit.py
// subject: ?draw_radius@MapWin@@QAEXHHHH@Z  at 0x0046A2A0  (502 bytes)
//
...
// g_00939340/g_009392c0 are indexed by a register (`[eax*4 + addr]`), so
// they need `extern int name[]`, not the plain const-pointer spelling above
// - the emitter does not detect base-immediate/offset-register on its own.
extern int g_00939340_tbl[];
extern int g_009392c0_tbl[];

void MapWin::draw_radius(int a1, int a2, int a3, int a4) {
    char *self = reinterpret_cast<char *>(this);

    unsigned char flags = *reinterpret_cast<unsigned char *>(g_009a64c0);
    int color = (flags & 0x80) ? -1 : *g_00939284;

    gen_radius(a1, a2, a3, color, (a4 < 0));

    if (*g_00939288 == 0) {
        int busy = *g_0093f660;
        if (busy != 0) {
            if (*g_0093a938 != 0) goto after;
        }
        if (a4 < 0) goto after;
        if (*g_009392a8 == 0) {
            if (*g_009392b0 == 0) goto after;
        }
        if (*g_00939294 == 0) {
            if (busy == 0) goto after;
            if ((*reinterpret_cast<unsigned char *>(g_009a64c0) & 2) == 0) goto after;
        }

        if (*reinterpret_cast<int *>(self + 0x1dd78) != 0) {
            if ((*reinterpret_cast<unsigned int *>(self + 0x1dd70) & 0x80000000u) == 0) goto after;
        }

        int idx = *g_009392b8;
        int laty = *reinterpret_cast<int *>(self + 0x1dda8);
        int tby = g_00939340_tbl[idx];
        int tbx = g_009392c0_tbl[idx];
        if (tby < laty) goto after;
        int ylim = *reinterpret_cast<int *>(self + 0x1ddd8) +
                   *reinterpret_cast<int *>(self + 0x1ddd0) + laty;
        if (tby >= ylim) goto after;

        int x = tbx;
        if ((*reinterpret_cast<unsigned char *>(g_0094988c) & 1) == 0) {
            int latx = *reinterpret_cast<int *>(self + 0x1dda4);
            int wrapw = *g_00949870;
            if (x < latx) x += wrapw;
            int xlim = *reinterpret_cast<int *>(self + 0x1ddd4) +
                       *reinterpret_cast<int *>(self + 0x1ddcc) + latx;
            if (x >= xlim) x -= wrapw;
        }
        int latx2 = *reinterpret_cast<int *>(self + 0x1dda4);
        if (x < latx2) goto after;
        int xlim2 = *reinterpret_cast<int *>(self + 0x1ddd4) +
