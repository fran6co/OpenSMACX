...
// indirect  0x005314D0 0x005314F6
...
//   build/byte-match/00531480/unit.cpp
// and score it with tools/agent_brief.py.
// GENERATED SKELETON - tools/emit_translation_unit.py
// subject: ?await_synch@NetDaemon@@QAEXXZ  at 0x00531480  (310 bytes)
//
...
void NetDaemon::await_synch() {
    char *self = reinterpret_cast<char *>(this);
    int *state = reinterpret_cast<int *>(self + 0x1bb8);
    ...
    if (*g_0093f660 != 0) {
        ...
        *state = synching;
        ...
        (*g_0093f664)++;
        unsigned long start_time = reinterpret_cast<TimeGetTimeFn>(*g_00669368)();
                ...
                if (!warned) {
                    ...
                    if (now - start_time > 0x7d0) {
                        ...
                        reinterpret_cast<NetMsg *>(g_00805338)->pop(
                            reinterpret_cast<const char *>(g_0068c404), 0, 2, 0);
                    }
                }
                wait_task();
                unsigned long v1, v2;
                int r = reinterpret_cast<Net *>(g_0093cd90)->get(&v1, &v2);
                if (r != 0) {
                    process_message(reinterpret_cast<char *>(r), v2,
                                    static_cast<int>(v1));
                }
                reinterpret_cast<Lock *>(self + 0x14a0)->check_global();
                check_spock();
            } while (*state == synching);
            if (warned) {
