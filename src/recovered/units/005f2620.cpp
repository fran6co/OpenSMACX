// ORIGINAL: 0x005F2620 FILE
// name      ?OnRButtonUp@Win@@QAAXPAXHHI@Z
// size      65 bytes
// spans     0x005F2620-0x005F2661
// prototype 
...
//   build/byte-match/005f2620/unit.cpp
// and score it with tools/agent_brief.py.
// GENERATED SKELETON - tools/emit_translation_unit.py
// subject: ?OnRButtonUp@Win@@QAAXPAXHHI@Z  at 0x005F2620  (65 bytes)
//
...
// This body dispatches through slot(s): 84
class VCall { public:
...
class Win { public:
    ...
    void __cdecl OnRButtonUp(void *, int, int, unsigned int);
};

// Own shim, not the emitter's `VCall` above: the verifier rebuilds the
// scaffolding fresh from the catalogue for every score (it does not
// read this file), so a slot signature edited in the class above is
// invisible to `verify_recovered_function.py` even though it IS what
// lands when this whole file is compiled as one unit. A same-shape
// class declared here, with slot084 given the arity this call needs,
...
// CLOSEST REACHED: MISMATCH, 66 rebuilt bytes vs 65 original, first
// divergence at mnemonic #5 (original 'call' vs rebuilt 'mov').
//
// The logic is right (confirmed against both the disassembly and the
// Ghidra hypothesis): zero the flag, ask get_mouse_window() for the
// window under the cursor by writing through &a2/&a3, and if one is
// found, dispatch through its OWN vtable slot 84 with (a2, a3, a4, the
// second global). The byte gap is a calling-convention wall, not a
// logic gap: `get_mouse_window` is mangled QAA (public, __cdecl,
// non-static), and the original call passes `this` through ECX but
// still lets the CALLER clean the two explicit ints (`add esp, 8`).
// This toolchain's compiler does not reproduce that hybrid from normal
// C++ call syntax - `&Win::get_mouse_window` types as
// `int (__thiscall Win::*)(int*, int*)` (confirmed: casting it to the
// `__cdecl` pointer-to-member type the declaration implies is
// rejected, C2440), and every call form tried compiles as a full
// __thiscall (this via ECX, callee cleans - no `add esp` at all),
// which is the observed one extra `mov ecx, <this>` plus the missing
// cleanup. RULED OUT: calling through `this->get_mouse_window(...)`
// directly, and through an explicit `__cdecl` pointer-to-member value.
