// ORIGINAL: 0x004B2470 ?on_iface_button_clicked@SocialWin@@QAEXH@Z 0x004B2470-0x004B264E FILE
// size      478 bytes
...
//   build/byte-match/004b2470/unit.cpp
// and score it with tools/agent_brief.py.
// GENERATED SKELETON - tools/emit_translation_unit.py
// subject: ?on_iface_button_clicked@SocialWin@@QAEXH@Z  at 0x004B2470  (478 bytes)
//
...
static int *const g_004b2650 = (int *)0x004B2650;
...
class SocialWin { public:
    ...
    void allocate_energy(int, int);
    void on_iface_button_clicked(int);
};

// INDEXED TABLE BASE: each of these four is indexed by a per-faction stride
// computed from a register, never a plain load - the scaffold's own pointer
// constants (g_0096cc2c, g_0096cd04, g_0096cd08 above) are `static T *const`
// variables, and folding one of those through index arithmetic reads its
// stored VALUE back from memory instead of encoding the address as an
// immediate/relocation the way the original's single `lea [reg*N+ADDR]`
// does. `extern T name[];` gives the right encoding. 0x96CC08 was never
// picked up by the emitter at all (no register*4 pattern at its own site),
// so it gets the same treatment here.
extern int32_t g_arr_96cc08[];
extern int32_t g_arr_96cc2c[];
extern int32_t g_arr_96cd04[];
extern int32_t g_arr_96cd08[];

void SocialWin::on_iface_button_clicked(int a1) {
    char *self = reinterpret_cast<char *>(this);

    // Plain call-target load, no arithmetic on the address - the scaffold's
    // `int *const g_008c6e68` is fine here, just retyped at the use site.
    ...
    int32_t *slot = reinterpret_cast<int32_t *>(self + 0x2230) + index;
    *slot = *slot + delta;
    if (*slot < 0) {
        *slot = 0;
    }

    int limit = energy_limit(*reinterpret_cast<int32_t *>(self + 0x2e0));
    if (*slot > limit) {
        *reinterpret_cast<int32_t *>(self + 0x2248) = 1;
        *slot = energy_limit(*reinterpret_cast<int32_t *>(self + 0x2e0));
    }

    SocialWin *base = reinterpret_cast<SocialWin *>(self - 0xa14);
    base->allocate_energy(index, *slot);

    {
        int32_t *p = reinterpret_cast<int32_t *>(self + 0x2230);
        for (int i = 0; i < 3; ++i) {
            p[i] = p[i + 3];
        }
    }

    g_arr_96cd04[*reinterpret_cast<int32_t *>(self + 0x2e0) * 2099] =
        *reinterpret_cast<int32_t *>(self + 0x2240);
    g_arr_96cd08[*reinterpret_cast<int32_t *>(self + 0x2e0) * 2099] =
        *reinterpret_cast<int32_t *>(self + 0x2244);

    social_set(*reinterpret_cast<int32_t *>(self + 0x2e0));

    int32_t faction = *reinterpret_cast<int32_t *>(self + 0x2e0);
    social_calc(reinterpret_cast<int32_t *>(self + 0x2e4),
                reinterpret_cast<int32_t *>(self + 0x2f4), faction, 0, 0);

    faction = *reinterpret_cast<int32_t *>(self + 0x2e0);
    social_calc(reinterpret_cast<int32_t *>(self + 0x2e4),
                &g_arr_96cc2c[faction * 2099], faction, 0, 0);

    energy_compute(*reinterpret_cast<int32_t *>(self + 0x2e0), 1);

    *reinterpret_cast<int32_t *>(self + 0x350) = *g_00749ae4;
    *reinterpret_cast<int32_t *>(self + 0x34c) = *g_00749c0c;
