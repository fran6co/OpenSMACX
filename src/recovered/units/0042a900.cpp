...
// MEASURED NO_COMPILE FOR A REASON UNRELATED TO THIS BODY: same defect as
// 0x004AF1E0 (SocialWin::draw_presets) - the live scaffolding for this
// address puts a class embedding a by-value `Buffer buffer_;` ahead of
// `class Buffer`'s own full definition, so it fails with C2079 "'buffer_'
// uses undefined class 'Buffer'" before this function is even reached -
// confirmed with a trivial empty body, which fails identically. This is
// an emitter ordering defect, not a C++ error in the body below, which is
// otherwise faithful to the disassembly and ready to score once the
