...
//   build/byte-match/0053c650/unit.cpp
// and score it with tools/agent_brief.py.
// GENERATED SKELETON - tools/emit_translation_unit.py
// subject: ?pact_ends@@YAXHH@Z  at 0x0053C650  (1376 bytes)
//
...
int __cdecl pact_withdraw(int, int);
...
static int *const g_0067c5c8 = (int *)0x0067C5C8;
static int *const g_0068d5b0 = (int *)0x0068D5B0;
static int *const g_0068d5bc = (int *)0x0068D5BC;
static int *const g_0068d5c8 = (int *)0x0068D5C8;
...
extern char g_96c9f8x[]; // per-(human,other) contact flag table, indexed (human*0x833+other)*4, bit0

// EH-UNWIND / INLINED-MULTI-BASE-CTOR WALL: same shape as 0x0047C530 -
// a full SEH frame (push -1; push 0x65e530; mov eax,fs:0; push eax;
// mov fs:0,esp; alloca_probe 0x5384) wraps one inlined multi-base local
// widget (Popup + a GraphicWin-shaped interface, hand-patched vtables
// 0x444 bytes apart) whose partial-construction unwind funclets live at
// 0x0065E470-0x0065E535. Not reproduced (compare 0x0047C530's documented
// wall) - this reproduces the call sequence, offsets, and all three
// branches (human withdraws / other withdraws / neither, with its
// early-out) only.
void __cdecl pact_ends(int a1, int a2) {
    ...
    g_0093f7cc[0] = a2;
    treaty_off(a1, a2, 1);
    int withdrawAB = pact_withdraw(a1, a2);
    int withdrawBA = pact_withdraw(a2, a1);
    draw_map(1);

    int human = *g_00939284;
    int useWithdraw;

    if (a1 == human) {
        char *rec = g_946d34x + a2 * 0x59c;
        *g_009bbfec = *reinterpret_cast<int *>(rec + 0x18);
        *g_009bbff0 = *reinterpret_cast<int *>(rec + 0x1c);
        parse_says(0, rec, -1, -1);
        parse_says(1, rec + 0x20, -1, -1);
        useWithdraw = withdrawAB;
    } else if (a2 == human) {
        char *rec = g_946d34x + a1 * 0x59c;
        *g_009bbfec = *reinterpret_cast<int *>(rec + 0x18);
        *g_009bbff0 = *reinterpret_cast<int *>(rec + 0x1c);
        parse_says(0, rec, -1, -1);
        parse_says(1, rec + 0x20, -1, -1);
        useWithdraw = withdrawBA;
    } else {
        if (spying(a1) == 0 && spying(a2) == 0 &&
            (*reinterpret_cast<unsigned char *>(g_009a64c0) & 0x80) == 0 &&
            (g_96c9f8x[(human * 0x833 + a1) * 4] & 1) == 0 &&
            (g_96c9f8x[(human * 0x833 + a2) * 4] & 1) == 0) {
            goto skip_popup;
        }
        char *rec1 = g_946d34x + a1 * 0x59c;
        *g_009bbfec = *reinterpret_cast<int *>(rec1 + 0x18);
        *g_009bbff0 = *reinterpret_cast<int *>(rec1 + 0x1c);
        parse_says(0, rec1, -1, -1);
        char *rec2 = g_946d34x + a2 * 0x59c;
        *g_009bbfec = *reinterpret_cast<int *>(rec2 + 0x18);
        *g_009bbff0 = *reinterpret_cast<int *>(rec2 + 0x1c);
        parse_says(1, rec2, -1, -1);
        parse_says(2, get_pact_hood(a1, a2), -1, -1);
        X_pop(reinterpret_cast<const char *>(g_0068d5c8), 0);
        goto skip_popup;
    }

    // Shared tail: only the "a1==human" / "a2==human" branches reach here.
    parse_num(0, useWithdraw);
    parse_says(2, get_pact_hood(a1, a2), -1, -1);
    {
        unsigned int mask = (*g_0093f660 != 0) ? 0xffffffffu : 0;
        unsigned int flag = (mask & 0xfff00000) + 0x100000;
        reinterpret_cast<Popup *>(w)->start(
            reinterpret_cast<char *>(g_009b8aa8), reinterpret_cast<const char *>(g_0068d5b0), -1,
            0, flag, 0);
    }
    if (useWithdraw != 0) {
        unsigned int mask = (*g_0093f660 != 0) ? 0xffffffffu : 0;
        unsigned int flag = ((mask & 0xfff00000) + 0x100000) | 0x80;
        reinterpret_cast<Popup *>(w)->start(
            reinterpret_cast<char *>(g_009b8aa8), reinterpret_cast<const char *>(g_0068d5bc), -1,
            0, flag, 0);
    }
    reinterpret_cast<BasePop *>(w)->exec(0, 0);

skip_popup:
    reinterpret_cast<Popup *>(w)->close();
    reinterpret_cast<Scroll *>(w + 0x3230)->close();
    reinterpret_cast<FlatButton *>(w + 0x4828)->close();
    reinterpret_cast<BaseButton *>(w + 0x4828)->~BaseButton();
    reinterpret_cast<FlatButton *>(w + 0x3cdc)->close();
    reinterpret_cast<BaseButton *>(w + 0x3cdc)->~BaseButton();
    reinterpret_cast<GraphicWin *>(w + 0x3230)->~GraphicWin();

    reinterpret_cast<BasePop *>(w)->close();
    reinterpret_cast<Spot *>(w + 0x3098)->~Spot();
    reinterpret_cast<Dialogs *>(w + 0x2358)->~Dialogs();
    reinterpret_cast<Dialog *>(w + 0x2d70)->~Dialog();
    reinterpret_cast<GraphicWin *>(w + 0x2358)->~GraphicWin();

    sub_4066c0();
    *g_009b3374 = *reinterpret_cast<int *>(w + 0x21a8 + 4);
    sub_4066c0();
    *g_009b3374 = *reinterpret_cast<int *>(w + 0x2178 + 4);

    reinterpret_cast<Sprite *>(w + 0x2118)->close();
    reinterpret_cast<FlatButton *>(w + 0x15a8)->close();
    reinterpret_cast<BaseButton *>(w + 0x15a8)->~BaseButton();
    reinterpret_cast<FlatButton *>(w + 0xa5c)->~FlatButton();
    reinterpret_cast<Heap *>(w + 0xa28)->shutdown();
