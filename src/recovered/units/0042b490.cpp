// ORIGINAL: 0x0042B490 FILE
// name      ?set_cat_adv_concept@Datalink@@QAEXXZ
// size      410 bytes
// spans     0x0042B490-0x0042B62A
// prototype void (__thiscall ?set_cat_adv_concept@Datalink@@QAEXXZ)(Datalink* this)
...
//   build/byte-match/0042b490/unit.cpp
// and score it with tools/agent_brief.py.
// GENERATED SKELETON - tools/emit_translation_unit.py
// subject: ?set_cat_adv_concept@Datalink@@QAEXXZ  at 0x0042B490  (410 bytes)
//
...
static int *const g_00683310 = (int *)0x00683310;
static int *const g_0068331c = (int *)0x0068331C;
static int *const g_00683328 = (int *)0x00683328;
...
// The catalogue's own `_itoa()` stub takes no parameters (the emitter
// could not resolve its real arity); called through its address reinterpreted
// to the true CRT prototype instead of redeclaring the symbol.
typedef char *(__cdecl *ItoaFn)(int, char *, int);

void Datalink::set_cat_adv_concept() {
    char *const self = reinterpret_cast<char *>(this);

    if (text_open(reinterpret_cast<char *>(g_0068331c),
                  reinterpret_cast<char *>(g_00683310)) != 0) {
        return;
    }
    ...
    while (*reinterpret_cast<char **>(g_009b7d00) != 0 &&
        ...
        *reinterpret_cast<char *>(g_009b86a0) = 0;
        strcat(reinterpret_cast<char *>(g_009b86a0),
              *reinterpret_cast<char **>(g_009b7d00));
        if (*g_009a64c0 & 0x1000) {
            strcat(reinterpret_cast<char *>(g_009b86a0),
                  reinterpret_cast<char *>(g_00683328));
            char buf[80];
            reinterpret_cast<ItoaFn>(_itoa)(i + 10000, buf, 10);
            strcat(reinterpret_cast<char *>(g_009b86a0), buf);
        }
        int32_t item_id = i + 10000;
        i++;
        reinterpret_cast<ListBox *>(self + 0xf628)
            ->item(reinterpret_cast<char *>(g_009b86a0), item_id);
        text_get();
    }
    text_close();

    Buffer *const buf_obj = reinterpret_cast<Buffer *>(self + 0x444);
    RECT local_rect;
    int32_t *const src = reinterpret_cast<int32_t *>(self + 0x103d4);
    local_rect.left = src[0];
    local_rect.top = src[1];
    local_rect.right = src[2];
    local_rect.bottom = src[3];
    buf_obj->box_sprite(&local_rect, reinterpret_cast<BoxSpriteParams *>(g_0078d618));

    local_rect.left += 3;
    local_rect.right -= 3;
    local_rect.top += 3;
    local_rect.bottom -= 3;
    buf_obj->set_text_color(0xe0, -1, 1, 1);

    buf_obj->set_font(reinterpret_cast<Font *>(self + 0x101cc), 0, 0, 0);

    int32_t idx = *reinterpret_cast<int32_t *>(
        *reinterpret_cast<char **>(g_009b90f8) + 0xb24);
    int32_t found = reinterpret_cast<Strings *>(g_009b90d8)->get(idx);
    if (found != 0) {
        char *const str = reinterpret_cast<char *>(found);
        buf_obj->write_cent_l(str, &local_rect, strlen(str));
    }
