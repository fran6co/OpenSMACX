...
// calls     0x0041A920 0x00459280 0x0045D380 0x004B6570 0x00511520 0x00531020 0x005310F0 0x00531480 0x0055BC00 0x00593220 0x005C1A20 0x005C1D70 0x005FAB00
...
// refusal        u004b8e40.cpp(148) : error C2079: 'spot_' uses undefined class 'Spot'
//
// The WHOLE unit as measured, scaffolding included: for the units
// that are byte-exact yet refuse extraction, the agent tuned the
// emitted scaffolding and the body alone will not reproduce the
// verdict. To resume, copy everything below back over
//   build/byte-match/004b8e40/unit.cpp
// and score it with tools/agent_brief.py.
// GENERATED SKELETON - tools/emit_translation_unit.py
// subject: ?on_left_click@StatusWin@@QAEXHH@Z  at 0x004B8E40  (403 bytes)
//
...
void StatusWin::on_left_click(int a1, int a2) {
    char *self = reinterpret_cast<char *>(this);
    if (*reinterpret_cast<int *>(self + 0x15d8) == 0) {
        reinterpret_cast<InfoWin *>(g_007ad2a0)->reset();
        if (*reinterpret_cast<int *>(self + 0x15d4) != 0) {
            *reinterpret_cast<int *>(self + 0x15d4) = 0;
            reinterpret_cast<SubInterface *>(g_006eeed8)->release_iface_mode();
        }
        *g_006e8150 = 0;
        int vehId;
        int kind;
        int checkResult = reinterpret_cast<Spot *>(self + 0x15e0)->check(a1, a2, &vehId, &kind);
        if (checkResult >= 0) {
            switch (kind) {
            case 0:
            case 10:
                if (veh_selectable(vehId) != 0) {
                    *g_0093928c = vehId;
                    if (reinterpret_cast<NetDaemon *>(g_0093cd90)->lock_veh(&vehId, 0, -1, -1, 0) == 0) {
                        veh_wake(vehId);
                        synch_veh(vehId);
            ...
            case 11:
                if ((*reinterpret_cast<unsigned char *>(g_009a64c0) & 0x80) != 0 ||
                    g_0097d044_arr[vehId * 0x134] == *g_00939284 ||
                    spying(g_0097d044_arr[vehId * 0x134]) != 0 ||
                    (g_0096c9f8_arr[(*g_00939284 * 0x833 + g_0097d044_arr[vehId * 0x134]) * 4] & 1) != 0) {
                    reinterpret_cast<BaseWin *>(g_006a7628)->zoom(vehId, 0);
                    return;
                }
                break;
            case 3:
                *reinterpret_cast<uint32_t *>(self + 0x15a8) = ~*reinterpret_cast<uint32_t *>(self + 0x15a8);
