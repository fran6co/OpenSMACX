...
// A row of 11 ints per facility, indexed by facilityID_. Register-scaled
// index (`[edx*4+0x94f35c]` with edx=facilityID_*11), so the indexed-table
// lever applies.
extern int32_t g_facility_table[];

// The scaffold's VCall shim declares slot001 nullary, but this call site
// passes one int argument. Only DECLARATION ORDER matters for a vtable
// shim (per the scaffold's own comment), not the class name, so a second
// shim class retypes the same slot instead of editing the given one.
...
// NO_COMPILE: this is a scaffold defect, not a body problem. Confirmed by
// compiling `void Datalink::draw_tech() {}` against the unmodified
// scaffold - it fails identically. `BaseButton` (line 135) and
// `FlatButton` (line 266) both embed `Buffer buffer_;` by value, but
// `Buffer`'s own class definition doesn't appear until line 552, so C2079
// ("uses undefined class 'Buffer'") fires while parsing BaseButton/
// FlatButton themselves, before this function's body is even reached.
// Datalink embeds 18 FlatButton members, so every candidate for this
// address hits the same wall. See PROPOSALS in the report.
