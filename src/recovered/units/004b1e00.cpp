...
// calls     0x004AECE0 0x004AEF10 0x004BDDE0
...
//   build/byte-match/004b1e00/unit.cpp
// and score it with tools/agent_brief.py.
// GENERATED SKELETON - tools/emit_translation_unit.py
// subject: ?on_button_clicked@SocialWin@@QAEXH@Z  at 0x004B1E00  (73 bytes)
//
...
static int *const g_004b1e4c = (int *)0x004B1E4C;
...
void SocialWin::on_button_clicked(int a1) {
    // 98% mnemonic-similar, single divergence: the compiled jump table
    // needs 4-byte alignment after this function's `ret`, and this
    // compile fills the 3-byte gap with `lea ecx,[ecx+0]` where the
    // catalogued 73-byte span has no such instruction (the original's
    // table sits at a fixed image address 3 bytes past its own `ret`,
    // so it pads the same gap, just not with an instruction this
    // symbol's span picks up). This is COMDAT/alignment layout, not a
    // reachable source difference - the 25 real instructions before it
    // match exactly.
