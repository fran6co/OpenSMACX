...
// calls     0x005D5890 0x0060B530 0x00644F3A
...
//   build/byte-match/0060cbc0/unit.cpp
// and score it with tools/agent_brief.py.
// GENERATED SKELETON - tools/emit_translation_unit.py
// subject: ?on_mouse_move@ListBox@@QAEXHH@Z  at 0x0060CBC0  (587 bytes)
//
...
// Register-level transliteration: the callee is reached with `this`
// pointing 0x48 bytes into the real ListBox (an adjustor-style base), and
// registers get reused for unrelated values throughout, so this is written
// as a direct mirror of the disassembly (goto-driven) rather than
// restructured control flow.
#define RD(a) (*reinterpret_cast<int *>(a))

void ListBox::on_mouse_move(int a1, int a2) {
    int self = reinterpret_cast<int>(this);
    int base = self - 0x48;
    int eax, ebx, ecx, edx, esi;
    int local10, remaining, callArg2;

    eax = RD(base);
    ecx = RD(eax + 8);
    eax = RD(ecx + self + 0x84);
    esi = ecx + self;
    if (eax == 0) {
        return;
    }
...
L_cd94:
    ...
    if (eax != -1) {
        ...
        ecx = eax - edx;
        *g_009b8ee8 = ecx;
        reinterpret_cast<ListBox *>(base)->draw_item(eax);
    }
    edx = RD(base);
    eax = RD(edx + 8);
    edx = RD(self - 0x30);
    ecx = RD(eax + self + 0xa8);
    ecx -= edx;
    *g_009b8ee8 = ecx;
    edx = RD(base);
    eax = RD(edx + 8);
    ...
    callArg2 = ecx;
L_draw2:
    reinterpret_cast<ListBox *>(base)->draw_item(callArg2);
    edx = RD(base);
    eax = RD(edx + 4);
    reinterpret_cast<GraphicWin *>(eax + self)->soft_update();
}
