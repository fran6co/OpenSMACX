...
// calls     0x0048C0A0 0x00539460 0x00593390 0x005933C0 0x005B4210 0x005B4550 0x00625E30
...
//   build/byte-match/004aef10/unit.cpp
// and score it with tools/agent_brief.py.
// GENERATED SKELETON - tools/emit_translation_unit.py
// subject: ?exit@SocialWin@@QAEXXZ  at 0x004AEF10  (508 bytes)
//
...
static int *const g_00687428 = (int *)0x00687428;
static int *const g_0068743c = (int *)0x0068743C;
static int *const g_0068744c = (int *)0x0068744C;
static int *const g_00687458 = (int *)0x00687458;
static int *const g_00687464 = (int *)0x00687464;
static int *const g_00687474 = (int *)0x00687474;
...
// INDEXED TABLE BASE: every one of these four is indexed by
// `factionID_ * 2099` (0x20cc bytes per faction as int32), and one is
// indexed by `factionID_ * 0x59c` as a byte array - arithmetic on the
// address, so each needs `extern T name[]` rather than the scaffold's
// plain pointer constant.
extern int32_t g_arr_96cc00[];
extern int32_t g_arr_96cc08[];
extern int32_t g_arr_96cc28[];
extern int32_t g_arr_96cc2c[];
extern uint8_t g_arr_946f58[];

void SocialWin::exit() {
    int32_t *cat = reinterpret_cast<int32_t *>(socCategory_);

    int iVar1 = social_upheaval(factionID_, cat) - g_arr_96cc28[factionID_ * 2099];

    uint8_t flags = *reinterpret_cast<uint8_t *>(g_009a64c0);
    if ((flags & 0x40) == 0 || (flags & 0x80) == 0) {
        if (iVar1 > 0) {
            parse_num(0, iVar1);
            parse_num(1, g_arr_96cc00[factionID_ * 2099]);

            if (iVar1 > g_arr_96cc00[factionID_ * 2099]) {
                popp(reinterpret_cast<char *>(*g_00691b0c),
                     reinterpret_cast<const char *>(g_0068743c),
                     0,
                     reinterpret_cast<const char *>(g_00687428),
                     reinterpret_cast<int(__cdecl *)()>(g_005398e0));
                return;
            }

            if (g_arr_946f58[factionID_ * 0x59c] & 0x80) {
                if (popp(reinterpret_cast<char *>(*g_00691b0c),
                         reinterpret_cast<const char *>(g_00687458),
                         0,
                         reinterpret_cast<const char *>(g_0068744c),
                         reinterpret_cast<int(__cdecl *)()>(g_005398e0)) == 0) {
                    return;
                }
            } else {
                if (popp(reinterpret_cast<char *>(*g_00691b0c),
                         reinterpret_cast<const char *>(g_00687474),
                         0,
                         reinterpret_cast<const char *>(g_00687464),
                         reinterpret_cast<int(__cdecl *)()>(g_005398e0)) == 0) {
                    return;
                }
            }
    ...
    if (iVar1 >= g_arr_96cc00[factionID_ * 2099]) {
        iVar1 = g_arr_96cc00[factionID_ * 2099];
    }
    g_arr_96cc28[factionID_ * 2099] += iVar1;

    for (int i = 0; i < 4; ++i) {
        g_arr_96cc08[factionID_ * 2099 + i] = cat[i];
    }

    synch_alloc(factionID_);
    synch_soc(factionID_);
    net_energy(factionID_, -iVar1, 0, 0, 0);
    social_calc(&g_arr_96cc08[factionID_ * 2099], &g_arr_96cc2c[factionID_ * 2099],
                factionID_, 0, 0);
