// ORIGINAL: 0x0042B7E0 FILE
// name      ?set_cat_effect@Datalink@@QAEXXZ
// size      340 bytes
// spans     0x0042B7E0-0x0042B934
// prototype void (__thiscall ?set_cat_effect@Datalink@@QAEXXZ)(Datalink* this)
...
//   build/byte-match/0042b7e0/unit.cpp
// and score it with tools/agent_brief.py.
// GENERATED SKELETON - tools/emit_translation_unit.py
// subject: ?set_cat_effect@Datalink@@QAEXXZ  at 0x0042B7E0  (340 bytes)
//
...
static int *const g_00683330 = (int *)0x00683330;
...
void Datalink::set_cat_effect() {
    char *self = reinterpret_cast<char *>(this);

    int i = 0;
    char *p = reinterpret_cast<char *>(g_00946598);
    do {
        if (p != reinterpret_cast<char *>(g_009466d0)) {
            *reinterpret_cast<uint8_t *>(g_009b86a0) = 0;
            strcat(reinterpret_cast<char *>(g_009b86a0), p);
            if ((*reinterpret_cast<int *>(g_009a64c0) & 0x1000) != 0) {
                strcat(reinterpret_cast<char *>(g_009b86a0), reinterpret_cast<char *>(g_00683330));
                char buf[80];
                _itoa(i + 130000, buf, 10);
                strcat(reinterpret_cast<char *>(g_009b86a0), buf);
            }
            reinterpret_cast<ListBox *>(self + 0xF628)
                ->item(reinterpret_cast<char *>(g_009b86a0), i + 130000);
        }
        p += 0x68;
        ++i;
    } while (p < reinterpret_cast<char *>(g_00946a10));

    RECT rect = *reinterpret_cast<RECT *>(self + 0x103D4);
    Buffer *buf2 = reinterpret_cast<Buffer *>(self + 0x444);
    buf2->box_sprite(&rect, reinterpret_cast<BoxSpriteParams *>(g_0078d618));
    ...
    int name_id = *reinterpret_cast<int *>(reinterpret_cast<char *>(strings_ptr) + 0xB54);
    int str = reinterpret_cast<Strings *>(g_009b90d8)->get(name_id);
    if (str != 0) {
        unsigned int len = strlen(reinterpret_cast<char *>(str));
        buf2->write_cent_l(reinterpret_cast<char *>(str), &rect, len);
    }
