// ORIGINAL: 0x004B0B00 FILE
// name      ?draw_labs_alloc@SocialWin@@QAEXXZ
// size      578 bytes
// spans     0x004B0B00-0x004B0D42
// prototype void (__thiscall ?draw_labs_alloc@SocialWin@@QAEXXZ)(SocialWin* this)
// callers   1   call targets   13
// kind      game
// flags     frame;hidden;sp_ready;purged_ok
// calls     0x005D5930 0x005D8000 0x005DA860 0x005DAC70 0x005DACB0 0x005DD130 0x005E2DD3 0x005E3203 0x005FD2B0 0x006169A0 0x006453E0 0x00645470 0x0064FC88
...
//   build/byte-match/004b0b00/unit.cpp
// and score it with tools/agent_brief.py.
// GENERATED SKELETON - tools/emit_translation_unit.py
// subject: ?draw_labs_alloc@SocialWin@@QAEXXZ  at 0x004B0B00  (578 bytes)
//
...
// EXTERN-SYMBOL LEVER: the four table-of-tables reads below are indexed
// with a register-scaled offset (`[reg*4 + imm]`), not a plain load/store,
// so a `static int *const` would let /O2 fold the address literal and
// drop the addressing mode. An extern array keeps it.
extern int g_0096cd08_arr[];

// The four `test eax,eax; je ...` after `mov eax, 0x9b86a0` test the
// ADDRESS of this scratch buffer, not its contents. Reproducing that needs
// a symbol VC6 cannot constant-fold, unlike the scaffold's
// `static int *const g_009b86a0`, whose value the compiler knows at
// compile time and would fold the always-true test away entirely.
...
void SocialWin::draw_labs_alloc() {
    char *self = reinterpret_cast<char *>(this);
    RECT *srcRect = reinterpret_cast<RECT *>(self + 0x1f270);
    RECT r;
    r.left = srcRect->left;
    r.top = srcRect->top;
    ...
    reinterpret_cast<Buffer *>(g_007aec64)->box_sprite(&r, reinterpret_cast<BoxSpriteParams *>(g_0078d618));

    RECT box2;
    r.right -= 3;
    box2.right = r.right - 0x17;
    r.left += 3;
    r.top += 3;
    box2.left = r.left;
    box2.top = r.top;
    r.bottom -= 3;
    box2.left += 0x17;
    box2.bottom = r.top + 0x14;
    reinterpret_cast<Buffer *>(g_007aec64)->box(&box2, 0xbc, 0xbc);

    g_009b86a0_buf[0] = 0;
    int allocId = g_0096cd08_arr[*reinterpret_cast<int *>(self + 0xcf4) * 2099];

    int msgId = *reinterpret_cast<int *>(*g_009b90f8 + 0xf0);
    char *msg = reinterpret_cast<char *>(reinterpret_cast<Strings *>(g_009b90d8)->get(msgId));
    strcat(g_009b86a0_buf, msg);
    strcat(g_009b86a0_buf, reinterpret_cast<char *>(g_00682820));

    char numbuf[80];
    my_itoa(allocId * 10, numbuf, 10);
    strcat(g_009b86a0_buf, numbuf);
    strcat(g_009b86a0_buf, reinterpret_cast<char *>(g_00682ea0));

    (*reinterpret_cast<CharUpperAFn *>(g_0066931c))(g_009b86a0_buf);

    reinterpret_cast<Buffer *>(g_007aec64)->set_font(reinterpret_cast<Font *>(self + 0x1ee68), 0, 0, 0);
    reinterpret_cast<Buffer *>(g_007aec64)->set_text_color(0x9f, -1, 1, 1);

    if (g_009b86a0_buf) {
        unsigned int len = strlen(g_009b86a0_buf);
        reinterpret_cast<Buffer *>(g_007aec64)->write_cent_l(g_009b86a0_buf, &box2, len);
    }

    int a = r.bottom - *reinterpret_cast<int *>(self + 0x3dac);
    int l0 = r.left + *reinterpret_cast<int *>(self + 0x3da8);
    int rightBound = r.right;
    int bottomBound = r.bottom;
    int l = l0 + (r.right - l0) * allocId / 10;

    if (l < r.right) {
        rightBound = l + (allocId != 0 ? 1 : 0);
        reinterpret_cast<Buffer *>(g_007aec64)->tile(reinterpret_cast<Buffer *>(self + 0x4a00),
            l, a, l, a, r.right - l, r.bottom - a);
    }
    if (l0 < rightBound) {
        reinterpret_cast<Buffer *>(g_007aec64)->tile(reinterpret_cast<Buffer *>(self + 0x4f88),
            l0, a, l0, a, rightBound - l0, bottomBound - a);
    }
