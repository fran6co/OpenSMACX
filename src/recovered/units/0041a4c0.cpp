// ORIGINAL: 0x0041A4C0 ?exit@BaseWin@@QAEXXZ 0x0041A4C0-0x0041A609 FILE
// size      329 bytes
...
//   build/byte-match/0041a4c0/unit.cpp
// and score it with tools/agent_brief.py.
// GENERATED SKELETON - tools/emit_translation_unit.py
// subject: ?exit@BaseWin@@QAEXXZ  at 0x0041A4C0  (329 bytes)
//
...
// INDEXED TABLE BASE: read directly with `[reg*4 + ADDR]`, so the emitter
// did not pick these up as plain globals.
...
void BaseWin::exit() {
    ...
    *reinterpret_cast<int *>(self + 0x40B28) = 0;
    ...
    int idx = *g_0093928c;
    int x = g_00952828[idx * 26 + 1];
    int y = g_00952828[idx * 26 + 0];
    int mapH = *g_00949870;

    if (x >= 0 && x < *g_00949874 && y >= 0 && y < mapH) {
        uint8_t *base = *reinterpret_cast<uint8_t **>(g_0094a30c);
        int stride = *g_0068faf0;
        int cell = stride * x + (y >> 1);
        uint8_t terrain = base[cell * 11 * 4];
        int hi = terrain & 0xE0;
        if (hi > 0x60) {
            int shifted = terrain >> 5;
            if (x - shifted + 3 < *g_00933458 + 2)
                goto center_view;
        }
        mapH = *g_00949870;
    }
    ...
    if (x < *g_00933458 + 2)
        goto center_view;
    if (x >= *g_00933480 + *g_00933488 + *g_00933458 - 2)
        goto center_view;

    if ((*reinterpret_cast<uint8_t *>(g_0094988c) & 1) == 0) {
        if (y < *g_00933454 + 2)
            y += mapH;
        if (y >= *g_0093347c + *g_00933484 + *g_00933454 - 2)
            y -= mapH;
    }
    if (y < *g_00933454 + 2)
        goto center_view;
