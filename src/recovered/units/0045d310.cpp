...
// calls     0x005D8000
// indirect  0x0045D356 0x0045D379
...
//   build/byte-match/0045d310/unit.cpp
// and score it with tools/agent_brief.py.
// GENERATED SKELETON - tools/emit_translation_unit.py
// subject: ?set_iface_mode@SubInterface@@QAEXXZ  at 0x0045D310  (109 bytes)
//
...
class VCall { public:
    virtual void slot000(int);  // <-- used, tuned: takes one int
};
...
void SubInterface::set_iface_mode() {
    ...
    int idx = *g_007d392c;
    int mode = *reinterpret_cast<int *>(self + 4);
    int cur = g_007ae778[idx];
    if (cur != mode && (mode != 10 || cur == 0)) {
        if (mode != cur) {
            VCall *obj = reinterpret_cast<VCall *>(g_007ae7f0[cur]);
            ++idx;
            *g_007d392c = idx;
            mode = *reinterpret_cast<int *>(self + 4);
            g_007ae778[idx] = mode;
            if (obj != 0) {
                obj->slot000(0);
            }
        ...
        mode = *reinterpret_cast<int *>(self + 4);
        VCall *obj2 = reinterpret_cast<VCall *>(g_007ae7f0[mode]);
        if (obj2 != 0) {
            obj2->slot000(1);
        }
    }
