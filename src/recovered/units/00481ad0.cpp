...
// calls     0x0047F960 0x0047FED0 0x00480180 0x00480370 0x00480900 0x005882F0 0x005D5890 0x005DAC70 0x005DACB0 0x005DD130 0x005E2DD3 0x005F7E90 0x006169A0 0x006453E0 0x00645470
// indirect  0x00481B92
...
//   build/byte-match/00481ad0/unit.cpp
// and score it with tools/agent_brief.py.
// GENERATED SKELETON - tools/emit_translation_unit.py
// subject: ?on_redraw@NetWin@@QAEXXZ  at 0x00481AD0  (384 bytes)
//
...
// This address's own address does work: the null-test on it would fold
// away if the compiler could see it is a nonzero literal, so it needs the
// extern-symbol lever, not the plain pointer constant above.
extern char g_009b86a0_str[];

void NetWin::on_redraw() {
    char *self = reinterpret_cast<char *>(this);
    if (reinterpret_cast<Win *>(this)->is_visible()) {
        Buffer *buf = reinterpret_cast<Buffer *>(self + 0x444);

        buf->box_sprite(reinterpret_cast<RECT *>(self + 0x474),
                        reinterpret_cast<BoxSpriteParams *>(g_0078d690));

        RECT localRect;
        localRect.left = *reinterpret_cast<long *>(self + 0xA14);
        localRect.top = *reinterpret_cast<long *>(self + 0xA18);
        localRect.right = *reinterpret_cast<long *>(self + 0xA1C);
        localRect.bottom = *reinterpret_cast<long *>(self + 0xA20);
        buf->box_sprite(&localRect,
        ...
        *reinterpret_cast<char *>(g_009b86a0) = 0;
        int idx = *reinterpret_cast<int *>(base + 0x3F0);
        int s = reinterpret_cast<Strings *>(g_009b90d8)->get(idx);
        strcat(reinterpret_cast<char *>(g_009b86a0),
               reinterpret_cast<char *>(s));
        typedef char *(__stdcall * CharUpperAFn)(char *);
        (*reinterpret_cast<CharUpperAFn *>(g_0066931c))(
        ...
        if (g_009b86a0_str != 0) {
            unsigned int len = strlen(g_009b86a0_str);
            buf->write_cent_l(g_009b86a0_str, &localRect, len);
        }
