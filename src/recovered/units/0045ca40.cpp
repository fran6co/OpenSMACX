...
// indirect  0x0045CABE 0x0045CAE5 0x0045CB7A 0x0045CBAD 0x0045CBD0 0x0045CBE9 0x0045CC42
...
//   build/byte-match/0045ca40/unit.cpp
// and score it with tools/agent_brief.py.
// GENERATED SKELETON - tools/emit_translation_unit.py
// subject: ?on_status@MainInterface@@QAEXH@Z  at 0x0045CA40  (568 bytes)
//
...
static int *const g_007d3944 = (int *)0x007D3944;
...
// EXTERN-SYMBOL LEVER: 0x7ae778 is indexed with a register-scaled offset
// (`[reg*4 + imm]`), not a plain load/store, so a `static int *const` would
// let /O2 fold the address literal and drop the addressing mode.
extern int g_007ae778_arr[];

// EXTERN-SYMBOL LEVER: 0x7f685c's ADDRESS is added to a loaded offset
// (`add ecx, 0x7f685c`), not just dereferenced, so it needs real linkage -
// a `static int *const` would let /O2 fold the literal away.
extern int g_007f685c_sym;

// Second shim: slot 1 is called both nullary (elsewhere, not here) and with
// one int argument here; VCall declares it nullary. Same layout, only that
// slot re-signed.
class VCall2 { public:
    virtual void slot000();
...
void MainInterface::on_status(int a1) {
    char *self = reinterpret_cast<char *>(this);

    if (a1 == 0) {
        reinterpret_cast<Time *>(g_007ad2d0)->stop();

        int status = g_007ae778_arr[*g_007d392c];
        if (status != 0xa && status != 3 && status != 2) {
            stop_timers();
            *g_007d3944 = 1;
        }
        ...
        int off = *reinterpret_cast<int *>(*g_007f685c + 4);
        reinterpret_cast<Win *>(reinterpret_cast<char *>(&g_007f685c_sym) + off)->hide();

        for (int i = 0; i < 0x2a; ++i) {
            reinterpret_cast<VCall *>(self + 0x3ec + i * 2892)->slot002();
        }

        if (g_007ae778_arr[*g_007d392c] != 0xa) {
            reinterpret_cast<VCall *>(self + 0x1e0b8)->slot002();
        }

        int status2 = g_007ae778_arr[*g_007d392c];
        if (!(status2 == 0xa ||
              (status2 == 5 && (*g_00885f40 == 4 || *g_00885f40 == 5)))) {
            reinterpret_cast<WorldWin *>(g_008e9f60)->hide_all();
            return;
        }
    } else {
        reinterpret_cast<Flic *>(self + 0x23b3c)->rewind();
        ...
        if (*g_007cd11c == 0x3e8) {
            int off = *reinterpret_cast<int *>(*g_007f685c + 4);
            reinterpret_cast<Win *>(reinterpret_cast<char *>(&g_007f685c_sym) + off)->show(0);
        }
