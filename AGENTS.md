...
- Thunks: 35.
- Current recovery backlog: 5,121 candidates.
- Current local legacy-island count: 149, reduced from 174.
- Source-recovery `DllMain` redirects: 29. The gameplay gate installs inactive-pass-through active-turn and post-increment upkeep hooks plus two call-site hooks; scenario behavior activates only when its environment is configured.
...
- Regenerated state after BasePop string-font recovery is 5,121 priorities and 149 islands.
...
- Legacy-island extraction separately remains bound to the independently analyzed pre-PRACX executable and produces 149 islands.
...
- `src/dllmain.cpp`: transactional signature-checked redirects; 29 source recoveries plus the gameplay gate's active-turn, post-increment upkeep, and call-site hooks.
...
- `docs/recovery/priorities.csv`: currently regenerated to 5,121 candidates.
...
- `build/mingw-i686-release/legacy-leaves/manifest.json`: current ignored 149-island manifest.
