...
- Do not pass `--force` as a matter of course. It bypasses both the stamp cache and the export checkpoint, turning a one-second verification into a multi-minute IDB parse, and it buys nothing: the ordinary path already regenerates and fails the byte comparison whenever an input or a committed output has drifted. The correct promotion loop is to run without `--force`, let it report the difference, copy the regenerated catalogs into `docs/recovery/`, and run again - the second run reuses the canonical export and stamps. Measured on this tree: cached 1.0s, reused-export after a line-shifting source edit 2.4s, `--force` several minutes.
