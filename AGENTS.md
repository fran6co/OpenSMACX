...
- the retired `verify_wine_test_locks`: checked the generated `CTestTestfile.cmake` so that every test invoking `run_windows_test.py` held `RESOURCE_LOCK wineprefix`; a Wine test missing from that list does not fail under `ctest -j`, it flakes. Retired 2026-08-13 with one wine-backed test left, so `OPENSMACX_WINE_LOCKED_TESTS` is now maintained BY HAND — see `docs/RETIRED_ROUTES.md`, which records that this trades a mechanical check for the defect shape this repository calls its highest-yield.
