...
- Thunks: 35.
- Current recovery backlog: 5,117 candidates.
- Current local legacy-island count: 147, reduced from 174.
- Source-recovery `DllMain` redirects: 33. The gameplay gate installs inactive-pass-through active-turn and post-increment upkeep hooks plus two call-site hooks; scenario behavior activates only when its environment is configured.
...
- Regenerated state after Dialog position-to-ID recovery is 5,117 priorities and 147 islands.
...
- Legacy-island extraction separately remains bound to the independently analyzed pre-PRACX executable and produces 147 islands.
...
- `src/dllmain.cpp`: transactional signature-checked redirects; 33 source recoveries plus the gameplay gate's active-turn, post-increment upkeep, and call-site hooks.
...
- `docs/recovery/priorities.csv`: currently regenerated to 5,117 candidates.
...
- `build/mingw-i686-release/legacy-leaves/manifest.json`: current ignored 147-island manifest.
